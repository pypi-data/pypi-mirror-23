...
#define TOO_SMALL(what, i, ierror)  RAISE_DELAYED_ERROR("Internal neighbor list exhausted, *" // what // "* too small: " // "nebtot = " // nebtot // "/" // nebsize // ", nebmax = " // nebmax // ", nebavg = " // nebavg // ", this%neb_last(i)-this%neb_seed(i)+1 = " // (this%neb_last(i)-this%neb_seed(i)+1) // ", i = " // i // "/" // natloc // " (" // nat // ")", ierror) ; nebtot = 1 ; this%neb_last(i) = this%neb_seed(i)
  ...
  recursive subroutine BOP_KERNEL( &
    ...
    real(DP) :: fipjkl, fimjkl, fijpkl, fijmkl
    real(DP) :: fijkpl, fijkml, fijklp, fijklm
#endif
...
!                   write (*, '(A,2I3,4F15.7)')  "(ji) P "//rebo2_el_strs(ktypj)//" "//rebo2_el_strs(ktypi)//":", j, i, nj(rebo2_H_), nj(rebo2_C_), nj(rebo2_Si_), pji
                   ...
                   if (ijpot == C_C) then
                      ...
                      call eval(this%Fsisi, nti+eps, ntj, nconji, nconjj, &
                           fipjkl, dfdni, dfdnj, dfdncni, dfdncnj)
                      call eval(this%Fsisi, nti-eps, ntj, nconji, nconjj, &
                           fimjkl, dfdni, dfdnj, dfdncni, dfdncnj)
                      call eval(this%Fsisi, nti, ntj+eps, nconji, nconjj, &
                           fijpkl, dfdni, dfdnj, dfdncni, dfdncnj)
                      call eval(this%Fsisi, nti, ntj-eps, nconji, nconjj, &
                           fijmkl, dfdni, dfdnj, dfdncni, dfdncnj)
                      call eval(this%Fsisi, nti, ntj, nconji+eps, nconjj, &
                           fijkpl, dfdni, dfdnj, dfdncni, dfdncnj)
                      call eval(this%Fsisi, nti, ntj, nconji-eps, nconjj, &
                           fijkml, dfdni, dfdnj, dfdncni, dfdncnj)
                      call eval(this%Fsisi, nti, ntj, nconji, nconjj+eps, &
                           fijklp, dfdni, dfdnj, dfdncni, dfdncnj)
                      call eval(this%Fsisi, nti, ntj, nconji, nconjj-eps, &
                           fijklm, dfdni, dfdnj, dfdncni, dfdncnj)

                      call eval(this%Fsisi, nti, ntj, nconji, nconjj, &
                           fij, dfdni, dfdnj, dfdncni, dfdncnj)

                      if (abs((fipjkl-fimjkl)/(2*eps)-dfdni) > eps) then
                         write (*, *)  "i", (fipjkl-fimjkl)/(2*eps), dfdni, abs((fipjkl-fimjkl)/(2*eps)-dfdni)
                      endif
                      if (abs((fijpkl-fijmkl)/(2*eps)-dfdnj) > eps) then
                         write (*, *)  "j", (fijpkl-fijmkl)/(2*eps), dfdnj, abs((fijpkl-fijmkl)/(2*eps)-dfdnj)
                      endif
                      if (abs((fijkpl-fijkml)/(2*eps)-dfdncni) > eps) then
                         write (*, *)  "ncni", (fijkpl-fijkml)/(2*eps), dfdncni, abs((fijkpl-fijkml)/(2*eps)-dfdncni)
                      endif
                      if (abs((fijklp-fijklm)/(2*eps)-dfdncnj) > eps) then
                         write (*, *)  "ncnj", (fijklp-fijklm)/(2*eps), dfdncnj, abs((fijklp-fijklm)/(2*eps)-dfdncnj)
                      endif
