...
class SupraListView(ListView, SupraAuthenticationMixin):
        ...
        @classmethod
        def as_url(cls):
                app = cls.model._name_.lower()
                return url('%s/list/$' % (app, ), cls.as_view(),)
        #end class

        def _ini_(self, dict_only = False, *args, **kwargs):
                self.dict_only = dict_only
                return super(SupraListView, self)._init_(*args, **kwargs)
        #end def
        ...
        def get_object_list(self, object_list):
                ...
                if self.list_display:
                        ...
                        if hasattr(self, 'Renderer'):
                                renderers = dict((key, F(value)) for key, value in self.Renderer._dict.iteritems() if not callable(value) and not key.startswith('_'))
...
class SupraDetailView(DetailView, SupraAuthenticationMixin):
        ...
        def _ini_(self, dict_only = False, *args, **kwargs):
                self.dict_only = dict_only
                return super(SupraDetailView, self)._init_(*args, **kwargs)
        #end def
        ...
        def dispatch(self, request, *args, **kwargs):
                ...
                # end if
                if hasattr(self, 'Renderer'):
                        renderers = dict((key, value) for key, value in self.Renderer._dict.iteritems() if not key.startswith('_'))
...
def all_supras(view):
        def is_for_supra(obj):
                ...
                return False
        #end def
        supras = inspect.getmembers(sys.modules[view._name_], is_for_supra)
        urls = []
