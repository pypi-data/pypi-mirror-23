...
def get_initial_user_parameters_from_command_line(metric_path, run_uuid, logger):
    select = '##### Initial USER parameters from Command Line:'
    df = select_tsv_to_df(metric_path, select, run_uuid, logger)
    return df

def get_all_user_parameters_from_command_line(metric_path, run_uuid, logger):
