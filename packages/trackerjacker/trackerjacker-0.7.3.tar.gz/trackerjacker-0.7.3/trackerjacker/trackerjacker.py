...
class Dot11Frame:
    ...
    def __init__(self, frame):
        ...
        if frame.haslayer(RadioTap):
            self.signal_strength = pkt[RadioTap].dbm_antsignal
            self.channel = int(pkt[RadioTap].channel_freq)
        else:
    ...
    def __str__(self):
        return 'Dot11 (type={}, from={}, to={}, bssid={}, ssid={}, signal_strength={}, chan={})'.format(
               self.type_name(), self.src, self.dst, self.bssid, self.ssid, self.signal_strengh, self.channel)
...
class Dot11Map:
    ...
    def add_frame(self, dot11_frame):
        if dot11_frame.channel not in self.map_data:
            self.map_data[dot11_frame.channel] = {'unassociated': {'ssid': None, 'macs': set()}}

        chan_to_bssid = self.map_data[dot11_frame.channel]
        ...
        self.check_for_new_stuff(dot11_frame)

    def check_for_new_stuff(self, frame):
        unseen_macs = (frame.macs - set([None])) - self.macs_seen
        self.macs_seen |= unseen_macs
        for mac in unseen_macs:
            self.logger.info('MAC found: {}, Channel: {}'.format(mac, frame.channel))

        if frame.ssid and frame.ssid not in self.ssids_seen:
            self.ssids_seen |= set([frame.ssid])
            self.logger.info('SSID found: {}, BSSID: {}, Channel: {}'.format(frame.ssid, frame.bssid, frame.channel))
...
class TrackerJacker:
    ...
    def process_packet(self, pkt):
        if pkt.haslayer(Dot11):
            ...
            if self.do_map:
                self.dot11_map.add_frame(dot11_frame)
