...
def secure_copy(user, host, src, dest,  # pylint: disable=too-many-arguments
    ...
    ssh.connect(host, username=user, pkey=pkey, look_for_keys=False)
...
def az_get_k8s_credentials(resource_group, cluster_name, ssh_key_path):
    ...
    :return: None
    """
    print("Downloading kubeconfig file to {} using {}".format(os.path.expanduser('~'),
                                                              ssh_key_path))
    ...
    while True:
        try:
            ...
            break
        except (paramiko.ssh_exception.AuthenticationException, socket.error) as exc:
            try_no += 1
            if try_no >= retries:
                raise CLIError(str(exc))
            timestamp = time.strftime('%Y-%m-%d %H:%M:%S')
            print('[{}] Failed downloading kubeconfig: {}'.format(timestamp, exc))
