...
def setup_k8s(context, root_name, resource_group, acr_login_server, acr_password, ssh_public_key,
    ...
    try:
        ...
        deploy_batch_frontend(k8s_ops, storage_account_name, storage_account_key,
...
def test_acs_k8s(context):
    try:
        get_k8s_realtime_frontend_url(context)
        return True
    except ApiException:
        print('')
...
def get_k8s_realtime_frontend_url(context):
    ...
    environment_var = {'AML_K8S_REALTIME_URL': frontend_url}
    write_variables_to_amlenvrc(context, environment_var, 'a+')
...
def batch_get_k8s_frontend_url(context):
    ...
    environment_var = {'AML_K8S_BATCH_URL': frontend_url}
    write_variables_to_amlenvrc(context, environment_var, 'a+')
