...
def env_cluster(verb, context=CommandLineInterfaceContext(), quiet=False):
    ...
    if not context.env_is_k8s:
        if not context.acs_master_url:
            print("No cluster available in your environment. To set up a cluster environment, run 'az ml env setup --cluster'.") # pylint: disable=line-too-long
            return
        try:
        ...
        basename = context.base_name or context.az_account_name[:-4]
        if not os.path.isfile(K8sConstants.KUBE_CONFIG_PATH) or not KubernetesOperations.set_current_context(basename):
            print("No .kube config entry found for {} at {}. Attempting to download.".format(basename, K8sConstants.KUBE_CONFIG_PATH))
        ...
        if not test_acs_k8s():
            raise CLIError("Unable to connect to Kubernetes Cluster.")
...
def env_setup(status, name, mesos, cluster, service_principal_app_id,
            ...
            if completed_deployment:
                try:
                    ...
                    if acs_master and acs_agent:
                        ...
                        env_statements = {'AML_ACS_MASTER': acs_master,
                                          'AML_ACS_AGENT': acs_agent}

                        write_variables_to_amlenvrc(context, env_statements, 'a+')
    ...
    if cluster:
        if not mesos:
            ...
            if k8s_configured is True:
                env_statements['AML_ACS_IS_K8S'] = True
        else:
    ...
    env_statements['AML_ACR_USER'] = context.acr_username
