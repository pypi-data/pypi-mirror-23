...
from ._realtimeutilities import realtime_view_service_header_to_fn_dict
from ._realtimeutilities import realtime_view_service_details_header_to_fn_dict
...
from .._constants import MMS_ENDPOINT_URL
...
def realtime_service_deploy_local(context, service_label, image, verbose, app_insights_enabled, model_data_collection_enabled):
    ...
    time.sleep(10)
    swagger_url = Constants.swagger_uri_format.format("http://127.0.0.1:{}".format(dockerport))
    input_data = get_sample_data(swagger_url, verbose=verbose)
    if not input_data:
        input_data = Constants.default_input_data
...
def realtime_service_run_local(service_name, input_data, verbose):
    ...
    try:
        ...
        if input_data == '':
            ...
            swagger_url = Constants.swagger_uri_format.format("http://127.0.0.1:{}".format(container_port))
...
def realtime_service_create(driver_file, dependencies, requirements, schema_file, service_name, custom_ice_url,
                            ...
                            num_replicas, package_id, model_name, conda_file, verb, context=cli_context, use_mms=False):
    _realtime_service_create(driver_file, dependencies, requirements, schema_file, service_name, custom_ice_url,
                             target_runtime, app_insights_logging_enabled, model_data_collection_enabled, model_file,
                             num_replicas, package_id, model_name, conda_file, verb, context, use_mms)
...
def _realtime_service_create(driver_file, dependencies, requirements, schema_file, service_name, custom_ice_url,
    ...
    if use_mms:
        return _mms_service_create(package_id, service_name, driver_file, model_name, model_file, schema_file,
                                   dependencies, target_runtime, requirements, app_insights_enabled,
                                   model_data_collection_enabled, verb, context)
    else:
        """Create a new realtime web service."""
        image = create_docker_image(driver_file, dependencies, service_name, verb, target_runtime, context, requirements,
                                    schema_file, model_file, custom_ice_url, conda_file)
...
def realtime_service_deploy(context, image, app_id, app_insights_enabled, verbose):
    ...
    print("Success.")
    print("Usage: az ml service run realtime -n " + app_id + " -d \"" + Constants.default_input_data + "\"")
    return SUCCESS_RETURN_CODE
...
def realtime_service_view(service_name, service_id, verb=False, context=cli_context):
    _realtime_service_view(service_name, service_id, verb, context)


def _realtime_service_view(service_name, service_id, verb=False, context=cli_context):
    ...
    if context.in_local_mode():
        ...
        if scoring_port:
            ...
            scoring_url = Constants.scoring_uri_format.format(service_host)
            swagger_url = Constants.swagger_uri_format.format(service_host)
            headers = {'Content-Type': 'application/json'}
        else:
    ...
    if not sample_data:
        sample_data = Constants.default_input_data
    if num_services:
        print('Usage:')
...
def realtime_service_run_cluster(context, service_name, input_data, verbose):
    ...
    if input_data == '':
        swagger_url = Constants.swagger_uri_format.format("{}:9091".format(context.acs_agent_url))
        input_data = get_sample_data(swagger_url, headers, verbose)
...
def realtime_service_run_kubernetes(context, service_name, service_id, input_data, verbose):
    ...
    if service_id:
        ...
        try:
            resp = context.http_call('get', endpoint_url, headers=mms_headers, timeout=MMS_SYNC_TIMEOUT_SECONDS)
        except requests.ConnectionError:
            raise CLIError('Error connecting to {}.'.format(endpoint_url))
        except requests.Timeout:
            raise CLIError('Error, request to {} timed out.'.format(endpoint_url))
    ...
    result = requests.post(scoring_endpoint, data=input_data, headers=scoring_headers)
    if verbose:
        print(result.content)
...
def realtime_service_update(service_id, package_id, driver_file, model_name, model_file, schema_file, dependencies,
                            target_runtime, requirements, verb, context=cli_context):
    _realtime_service_update(service_id, package_id, driver_file, model_name, model_file, schema_file, dependencies,
                             target_runtime, requirements, verb, context)


def _realtime_service_update(service_id, package_id, driver_file, model_name, model_file, schema_file, dependencies,
                             target_runtime, requirements, verb, context=cli_context):
    return _mms_service_update(service_id, package_id, driver_file, model_name, model_file, schema_file, dependencies,
                               target_runtime, requirements, verb, context)


# MMS functions
def _mms_service_create(package_id, service_name, driver_file, model_name, model_file, schema_file, dependencies,
                        runtime, requirements, app_insights_enabled, model_data_collection_enabled, verb, context):
    ...
    endpoint_url = MMS_ENDPOINT_URL.format(base_mms_url, subscription, resource_group, host_account_name)

    if not package_id:
        package_create_success, package_id = _package_create(driver_file, None, model_name, model_file, None, None,
                                                             None, schema_file, dependencies, runtime, requirements,
                                                             verb, context)
        if package_create_success is not SUCCESS_RETURN_CODE:
            return

    if context.in_local_mode():
        image = '{}/{}:latest'.format(context.acr_home, package_id)
        return realtime_service_deploy_local(context, service_name, image, verb, app_insights_enabled, model_data_collection_enabled)
    ...
    body = {'packageId': package_id, 'appName': service_name, 'kubeConfig': kube_config_string,
            'acrImagePullSecret': context.acr_user + 'acrkey', 'acsInfo': {'acsMasterUrl': context.acs_master_url,
                                                                           'acsAgentUrl': context.acs_agent_url}}

    try:
        resp = context.http_call('post', endpoint_url, headers=headers, json=body, timeout=MMS_SYNC_TIMEOUT_SECONDS)
    except requests.ConnectionError:
        raise CLIError('Error connecting to {}.'.format(endpoint_url))
    except requests.Timeout:
        raise CLIError('Error, request to {} timed out.'.format(endpoint_url))
...
def _mms_service_show(service_id, verb, context):
    ...
    try:
        resp = context.http_call('get', endpoint_url, headers=headers, timeout=MMS_SYNC_TIMEOUT_SECONDS)
    except requests.ConnectionError:
        raise CLIError('Error connecting to {}.'.format(endpoint_url))
    except requests.Timeout:
        raise CLIError('Error, request to {} timed out.'.format(endpoint_url))
    ...
    if resp.status_code == 200:
        print(get_success_and_resp_str(context, resp,
                                       response_obj=MultiTableResponse([realtime_view_service_header_to_fn_dict,
                                                                        realtime_view_service_details_header_to_fn_dict]))[1])
        return SUCCESS_RETURN_CODE
    else:
...
def _mms_service_list(verb, context):
    ...
    if resp.status_code == 200:
        print(get_success_and_resp_str(context, resp, response_obj=TableResponse(realtime_list_service_header_to_fn_dict))[1])
        return SUCCESS_RETURN_CODE
    else:
...
def _mms_service_delete(service_id, verb, context):
    ...
    try:
        resp = context.http_call('delete', endpoint_url, headers=headers, timeout=MMS_SYNC_TIMEOUT_SECONDS)
    except requests.ConnectionError:
        raise CLIError('Error connecting to {}'.format(endpoint_url))
    except requests.Timeout:
        raise CLIError('Error, request to {} timed out.'.format(endpoint_url))
...
def _mms_service_update(service_id, package_id, driver_file, model_name, model_file, schema_file, dependencies,
                        target_runtime, requirements, verb, context):
    base_url, subscription, resource_group, host_account_name = get_sub_and_account_info()
    auth_token = get_host_account_token()
    headers = {'Content-Type': 'application/json', 'Authorization': 'Bearer {}'.format(auth_token)}
    ...
    if not package_id:
        package_create_success, package_id = _package_create(driver_file, None, model_name, model_file, None,
                                                             None, None, schema_file, dependencies, target_runtime,
                                                             requirements, verb, context)

    body = {'packageId': package_id, 'acrImagePullSecret': context.acr_user + 'acrkey'}
    try:
        resp = context.http_call('put', endpoint_url, headers=headers, json=body, timeout=MMS_SYNC_TIMEOUT_SECONDS)
    except requests.ConnectionError:
        raise CLIError('Error connecting to {}.'.format(endpoint_url))
    except requests.Timeout:
        raise CLIError('Error, request to {} timed out.'.format(endpoint_url))
