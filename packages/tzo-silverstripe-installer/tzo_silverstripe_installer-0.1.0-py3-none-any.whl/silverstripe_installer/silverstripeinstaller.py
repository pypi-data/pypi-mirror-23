...
class SilverStripeInstaller:
    ...
    def __init__(self, repo):
        ...
        _gen_ss = input('Should generate local _ss_environment.php? (default: yes):').lower()
        while True:
            if _gen_ss == 'yes' or _gen_ss == 'no':
                self._gen_ss = _gen_ss
                break
    ...
    def create_gitlab_repo(self):
        ...
        except TZOGitLabException:
            print('Error when try to connect to GitLab. Please check if your GitLab api token is correct(location {}).'
                  .format(self.gitlab_credential_path))
            exit(1)
