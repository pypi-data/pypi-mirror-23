...
@click.option('-v', '--debug', count=True, help="Turn on debugging. More v's, more debugging")
@click.pass_context
def upload(ctx, path, recursive, skip_check_version, to_service, service_url, exclude_directory_pattern, debug):
    """Quick uploader"""
    set_debug_param(debug)
    if recursive:
        log.info('uploading recursively from: %s', path)
        upload_recursive(path, connection=ctx.obj, exclude_directory_pattern=exclude_directory_pattern)
    else:
        graph = from_pickle(path, check_version=(not skip_check_version))
        if to_service:
            to_pybel_web(graph, service_url)
        else:
            to_database(graph, connection=ctx.obj)
...
@click.pass_context
def convert(ctx, enable_upload, store_parts, no_enrich_authors, no_enrich_genes, no_enrich_go,
            no_citation_clearing, allow_nested, directory, use_stdin, send_pybel_web, exclude_directory_pattern, debug,
            cool):
    ...
    if use_stdin:
        ...
        convert_directory(directory, exclude_directory_pattern=exclude_directory_pattern, **kwargs)
...
def upload_resources(directory):
