...
def _guess_real_stat_type_and_reason(vals):
    ...
    # Compute the correlation of the column's values and their logs with normal
    # quantiles.
    q = norm.ppf(np.linspace(1, len(x), num=len(vals)) / (len(x) + 1))
    ...
    # If both normal and lognormal quantiles match the distribution of the
    # values well, return realAdditive. Otherwise, pick realMultiplicative if
    # lognormal quantiles match better, and realAdditive if normal quantiles
    # match better.
    reason = 'normal cor. %.3f, lognormal cor. %.3f' % (q_cor, lq_cor)
