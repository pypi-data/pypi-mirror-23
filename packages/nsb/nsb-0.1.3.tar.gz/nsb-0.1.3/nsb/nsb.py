...
__copyright__ = "Copyright 2017, Matthias Büchele"
__credits__ = ["Matthias Buechele"]
__license__ = "GNU GPL v3"
__shortname__ = "nsb"
__longname__ = ("Draws nightsky allskymaps corresponding to KRISCIUNAS Model of the Brightness of Moonlight,\n"
...
__version__ = "0.0.2"
__maintainer__ = "Matthias Buechele"
__email__ = "matthias.buechele[at]fau[dot]de"
...
from nsb import help
from nsb import input_options
from nsb import fitsimage
from nsb import config
from nsb import nsbtools
from nsb import mypycat
from nsb import gaia


def main():
    Help = help.PlatformHelp()
    Options = input_options.ReadOptions(sys.argv)
    try:
    ...
    Help.show_title()
    Configuration = config.TheConfiguration()
    ...
    Image_allsky = fitsimage.FitsImage(size, 180, 90.0, 0.0, None, lon, lat, elevation, date_and_time, B_zero, k, use_mag)
    Image_FOV = fitsimage.FitsImage(size, fov, alt, az, source, lon, lat, elevation, date_and_time, B_zero, k, use_mag)

    print("* Moon at \tzen %.2f\taz %.2f\twith a phase of %.2f %% (%.2f deg)" % (
          math.degrees(Image_FOV.zen_moon), math.degrees(Image_FOV.az_moon), 100 * Image_FOV.moon.moon_phase,
          math.degrees(Image_FOV.moon_alpha)))

    gaiamap = gaia.Gaia(level)
    ...
    if Options.savefits:
        Image_FOV.fitsfile.writeto(output_dir
                                   + "NSB_of_"
                                   + nsbtools.makeDateString(date_and_time)
    ...
    if Options.noscreen:
        ...
        nsbtools.plotMaps([Image_allsky, Image_FOV])#, Image_ObsModel])
