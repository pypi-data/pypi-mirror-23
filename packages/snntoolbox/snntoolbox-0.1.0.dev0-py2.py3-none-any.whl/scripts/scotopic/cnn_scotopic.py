...
Demonstrates scotopic network on CIFAR-10 or MNIST.
...
import keras
import numpy as np
from snntoolbox.scotopic.cnn_scotopic_init import cnn_scotopic_init

from scripts.scotopic.config import settings_scotopic
...
def cnn_scotopic():
    ...
    if settings_scotopic['estimate_ppp']:
        settings_scotopic['sp']['ppp_estimator'] = np.load(os.path.join(
            settings['path_cw'], settings_scotopic['dataset'] +
                                 '_PPPEstimator.mat'))

    # Prepare model and data
    net = keras.models.load_model(os.path.join(settings['path_cw'],
                                               settings['filename_ANN']))
    net = cnn_scotopic_init(net)

    imdb = get_dataset(settings)
    ...
    % testinfo = cnn_scotopic_mnist_inceval(net, imdb, getBatch(opts), opts,
                                            find(imdb.images.set == 3));
    return;
    % testinfo = cnn_collect(net, getBatch(opts), opts);
    return;
    testinfo.IT = cnn_test(net, imdb, getBatch(opts), opts,
                           find(imdb.images.set == 3)); % interrogation
    testinfo.FR = SAT_constant_threshold(testinfo.IT.PPPArray,
                                         testinfo.IT.maxlogP_persample, ...
    testinfo.IT.acc_persample, opts.thArray, opts); % free
    response
    testinfo.OFR = SAT_opt_thresohld(testinfo.IT, opts); % free
    response
    with optimized parameters

    function
    net = load_test_net(expDir, info)
    [~, mid] = min(info.val.error(1,:));
    fprintf('Test using epoch %d\n', mid);
    load(fullfile(expDir, sprintf('net-epoch-%d.mat', mid)), 'net');

    function
    printSavefile(functionName, savefile)
    fprintf('%s: saved results to\n%s\n', functionName, savefile);

    function
    savefile = getSaveFile(opts, funcSig)
    % funcSig: signature
    of
    ...
    funcSig, estStr, length(
        opts.sp.PPPArray), opts.sp.snr, opts.sp.amp_sigma, opts.sp.fpn_sigma))
    end

    function
    info = cnn_test(net, imdb, getBatch, opts, test)
    ...
    net.layers
    {l}.weights
    {ii} = interp1(f(opts.sp.trainPPPArray), ...
    net.layers
    {l}.weights
    {ii}
    ', f(testPPPArray), interpmode)';
    end
    net.layers
    {l}.weights
    {3} = permute(interp1(f(opts.sp.trainPPPArray), ...
    permute(net.layers
    {l}.weights
    {3}, [3 1 2]), f(testPPPArray), interpmode), ...
    [2
    3
    1]);
    net.layers
    {l}.PPPArray = testPPPArray;
    end
    end
    end
    ...
    for i = 1:numBatches
    idxes
    {i} = (i - 1) * batchSize + 1:min(i * batchSize, numel(test));
    batches
    {i} = test(idxes
    {i});
    end
    ...
    {end - 1}.precious = true;
    PPPEstimator = [];
    if opts.sp.estimate_PPP, PPPEstimator = opts.sp.PPPEstimator;
    end
    ...
    [im, label] = getBatch(imdb, batches
    {i});
    res = vl_simplenn(net, im, [], [], 'mode', 'test', 'conserveMemory', true);
    else % estimate
    PPP
    [im, label, raw] = getBatch(imdb, batches
    {i});
    [~, PPP_I] = estimate_PPP(dataset, raw, PPPEstimator, testPPPArray); % force
    the
    estimates
    to
    be
    listed in testPPPArray
    res = vl_simplenn_estimate_PPP(net, im, PPP_I, [], [], 'mode', 'test',
                                   'conserveMemory', true);
    end
    batchSize = length(batches
    {i});
    labels
    {i} = label(1:batchSize);
    % compute
    ...
    allLogP
    {i} = bsxfun( @ minus, logP, logsumexp(logP, 1));
    ...
    % -------------------------------------------------------------------------
    function
    res = vl_simplenn_estimate_PPP(net, im, PPP_I, varargin)
    for i=1:length(net.layers)
    if strcmp(net.layers
    {1}.type, 'custom')
    net.layers
    {1}.PPP_I = PPP_I; % timed_Bnorm
    will
    see
    this and choose
    the
    ...
    % -------------------------------------------------------------------------
    function
    info = SAT_constant_threshold(PPP_test, L, cc, thetaArray, opts)
    savefile = getSaveFile(opts, 'FR');
    if exist(savefile, 'file'), load(savefile, 'info');
    return;
    end
    ...
    if ~exist(savefile, 'file')
        ...
        [~, specialistIDArray] = min(
            abs(bsxfun( @ minus, log(opts.sp.trainPPPArray(:)), log(
            opts.sp.PPPArray(:)'))), [], 1);

        for s_I = 1:max(specialistIDArray)
        ppp_IArray = (specialistIDArray == s_I);
        testinfo.IT.acc(ppp_IArray) = en
        {s_I}.IT.acc(ppp_IArray);
        testinfo.IT.acc_persample(:, ppp_IArray) = en
        {s_I}.IT.acc_persample(:, ppp_IArray);
        testinfo.IT.maxlogP_persample(:, ppp_IArray) = en
        {s_I}.IT.maxlogP_persample(:, ppp_IArray);
        testinfo.IT.logP_persample(:,:, ppp_IArray) = en
        {s_I}.IT.logP_persample(:,:, ppp_IArray);
        end

        info = testinfo.IT;
        save(savefile, 'info');
        else
        t = load(savefile, 'info');
        testinfo.IT = t.info;
        opts.sp.trainPPPArray = opts.sp.PPPArray;
        opts.sp.PPPArray = opts.sp.testPPPArray;
        end
    ...
    min((logP - log(1 - exp(logP))) / log(10), 20);
    thCandidateArray = F([log(logspace(log10(0.09), log10(1), 500))]);
    thCandidateArray(end) = 3;

    [~, info.mRT, info.ER, info.rt, info.isCorrect] = sequential_threshold_solver(
        opts.sp.PPPArray, ...
    ...
    getScotopicSimpleNNCifarBatch(x, y, opts.sp);
    case
    'mnist'
    fn =
    ...
    getScotopicSimpleNNMnistBatch(x, y, opts.sp);
    end
    case
    'dagnn'
    ...
    image
    data
    with mean image subtracted
    unpackPath = fullfile(opts.dataDir, 'cifar-10-batches-mat');
    files = [arrayfun( @ (n) sprintf('data_batch_%d.mat', n),
             1:5, 'UniformOutput', false) ...
    {'test_batch.mat'}];
    files = cellfun( @ (fn)
    fullfile(unpackPath, fn), files, 'UniformOutput', false);
    ...
    {fi});
    data
    {fi} = permute(reshape(fd.data
    ',32,32,3,[]),[2 1 3 4]) ;
    labels
    {fi} = fd.labels
    ' + 1; % Index from 1
    sets
    {fi} = repmat(file_set(fi), size(labels
    {fi}));
    end
    ...
    by
    image
    mean and std as suggested in `An
    Analysis
    of
    % Single - Layer
    ...
    organize
    data
    with different PPPs in a batch
    % 'batch'
    %
    return N * ppp_N
    images, list
    ...
    transformedMean = dcTransform.scaling * imdb.images.dataMean(:) + \
        dcTransform.offset;
    ...
    sp.mode
    case
    {'batch', 'batch_stochastic'}
    Nbins = length(sp.PPPArray);
    % obtain
    data
    with serialized pixels
    X = cumsum(reshape(spikify_pixel(X(:), sp), d, N, Nbins), 3); % [H * W * C N
                                                                     sp.Nbins]
    ...
    raw
    data
    X = sp.maxPPP * bsxfun( @ rdivide, bsxfun( @ plus, X, transformedMean /
        sp.maxPPP * sp.alpha), ...
    ...
    if nargout > 2, raw = reshape(X, [inputs_size N]);
    end
    X = sp.maxPPP * bsxfun( @ rdivide, bsxfun( @ plus, X, transformedMean /
        sp.maxPPP * sp.alpha), ...
    ...
    local_std = max(40, imdb.contrastNormalization.contrast(
        batch)) * dcTransform.scaling;
    ...
    url = sprintf('http://yann.lecun.com/exdb/mnist/%s.gz', files
    {i});
    fprintf('downloading %s\n', url);
    ...
    imdb.images.set = set;
    imdb.meta.sets = {'train', 'val', 'test'};
    imdb.meta.classes = arrayfun( @ (x)
    sprintf('%d', x), 0: 9, 'uniformoutput', false);
    ...
    data
    with serialized pixels
    raw = cumsum(reshape(spikify_pixel(X(:), sp), d, N, Nbins), 3); % [H * W * C N
                                                                       sp.Nbins]
    % divisive
    normalize
    X, then
    ...
    raw
    data
    X = sp.maxPPP * bsxfun( @ rdivide, bsxfun( @ plus, raw, transformedMean /
        sp.maxPPP * sp.alpha), ...
    reshape(sp.PPPArray + sp.alpha, [1, 1, Nbins]));
    case
    'paired'
    ...
    if nargout > 2, raw = reshape(raw, [input_shape numel(X) / d]); else raw = [];
    end
    % preprocessing:
    % 1.
    remove
