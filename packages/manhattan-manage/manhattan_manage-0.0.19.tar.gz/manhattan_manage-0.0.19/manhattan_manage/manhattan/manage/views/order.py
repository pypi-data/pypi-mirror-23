...
This chain can be configured using the initial state as follows:
...
# - I'm not overly convinced about the initial state configuration of chains, if
#   we're going to take this approach we should have a standard defaults link
#   for configuring defaults and review other views to determine if they are
#   better configured this way (e.g over config properties - they probably are
#   if for no other reason than it means the same chain can be copied and
...
def get_document_ids(state):
    ...
    This link adds `document_ids` to the state.
    """
...
def set_order(state):
    ...
    for document in state.documents:
        setattr(document, position_attr, state.ordered_ids.index(document._id))

    # Update the documents
    state.manage_config.frame_cls.update_many(state.documents, position_attr)
