...
def from_kpoints_schema(kpoints_config):
    if kpoints_config['mode'] == 'grid':
        if kpoints_config['centering'] == 'gamma':
            return Kpoints.gamma_automatic(kpoints_config['grid'], kpoints_config['offset'])
        elif kpoints_config['centering'] == 'monkhorst':
            return Kpoints.monkhorst_automatic(kpoints_config['grid'], kpoints_config['offset'])
        else:
            raise NotImplementedError('Kpoint centering %s not supported' % kpoints_config['centering'])
    elif kpoints_config['mode'] == 'line':
        # Copies implementation of automatic_linemode in python (kpoints)
        kpoints = []
        labels = []

        for path in kpoints_config['paths']:
            kpoints.append(path[0]['kpoint'])
            labels.append(path[0]['label'])
            for i in range(1, len(path) - 1):
                kpoints.append(path[i]['kpoint'])
                labels.append(path[i]['label'])
                kpoints.append(path[i]['kpoint'])
                labels.append(path[i]['label'])
            kpoints.append(path[-1]['kpoint'])
            labels.append(path[-1]['label'])

        kpoints = Kpoints(**{
            'comment': 'User Kpoint Line',
            'num_kpts': int(kpoints_config['divisions']),
            'style': Kpoints.supported_modes.Line_mode,
            'coord_type': 'Reciprocal',
            'kpts': kpoints,
            'kpts_weights': [1.0 for _ in range(len(kpoints))],
            'labels': labels
        })
        return kpoints
    else:
        raise NotImplementedError('Kpoint mode %s not supported yet' % kpoints_config['mode'])
...
def from_potcar_schema(potcar_config):
    functional = potcar_config['functional']
    symbols = [s['element'] + '_' + s['extension'] if s['extension'] else s['element'] for s in potcar_config['symbols']]
    return Potcar(symbols, functional)
...
class CalculationResourceItem(ResourceItem):
    ...
    @property
    def initial_structure(self):
        structure_rec = StructureResourceItem(self.data['initial_structure'])
        structure_rec.get()
    ...
    def input(self):
        if self.format == 'VASP':
            poscar = Poscar(self.initial_structure)
            incar = Incar.from_string(self.data['inputs']['incar'])
            kpoints = from_kpoints_schema(self.data['inputs']['kpoints'])
            potcar = from_potcar_schema(self.data['inputs']['potcar'])
    ...
    def upload_results(self, filename):
        if self.format == 'VASP':
            with open(filename, 'rb') as f:
                url = API_ROOT + '/calculations/vasp'
                files = {
    ...
    def __repr__(self):
        return f'<CalculationResourceItem: id: {self.id}, name: {self.name}, labels: {self.labels}>'
