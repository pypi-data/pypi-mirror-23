...
from osrframework.general import error, warning, success, info, title, emphasis
        ...
        for struct in fuzzingStructures:
            ...
            try:
                ...
                if nick in html:
                    ...
                    print(success("\tPossible usufy found!!!\n"))
            except:
...
def getPageWrapper(p, nick, rutaDescarga, avoidProcessing = True, avoidDownload = True, outQueue=None):
    ...
    try:
        ...
        print(error("ERROR: something happened when processing " + str(p) +". You may like to deactivate this wrapper if the error persist."))
        return []
...
def processNickList(nicks, platforms=None, rutaDescarga="./", avoidProcessing=True, avoidDownload=True, nThreads=12, maltego=False, verbosity=1, logFolder="./logs"):
    ...
    for nick in nicks:
        ...
        try:
            ...
            pool.terminate()
            print(warning("[!] The following platforms were not processed:"))
            pending = ""
            for p in platforms:
                processed = False
...
This is free software, and you are welcome to redistribute it under certain conditions. For additional info, visit <http://www.gnu.org/licenses/gpl-3.0.txt>."""
    ...
    if args.license:
        ...
        # showing the tags of the usufy platforms
            ...
            print(info("This is the list of platforms grouped by tag.")
            print()
            print(json.dumps(tags, indent=2, sort_keys=True))
            print()
            print(info("[Tip] Remember that you can always launch the platform using the -t option followed by any of the aforementioned."))
            print()
            return tags
        ...
        # Executing the corresponding process...
            ...
            if not args.maltego:
                ...
                print(bold(str(startTime) +"\tStarting search in " + str(len(listPlatforms)) + " platform(s)... Relax!")
                print()
                print(bold("\tPress <Ctrl + C> to stop...")
                print()
            ...
            if args.output_folder != None:
                ...
                try:
                    ...
                    print(error("Exception grabbed when processing the nicks: " + str(e)))
                    print(error(traceback.print_stack()))
            ...
            # Printing the results if requested
                ...
                print(info("A summary of the results obtained are shown in the following table:"))
                ...
                print(info("You can find all the information collected in the following files:"))
                for ext in args.extension:
                    # Showing the output files
                    print(bold(ileHeader + "." + ext))

                # Showing the execution time...
                print()
                endTime= dt.datetime.now()
                print(info(str(endTime) +"\tFinishing execution..."))
                print()
                print(info("Total time used:\t" + colorama.Style.BRIGHT + str(endTime-startTime)))
                print(info("Average seconds/query:\t" + colorama.Style.BRIGHT + str((endTime-startTime).total_seconds()/len(listPlatforms)) +" seconds"))
                print()
