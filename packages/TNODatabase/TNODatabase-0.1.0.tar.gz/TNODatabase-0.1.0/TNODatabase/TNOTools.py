...
def compute_chip(coord, expnum):
    """

    :param coord: Astropy SkyCoord object representing point
    :type coord: astropy.coordinates.SkyCoord
    :param expnum: exposure number of object
    :return:
    """

    slice = exposures[exposures.expnum == expnum]
    if len(slice) == 0:
        return 'None', -99

    return __compute_chip(coord.ra.radian, coord.dec.radian, slice.ra.iloc[0], slice.dec.iloc[0])


def __compute_chip(rockra, rockdec, expra, expdec):
    '''
    ...
    '''
    deltara = 180 / np.pi * ephem.degrees(
        rockra - expra).znorm  # compute difference in degrees (normalized between -180, +180)
    deltadec = 180 / np.pi * ephem.degrees(
        rockdec - expdec).znorm  # the 180/pi is because ephem.Angle objects are natively in radians
    ccdname = 'None'
    for k in ccdBounds:
        if ccdBounds[k][0] < deltara < ccdBounds[k][1] and ccdBounds[k][2] < deltadec < ccdBounds[k][3]:
            ccdname = k
    return ccdname, ccdNum[ccdname]


def __test():
    print compute_chip(SkyCoord(ra='2:43:29.55 hour', dec='-0:24:33.2 degree', frame='icrs'), 586619)
