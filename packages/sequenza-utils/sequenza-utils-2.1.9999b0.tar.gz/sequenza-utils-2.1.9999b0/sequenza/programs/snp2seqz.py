...
def snp2seqz_args(parser, argv):
    ...
    parser_in.add_argument(
        '-v', '--vcf', dest='vcf', default=None,
        help=('VCF input file, It needs'))
    parser_in.add_argument(
        '-gc', dest='gc', help='The GC-content wiggle file')
    ...
    parser_vcf.add_argument(
        ...
        help=('Column separated VCF tags in the format column '
              ...
              'Default "AD:AD"'))
    parser_vcf.add_argument(
        '--preset', dest='vcf_preset', type=str, choices=['mutect', 'mpileup'],
...
def snp2seqz(subparsers, module_name, argv, log):
    ...
    if args.vcf is None:
        ...
        if args.gc is None:
            raise Exception('The -gc option is required with vcf input')
