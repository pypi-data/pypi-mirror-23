   ...
   //Py_INCREF(ret);
    ...
    noend     = 0;
        ...
        if (! PyArg_ParseTupleAndKeywords(args, kwargs, "zziC|iii", kwlist,
            &in_pileup, &in_quality, &depth, &reference, &qlimit,
            &noend, &nostart)) {
            goto except;
        }
    #else
        if (! PyArg_ParseTupleAndKeywords(args, kwargs, "OOic|iii", kwlist,
            &in_pileup, &in_quality, &depth, &reference, &qlimit,
            &noend, &nostart)) {
            goto except;
        }
    ...
    pileup  = py2str(in_pileup);
    quality = py2str(in_quality);
    //Py_DECREF(in_pileup);
    //Py_DECREF(in_quality);
    //int n = 0;
    //int q = 0;
