...
def vcf_parse(vcf_file, sample_order='n/t', field='FORMAT', depth=['DP', 'DP'],
    ...
    for vcf_item in vcf_read:
        if 'headerlist' in vcf_item:
            ...
            normal_i = split_format(format_i, normal_i, depth[0], alleles[0])
            tumor_i = split_format(format_i, tumor_i, depth[1], alleles[1])
            yield((chromosome, position, position + 1),
                  (line_i[3], line_i[4], normal_i, tumor_i))


def split_format(format_str, format_content, depth, alleles,
                 split_char1=':', split_char2=','):
