...
from dashbase.tools import resultview
...
def doQuery(conn, req, headers):
    conn.request("POST", "/v1/query", json.dumps(req), headers)
...
def main():
    ...
    dashbase_req["table"] = []
    if table != None:
        dashbase_req["table"].append(table)
    ...
    conn = httplib.HTTPConnection(host, port, timeout=600)

    if args.stream:
        try:
            while True:
                try:
                    result = doQuery(conn, dashbase_req, headers)
        ...
        print dashbase_req
        result = doQuery(conn, dashbase_req)
