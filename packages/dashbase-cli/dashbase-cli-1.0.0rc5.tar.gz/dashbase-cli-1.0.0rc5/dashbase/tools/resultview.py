...
def drawNumericAggregation(name, numeric, aggr_req):
    ...
    if numeric != None:
        data.append([numeric["numDocs"], numeric["value"]])
    return table.table
...
def printTsAggregation(name, histograms):
    ...
    if histograms != None:
        ...
        for bucket in buckets:
            ...
            data_array = [start, end, str(bucket["count"])]
...
def drawTopnAggregation(name, topn):
    ...
    if topn != None:
        ...
        facets = topn["facets"]
        for facet in facets:
            data.append([facet["value"], facet["count"]])
    else:
        table = AsciiTable(data, name)
...
def printTimeRange(request):
    t1 = None
    t2 = None
    if request["timeRangeFilter"] != None:
