...
class Depot(object):
    ...
    def get(self, entry):
        ...
        max_chunks = (entry.depot_object.size + chunk_size - 1) / chunk_size
        stream = entry.depot_object.download_object_as_stream(chunk_size=chunk_size)
        cur_chunk = 0
        with progressbar.ProgressBar(max_value=max_chunks) as bar:
            for data in stream:
                tmpfile.write(data)
                bar.update(cur_chunk)
                cur_chunk += 1
        tmpfile.close()
