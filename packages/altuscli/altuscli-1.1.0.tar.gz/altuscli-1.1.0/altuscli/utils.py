...
class ArgumentGenerator(object):
    ...
    def generate_skeleton(self, shape, include_hidden):
        if shape.type_name == OBJECT_TYPE:
            return self._generate_type_object(shape, include_hidden)
        elif shape.type_name == LIST_TYPE:
            return self._generate_type_array(shape, include_hidden)
    ...
    def _generate_type_object(self, shape, include_hidden):
        skeleton = OrderedDict()
        for member_name, member_shape in shape.members.items():
            if member_shape.is_hidden and not include_hidden:
                continue
            skeleton[member_name] = self.generate_skeleton(member_shape,
                                                           include_hidden)
        return skeleton

    def _generate_type_array(self, shape, include_hidden):
        # For list elements we've arbitrarily decided to return the first
        # element for the skeleton list.
        return [
            self.generate_skeleton(shape.member, include_hidden),
        ]
