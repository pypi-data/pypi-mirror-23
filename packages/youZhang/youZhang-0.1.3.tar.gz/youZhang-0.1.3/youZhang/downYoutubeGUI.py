...
def downVideoGUI():
    ...
    def choose():
        filename = tkFileDialog.askopenfilename(initialdir='/', filetypes=[('mp4', '*.mp4'), ('pdf', '*.pdf')])
    ...
    labe_txt = Label(root, text='输入链接地址：')
    ...
    link_contend.set('')
    choose_button = Button(root, text='选择MP4视频文件', command=choose, width=15)
    choose_button.grid(row=1, column=0)
    M3U8_choose_button = Button(root, text='V站下载', command=vidolM3U8ZD, width=15)
    M3U8_choose_button.grid(row=1, column=1)
    playM3U8_choose_button = Button(root, text='V站播放', command=playVidolM3U8ZD,
                                    width=15)
    playM3U8_choose_button.grid(row=1, column=2)
    pic_watermark_gif_button = Button(root, text='图片水印', command=picMark, width=15)
    pic_watermark_gif_button.grid(row=2, column=0)
    gif_button = Button(root, text='提取GIF图片', command=generateGIF, width=15)
    gif_button.grid(row=2, column=1)
    split8Segments_gif_button = Button(root, text='分割8视频', command=split_8_video, width=15)
    split8Segments_gif_button.grid(row=2, column=2)
    ...
    start_button = Button(root, text='清空', command=emptyIt, width=15)
    start_button.grid(row=3, column=2)
    start_button = Button(root, text='下载pdf', command=download_pdf_for_link, width=15)
    start_button.grid(row=4, column=0)
    start_button = Button(root, text='重命名子文件', command=rename_it, width=15)
    start_button.grid(row=4, column=1)

    info_entry = Text(root, width=25, height=4, )
    info_entry.grid(row=5, column=0, columnspan=2, rowspan=4)
...
''', 'wechat.gif')
    ...
    wechat_label.grid(row=4, column=2, columnspan=2, rowspan=5)
