...
def downVideoGUI():
    ...
    def generateGIF():
        ...
        ffmpeg_cmd = 'ffmpeg -ss 00:01:11 -t 00:00:06 -i ' + new_video_path + ' -r 1 -s 480*270 -f gif ' + timestamp + '.gif'
...
text='From AcFun@daleloogn': fontcolor=white: fontsize=60: box=1: boxcolor=black@0.5: \
boxborderw=5: x=100: y=100" -codec:a copy split0.mp4'''
    ...
    def picMark():
        urllib.urlretrieve("http://i2.kiimg.com/1949/9cc9111f14126a4f.png", "mark.png")
        start = time.time()
        timestamp = nowTimeStr()
    ...
    def split_video():
        ...
        os.system(cmd_line)
        cmd_line = "ffmpeg -ss 00:12:00  -i " + new_video_path + " -strict -2 -vcodec copy -acodec copy -t 00:12:00 -y " + "split02.mp4"
        os.system(cmd_line)
        cmd_line = "ffmpeg -ss 00:24:00  -i " + new_video_path + " -strict -2 -vcodec copy -acodec copy -t 00:12:00 -y " + "split03.mp4"
        os.system(cmd_line)
        cmd_line = "ffmpeg -ss 00:36:00  -i " + new_video_path + " -strict -2 -vcodec copy -acodec copy -y " + "split04.mp4"
        os.system(cmd_line)
        return 0

    def split_8_video():
        timestamp = nowTimeStr()
        video_path = split8Segments_link_contend_gif.get()
        video_format = video_path.split('.')[-1]
        new_video_path = timestamp + '.' + video_format
        print new_video_path
        os.rename(video_path, new_video_path)
        ...
        cmd_line = "ffmpeg -ss 00:24:00  -i " + new_video_path + " -strict -2 -vcodec copy -acodec copy -t 00:12:00 -y " + "split03.mp4"
        os.system(cmd_line)
        cmd_line = "ffmpeg -ss 00:36:00  -i " + new_video_path + " -strict -2 -vcodec copy -acodec copy -t 00:12:00 -y " + "split04.mp4"
        os.system(cmd_line)
        cmd_line = "ffmpeg -ss 00:48:00  -i " + new_video_path + " -strict -2 -vcodec copy -acodec copy -t 00:12:00 -y " + "split05.mp4"
        os.system(cmd_line)
        cmd_line = "ffmpeg -ss 01:00:00  -i " + new_video_path + " -strict -2 -vcodec copy -acodec copy -t 00:12:00 -y " + "split06.mp4"
        os.system(cmd_line)
        cmd_line = "ffmpeg -ss 01:12:00  -i " + new_video_path + " -strict -2 -vcodec copy -acodec copy -t 00:14:00 -y " + "split07.mp4"
        os.system(cmd_line)
        cmd_line = "ffmpeg -ss 01:26:00  -i " + new_video_path + " -strict -2 -vcodec copy -acodec copy -y " + "split08.mp4"
        os.system(cmd_line)
        return 0
    ...
    def split_8_seg_water_choose():
        filename = tkFileDialog.askopenfilename(initialdir='/home/zhangxulong/video', filetypes=[('mp4', '*.mp4')])
        split8Segments_link_contend_gif.set(filename)
        info_entry.insert(1.0, '选择文件：\n', 'a')
        info_entry.insert(1.0, filename, 'a')
        return 0
    ...
    splitSegments_gif_button = Button(root, text='分割4视频', command=split_video, width=15)
    splitSegments_gif_button.grid(row=5, column=4)

    split8Segments_labe_txt_gif = Label(root, text='选择视频文件：')
    split8Segments_labe_txt_gif.grid(row=6, column=0)
    split8Segments_entry_link_gif = Entry(root, width=40)
    split8Segments_entry_link_gif.grid(row=6, column=1, columnspan=2)
    split8Segments_link_contend_gif = StringVar()
    split8Segments_entry_link_gif.config(textvariable=split8Segments_link_contend_gif)
    split8Segments_link_contend_gif.set('')
    split8Segments_choose_button = Button(root, text='选择视频', command=split_8_seg_water_choose, width=15)
    split8Segments_choose_button.grid(row=6, column=3)
    split8Segments_gif_button = Button(root, text='分割8视频', command=split_8_video, width=15)
    split8Segments_gif_button.grid(row=6, column=4)

    info_entry = Text(root, width=50, height=4, )
    info_entry.grid(row=7, column=0, columnspan=5, rowspan=2)
