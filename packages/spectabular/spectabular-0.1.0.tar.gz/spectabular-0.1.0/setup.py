...
path_version_file = "spectabular/version.py"
if not os.path.exists(path_version_file):
	system("version=`git describe --tags --long`; spectabular/setversion.py ${version}")

version = imp.load_source('version', path_version_file)

#system("version=`git describe --tags --long`; spectabular/spectabular/setversion.py ${version}")
...
#full_name = spectabular.__full_name__
cmdclass = {}

if has_py2app and sys.argv[1] == "py2app":
	import spectabular.ui
	class my_py2app(py2app.build_app.py2app):
		...
		def run(self):
			...
			if 0:
				...
				targetdir = 'dist/spectabular.app/Contents/Resources/lib/'
				...
				libpng15.15.dylib
				""".strip().splitlines()]
				targetdir = 'dist/spectabular.app/Contents/Resources/'
				for filename in libs:
					#path = os.path.join(libpath, filename)
					cmd = "cp %s %s" % (path, targetdir)
					print(cmd)
			...
			os.system("cp data/helmi-dezeeuw-2000-10p.hdf5 dist/spectabular.app/Contents/Resources/")
			os.system("cd dist")
			zipname = "%s.zip" % spectabular.__build_name__
			os.system("cd dist;rm %s" % zipname)
			os.system("cd dist;zip -q -r %s %s.app" % (zipname, spectabular.__program_name__))
...
#print DATA_FILES
OPTIONS = {'argv_emulation': False, 'excludes':[], 'resources':['spectabular/ui/icons'],
           'matplotlib_backends':'-',
           'no_chdir':True,
		   'includes': ['h5py',
                 'h5py.defs',
           ...
           'iconfile': 'spectabular/ui/icons/spectabular.icns'
...
extensions = [
	Extension("spectabular.spectabularfast", ["src/spectabular/spectabularfast.cpp"],
...
#print "ver#sion", spectabular.__release__
setup(
	entry_points={
		'console_scripts': ['spectabular = spectabular.__main__:main'],
		'gui_scripts': ['spectabulargui = spectabular.__main__:main'] # sometimes in osx, you need to run with this
  	},
	app=["bin/spectabular"],
	name="spectabular", #spectabular.__program_name__,
    ...
    includes=["spectabular", "md5", "astropy", "aplus", "six"],
    packages=["spectabular", "spectabular.test", "spectabular.misc", "spectabular.notebook", "spectabular.file", "spectabular.ext"],
    install_requires=reqs,
    #entry_points={ 'console_scripts': [ 'spectabular=spectabular.ui.main:main']  },
    #scripts=[os.path.join("bin", "spectabular")],
    ext_modules=extensions,
#    package_data={'spectabular': ['ui/icons/*.png', 'ui/icons/*.icns', 'ext/*.js']},
    package_dir={'spectabular':'spectabular'},
    cmdclass=cmdclass,
    description="Spectabular is a graphical tool to visualize and explore large tabular datasets.",
    long_description=open("README.rst").read(),
    url="https://www.astro.rug.nl/~breddels/spectabular",
	classifiers=[
