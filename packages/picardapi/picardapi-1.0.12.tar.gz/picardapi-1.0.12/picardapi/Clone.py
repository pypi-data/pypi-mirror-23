...
class Clone(object):
    ...
    def doctype(self, name, source_stack, target_stack):
        """
        ...
        #1 Build the new doctype
        rv = picard.query("/custom/doctype", method="post", params=doctype)
    ...
    def app(self, name, source_stack, target_stack, overwrite=False):
        ...
        a = 1
    #
    # def get_files(self, picard, image_doc):
    #     files = picard.query("/custom/document", {"id": image_doc['id'], "files": True}).json['files']['Contents']
    #     for file in _.pluck(files, 'Key'):
    #         a = picard.query("/files/file", {"id": page['_id'], "filename": os.path.basename(file)})
    #         try:
    #             b = s3cli.put_object(Body=requests.get(a.json['url'], stream=True).content,
    #                                  Bucket=app.customfs,
    #                                  Key=default_app['_source']['pages']+"/"+image_doc['id']+"/"+os.path.basename(file))
    #         except Exception, e:
    #             pass
    #
    ...
    def get_app_doctype(self, picard, app):
        ...
        params = {
            ...
            "size": 1000,
            "sort": ["_modified:asc"],
            "should": [{"term": {"tags.text": app['_source']['name']}}]
        }
