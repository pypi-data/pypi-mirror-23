    ...
    def write_cqp(self, fname, f_out=None):
        ...
        for t in exml.read_trees_exml(fname):
            if hasattr(t, 'sent_no'):
                print >>f_out, "<s id=%s>" % (t.sent_no,)
            elif hasattr(t, 'xml_id'):
                print >>f_out, "<s id=%s>" % (t.xml_id,)
            else:
                print >>f_out, "<s>"
            ...
            if self.want_deprels:
                for n in t.terminals:
                    ...
                    if not hasattr(n, 'lemma') or n.lemma is None:
                        ...
                        lemma = n.lemma
                    print >>f_out, "%s\t%s\t%s\t%s\t%s\t%s" % (
                        n.word, n.cat, lemma, n.morph, lbl, attach)
            else:
                for n in t.terminals:
                    print >>f_out, "%s\t%s\t%s\t%s" % (
                        n.word, n.cat, n.lemma, n.morph)
            print >>f_out, "</s>"
        return count
    ...
    if os.path.isdir(args[0]):
        ...
        if os.path.exists(offsets_fname):
            ...
            for l in file(offsets_fname):
                ...
                print >>f_out, "<doc id=%s>"%(fname0,)
                count_file = app.write_cqp(f_path, f_out)
                print >>f_out, "</doc>"
                posn_offset = int(line[1])
                if posn_offset != count_total:
                    print >>sys.stderr, "ERROR: %s at position %d, should be %d"%(
                        fname, count_total, posn_offset)
                    print >>sys.stderr, "before: %s"%(old_fname,)
                count_total = posn_offset + count_file
                old_fname = fname
            if all_files:
                #TODO: append new files to offsets.txt?
                for fname in sorted(all_files):
                    print >>sys.stderr, "ERROR: %s is missing in offsets.txt"%(
                        fname,)
        else:
