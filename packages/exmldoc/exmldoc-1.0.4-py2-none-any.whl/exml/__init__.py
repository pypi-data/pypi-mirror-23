...
from exml import tree
from exml.topsort import topsort
from exml.alphabet import PythonAlphabet
...
ConnectiveRel = namedtuple('ConnectiveRel', ['rel1', 'rel2'])
...
__doc__ = '''
pytree.exml contains code related to the ExportXMLv2 format, including
...
class TextAttribute:
    ...
    def unmap_attr(self, val, doc, encoding):
        return unmunge_xml(val, doc, encoding)

    def get_updown(self, obj, doc, result):
        pass

    def javaClass(self, doc=None):
        return 'String'
...
class RefAttribute:
    ...
    def javaClass(self, doc=None):
        if self.restrict_target:
            if self.restrict_target and len(self.restrict_target) == 1:
                tp = self.restrict_target[0]
                if tp == 'word':
                    return 'TuebaTerminal'
                else:
                    return 'Tueba%sMarkable' % (tp[0].upper() + tp[1:])
            elif doc is not None:
                iface = doc.common_interface(self.restrict_target)
                if iface is not None:
                    return 'Tueba%sInterface'%(iface[0].upper() + iface[1:])
        return 'NamedObject'
...
class MarkableSchema:
    ...
    def create_from_json(self, obj, doc):
        ...
        try:
            ...
            print >>sys.stderr, "Cannot instantiate %s" % (self.cls)
            raise
        m.xml_id = obj['_id']
        m.span = obj['span']
    ...
    def create_from_xml(self, elem, doc, encoding=None):
        ...
        for att in self.init_attrs:
            v = None
            try:
                v = unmunge_xml(elem.attrib[att.prop_name], doc, encoding)
            except KeyError:
                pass
            args.append(v)
        try:
...
def open_tag(f, name, items, indent=0, encoding=None):
    ...
    for k, v in items:
        ...
        if isinstance(v, unicode) and encoding is not None:
            f.write(' %s="%s"'%(k, escape(v.encode(encoding), {'"':"&quot;"})))
        else:
...
def unmunge_xml(obj, doc, encoding):
...
class TerminalSchema(object):
    ...
    def create_from_xml(self, node, doc, encoding=None):
        obj = self.cls(node.attrib['pos'],
                       unmunge_xml(node.attrib['form'], doc, encoding))
...
class Document:
    ...
    def get_interfaces(self, name):
        ...
        result = []
        for k, v in self.interface_classes.iteritems():
            print >>sys.stderr, name, k, v, (name in v)
            if name in v:
                result.append(k)
    ...
    def register_object(self, obj, schema=None):
        if schema is None:
            ...
            mlevel = schema
        if mlevel is None:
            print >>sys.stderr, self.schema_by_class
        ...
        if topic_re.match(arg):
            ...
            if len(endpoints) == 1:
                ...
                if (start_edu, end_edu) in self.edu_ranges:
                    ...
                    x.xml_id = 'edus%s_%s-%s' % (self.xml_id.split('_')
                                                 [-1], start_edu.replace('.', '_'), end_edu.replace('.', '_'))
...
class ConnectiveEdges:
    ...
    def __init__(self, name):
        ...
        for att in [self.attributes[1], self.attributes[2]]:
            ...
            att.add_item('speech_act',
                         'circumstances for a speech act (causal)')
    ...
    def get_edges(self, obj, doc):
        ...
        if info is not None:
            konn = getattr(info, 'word',
                           obj.lemma.decode('ISO-8859-15')).encode('ISO-8859-15')
    ...
    def set_edges(self, obj, vals, doc):
        if vals:
            val = vals[0]
            info = ConnectiveRel(val[1], val[2])
            obj.konn_rel = info
...
def make_syntax_doc(want_ne=True, want_dc=False, want_wsd=False,
                    want_deps=False):
    '''
    ...
    if want_dc:
        t_schema.edges.append(ConnectiveEdges('connective'))
    if want_deps:
...
class XMLCorpusReader(object):
    ...
    def addNext(self):
        print >>sys.stderr, "addNext called"
                ...
                if elem.tag == 'word':
                    ...
                    try:
                        ...
                        if edge_schema is not None:
                            print >>sys.stderr, "edge: %s"%(elem.tag)
                        else:
                            print "No schema:", elem.tag, [s.name for s in doc.schemas]
                    else:
                        obj = schema.create_from_xml(elem, doc, encoding)
                        obj.span = [cur_pos, None]
...
def read_trees_exml(fname):
    ...
    while True:
        try:
            ...
            print >>sys.stderr, "last_stop=%d new_stop=%d"%(
                last_stop, new_stop)
            if new_stop != last_stop:
                trees = doc.get_objects_by_class(
                    tree.Tree, last_stop, new_stop)
                print >>sys.stderr, "%d sentences in %d tokens"%(
                    len(trees), new_stop-last_stop)
                for t in trees:
                    #TODO fill terminals
                    t.terminals = doc.w_objs[t.span[0]:t.span[-1]]
                    for i, n in enumerate(t.terminals):
...
def write_corpus_xml(doc, reader, f_out, encoding="ISO-8859-15"):
    ...
    '''
    print >>f_out, '<?xml version="1.0" encoding="%s"?>'%(encoding,)
    print >>f_out, '<exml-doc>'
    doc.describe_schema(f_out)
    # do the actual conversion
    print >>f_out, '<body serialization="inline">'
    last_stop = len(doc.words)
    while True:
        try:
            new_stop = reader.addNext()
...
def write_corpus_json(doc, reader, f_out):
    ...
    while True:
        try:
            new_stop = reader.addNext()
            if (new_stop != last_stop):
                print >>f_out, json.dumps(doc.json_chunk(last_stop, new_stop))
                doc.clear_markables(last_stop, new_stop)
                last_stop = new_stop
        except StopIteration:
            break
    ...
    for chld in elem.getchildren():
        if chld.tag == 'text-attr':
            ...
            try:
                ...
                print >>sys.stderr, "Undeclared text attribute: %s.%s"%(
            ...
            try:
                ...
                print >>sys.stderr, "Undeclared enum attribute: %s.%s"%(
                    schema.name, tag_name,)
            ...
            try:
                ...
                print >>sys.stderr, "Undeclared node reference: %s.%s"%(
                    schema.name, tag_name,)
                att = RefAttribute(tag_name,
                                   prop_name='_auto_%s'%(tag_name,))
                schema.add_attribute(att)
        else:
            print >>sys.stderr, 'Unkown tag in node schema: %s'%(chld.tag,)
    ...
    for chld in elem.getchildren():
        if chld.tag == 'tnode':
            ...
            try:
                ...
                print >>sys.stderr, "Undeclared markable level: %s"%(node_name,)
            ...
            for node_name in node_names:
                ...
                try:
                    ...
                    print >>sys.stderr, "Undeclared edge schema: %s.%s"%(
                        node_name, edge_name)
