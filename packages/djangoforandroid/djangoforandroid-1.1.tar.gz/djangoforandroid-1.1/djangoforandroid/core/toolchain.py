...
def build_dist_from_args(ctx, dist, args):
    ...
    info('Dist can be found at (for now) {}'.format(os.path.join(ctx.dist_dir, ctx.dist_name)))
...
class ToolchainCL(object):
    ...
    def __init__(self, args):
        ...
        self.ctx.setup_dirs(args.storage_dir)
    ...
    def hook(self, name):
        ...
        if not hasattr(self, "hook_module"):
            ...
            self.hook_module = imp.load_source("djangoforandroid.core.hook", self.args.hook)
    ...
    #def _read_configuration(self, file='.p4a'):
        ## search for a .p4a configuration file in the current directory
        #if not exists(file):
            #return
        #info("Reading {} configuration".format(file))
    ...
    #def clean_bootstrap_builds(self, args):
        ...
        #if exists(os.path.join((self.ctx.build_dir, 'bootstrap_builds')):
            #shutil.rmtree(os.path.join((self.ctx.build_dir, 'bootstrap_builds'))
        ## for bs in Bootstrap.list_bootstraps():
        ##     bs = Bootstrap.get_bootstrap(bs, self.ctx)
    ...
    #def clean_builds(self, args):
        ...
        #libs_dir = os.path.join((self.ctx.build_dir, 'libs_collections')
        #if exists(libs_dir):
    ...
    def apk(self, release=False):
        ...
        build = imp.load_source('build', os.path.join(dist.dist_dir, 'build.py'))
        with current_directory(dist.dist_dir):
            ...
            output = shprint(ant, self.args.build_mode, _tail=20, _critical=True, _env=env)
        ...
        if not apk_file:
            ...
            apks = glob.glob(os.path.join(dist.dist_dir, 'bin', '*-*-{}.apk'.format(suffix)))
            if len(apks) == 0:
    ...
    #def sdk_tools(self, args):
        ...
        #ctx.prepare_build_environment(user_sdk_dir=self.sdk_dir,
                                      ...
                                      #user_android_api=self.android_api,
                                      #user_ndk_ver=self.ndk_version)
        #android = sh.Command(os.path.join((ctx.sdk_dir, 'tools', args.tool))
        #output = android(
    ...
    #def _adb(self, commands):
        ...
        #ctx.prepare_build_environment(user_sdk_dir=self.sdk_dir,
            ...
            #adb = sh.Command(os.path.join((ctx.sdk_dir, 'platform-tools', 'adb.exe'))
        #else:
            #adb = sh.Command(os.path.join((ctx.sdk_dir, 'platform-tools', 'adb'))
    ...
    #def build_status(self, args):
        ...
        #bootstrap_dir = os.path.join((self.ctx.build_dir, 'bootstrap_builds')
        ...
        #print('{Style.BRIGHT}Recipes that are probably already built:'
              #'{Style.RESET_ALL}'.format(Style=Out_Style))
        #other_builds_dir = os.path.join((self.ctx.build_dir, 'other_builds')
