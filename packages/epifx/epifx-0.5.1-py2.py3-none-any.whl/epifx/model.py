...
class SEIR(pypfilt.Model):
    ...
    def update(self, params, step_date, dt, is_fs, prev, curr):
        ...
        t0 = params['time'].to_scalar(params['time'].start)
...
class SEEIIR(pypfilt.Model):
    ...
    __info = [
        ("S", False, 0, 1), ("E0", False, 0, 1), ("E1", False, 0, 1),
        ("I0", False, 0, 1), ("I1", False, 0, 1), ("R", False, 0, 1),
        ("R0", True, 1, 2), ("sigma", True, 1/3, 2),
        ("gamma", True, 1/3, 1), ("eta", True, 1, 2),
        ("alpha", True, -0.2, 0.2),
        ("t0", False, 0, 100)]
    ...
    def sample_params(self, params, rnd, step_date, dt, curr, mask_init):
        ...
        forcing = 'forcing' in params['epifx']
        curr[mask_init, 6] = prior['R0'](rnd, size=rnd_size)
        curr[mask_init, 7] = prior['sigma'](rnd, size=rnd_size)
        curr[mask_init, 8] = prior['gamma'](rnd, size=rnd_size)
        curr[mask_init, 9] = prior['eta'](rnd, size=rnd_size)
        if forcing:
            curr[mask_init, 10] = prior['alpha'](rnd, size=rnd_size)
        else:
            curr[mask_init, 10] = 0
    ...
    def update(self, params, step_date, dt, is_fs, prev, curr):
        ...
        if np.any(mask_init):
            ...
            # Enforce invariants on model parameters.
            # Note: a memory leak arose when using the ``out`` parameter.
            curr[mask_init, 6:] = np.clip(curr[mask_init, 6:],
                                          p_min[6:], p_max[6:])
        ...
        if np.any(mask_step):
            ...
            # Determine the force of infection.
            beta = curr[mask_step, 6] * curr[mask_step, 8]
            if forcing:
                # Modulate the force of infection with temporal forcing.
                force = params['epifx']['forcing'](step_date)
                force *= curr[mask_step, 10]
                # Ensure the force of infection is non-negative (can be zero).
                beta *= np.maximum(1.0 + force, 0)

            s_to_e0 = dt * (beta * np.sum(curr[mask_step, 3:5], axis=-1) *
                            curr[mask_step, 0] ** curr[mask_step, 9])
            e0_to_e1 = dt * (2 * curr[mask_step, 7] * curr[mask_step, 1])
            e1_to_i0 = dt * (2 * curr[mask_step, 7] * curr[mask_step, 2])
            i0_to_i1 = dt * (2 * curr[mask_step, 8] * curr[mask_step, 3])
            i1_to_r = dt * (2 * curr[mask_step, 8] * curr[mask_step, 4])
            # Account for stochastic behaviour, if appropriate.
            if params['epifx']['stoch']:
                # Define the relative scales of the noise terms.
                scale = np.empty(shape=10)
                scale[:5] = params['prior']['noise_flow'](rnd)
                scale[5:] = params['prior']['noise_param'](rnd)
                n_size = rnd_size + (10,)
                noise = scale[np.newaxis, :] * dt * rnd.normal(size=n_size)
                # Scale the noise parameters in proportion to the flow rates
                # in to and out of each model compartment (i.e., S, E, I, R),
                # according to the scaling law of Gaussian fluctuations.
                # For more details see doi:10.1016/j.mbs.2012.05.010
                noise[..., 0] *= np.sqrt(s_to_e0 / dt)
                noise[..., 1] *= np.sqrt(e0_to_e1 / dt)
                noise[..., 2] *= np.sqrt(e1_to_i0 / dt)
                noise[..., 3] *= np.sqrt(i0_to_i1 / dt)
                noise[..., 4] *= np.sqrt(i1_to_r / dt)
                # Add noise to the inter-compartment flows.
                s_to_e0 += noise[..., 0]
                e0_to_e1 += noise[..., 1]
                e1_to_i0 += noise[..., 2]
                ...
                i1_to_r += noise[..., 4]
                # Add noise to the model parameters *except* t0.
                curr[mask_step, 6:11] += noise[..., 5:]
                # Enforce invariants on model parameters.
                curr[mask_step, 6:11] = np.clip(curr[mask_step, 6:11],
                                                p_min[6:], p_max[6:11])
                if not forcing:
                    # Do not allow sigma to vary if there is no forcing.
                    curr[mask_step, 10] = 0
            # Update compartment sizes.
            curr[mask_step, 0] -= s_to_e0
            curr[mask_step, 1] += s_to_e0 - e0_to_e1
            curr[mask_step, 2] += e0_to_e1 - e1_to_i0
            curr[mask_step, 3] += e1_to_i0 - i0_to_i1
            curr[mask_step, 4] += i0_to_i1 - i1_to_r
            # Enforce invariants on S, E, and I compartments.
            curr[mask_step, :5] = np.clip(curr[mask_step, :5], 0, 1)
            mask_invalid = np.sum(curr[mask_step, :5], axis=-1) > 1
            if np.any(mask_invalid):
                # Ensure we're updating the original matrix, not a copy.
                mask_sub = np.logical_and(mask_step,
                                          np.sum(curr[:, :5], axis=-1) > 1)
                sub = (np.sum(curr[mask_sub, :5], axis=-1) - 1.0)[:, None]
                curr[mask_sub, :5] = (1 - sub) * curr[mask_sub, :5]
            # Calculate the size of the R compartment and clip appropriately.
            curr[mask_step, 5] = np.clip(
                1.0 - np.sum(curr[mask_step, :5], axis=-1), 0.0, 1.0)
    ...
    def pr_inf(self, prev, curr):
        ...
        # Count the number of susceptible / exposed individuals at both ends
        # of the simulation period.
        prev_amt = np.sum(prev[..., 0:3], axis=-1)
        curr_amt = np.sum(curr[..., 0:3], axis=-1)
        # Avoid returning very small negative values (e.g., -1e-10).
        return np.maximum(prev_amt - curr_amt, 0)
