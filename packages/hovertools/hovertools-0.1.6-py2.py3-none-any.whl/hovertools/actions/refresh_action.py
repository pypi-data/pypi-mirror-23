...
def provision_system(prov_doc, containerid):
...
def parse_volumes(volumes):
    for k, v in volumes.items():
        resolved_path = os.path.abspath(os.path.expandvars(k))
        logger.info("Resolved {0} to {1}".format(k, resolved_path))
        volumes[resolved_path] = volumes.pop(k)
    return volumes
...
def do_refresh(ctx, name):
    ...
    volumes = None
    if 'volumes' in yaml_doc:
        volumes = yaml_doc['volumes']
    ...
    if 'provisioning' in yaml_doc:
        provision_system(yaml_doc['provisioning'], container_id)
