...
class Cursor(common.DBAPICursor):
    ...
    def __init__(self, host, port='9000', username=None, password=None, catalog='hive', cluster=None, schema='default',
    ...
    def execute(self, operation, parameters=None):
        ...
        headers = {
            ...
            'X-Presto-Cluster': self._cluster
        }
        ...
        if sql == "login":
            ...
            self._protocol,
            '{}:{}'.format(self._host, self._port), '/login', None, None, None))
        else:
            url = urlparse.urlunparse((
            self._protocol,
        ...
        response = self._session.post(url, data=sql.encode('utf-8'), headers=headers)
    ...
    def cancel(self):
        ...
        response = self._session.delete(self._nextUri)
        if response.status_code != self._session.codes.no_content:
    ...
    def poll(self):
        ...
        if self._nextUri is None:
            assert self._state == self._STATE_FINISHED, "Should be finished if nextUri is None"
            return None
        response = self._session.get(self._nextUri)
        self._process_response(response)
        return response.json()

    def _fetch_more(self):
        """Fetch the next URI and update state"""
        self._process_response(self._session.get(self._nextUri))
