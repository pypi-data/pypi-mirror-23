...
import logging_helper; logging = logging_helper.setup_logging(level=u"INFO")
...
class InterceptFrame(BaseFrame):
    ...
    def _scenarios_config(self):
        ...
        for intercept_server in self.intercept_server.http_server_instances.values():
            try:
                intercept_server.RequestHandlerClass.PROXY_CONFIG.reload_config()
            except AttributeError as e:
                # TODO: Is AttributeError: class HTTPRequestHandler has no attribute 'PROXY_CONFIG' acceptable?
                logging.exception(e)
    ...
    def _active_scenario_config(self):
        ...
        # TODO: This does not make sense as it is a dict by default!
        if self.intercept_server.http_server_instances is not None:
            self.intercept_server.reload_config()
            # self.intercept_server \
            #     .http_server \
            #     .RequestHandlerClass \
            #     .PROXY_CONFIG \
            #     .reload_config()
