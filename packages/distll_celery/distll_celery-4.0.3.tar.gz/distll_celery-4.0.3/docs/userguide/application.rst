...
including the name of the app class (    >>> from celery import Celery
    >>> app = Celery()
    >>> app
    <Celery __main__:0x100469fd0>
...
.. topic:: Evolving the API
    ...
    .. code-block:: pycon
            ...
            >>> from distll_celery.execute import apply_async

            >>> apply_async(hello, ('world!',))

        or you could also create a         def hello(to):
                return 'hello {0}'.format(to)
...
.. code-block:: pycon
    ...
    >>> from distll_celery import Celery, Task
