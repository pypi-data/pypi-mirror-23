...
from distll_celery import _find_option_with_arg
from distll_celery import platforms
from distll_celery.exceptions import SecurityError
from distll_celery.five import WhateverIO
from distll_celery.platforms import (
    get_fdmax,
    ignore_errno,
    check_privileges,
    set_process_title,
...
def test_close_open_fds(patching):
    ...
    fdmax = patching('distll_celery.platforms.get_fdmax')
...
class test_set_process_title:
    ...
    @patch('distll_celery.platforms.set_process_title')
    @patch('distll_celery.platforms.current_process')
...
class test_maybe_drop_privileges:
    ...
    @patch('os.geteuid')
    @patch('os.getuid')
    @patch('distll_celery.platforms.parse_uid')
    @patch('distll_celery.platforms.parse_gid')
    @patch('pwd.getpwuid')
    @patch('distll_celery.platforms.setgid')
    @patch('distll_celery.platforms.setuid')
    @patch('distll_celery.platforms.initgroups')
...
class test_initgroups:
    ...
    @patch('distll_celery.platforms.setgroups')
    @patch('grp.getgrall')
    @patch('pwd.getpwuid')
...
class test_detached:
    ...
    @patch('distll_celery.platforms._create_pidlock')
    @patch('distll_celery.platforms.signals')
    @patch('distll_celery.platforms.maybe_drop_privileges')
    @patch('os.geteuid')
...
class test_DaemonContext:
    ...
    def test_open(self, _close_fds, dup2, open, close, closer, umask, chdir,
        ...
        with patch('distll_celery.platforms.mputil') as mputil:
...
class test_Pidfile:
    ...
    @patch('distll_celery.platforms.Pidfile')
...
class test_setgroups:
    ...
    @skip.if_win32()
    @patch('distll_celery.platforms._setgroups_hack')
