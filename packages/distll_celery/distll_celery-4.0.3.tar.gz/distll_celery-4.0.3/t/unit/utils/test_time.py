...
from distll_celery.utils.time import (
    delta_resolution,
    humanize_seconds,
    maybe_iso8601,
    maybe_timedelta,
...
from distll_celery.utils.iso8601 import parse_iso8601


class test_LocalTimezone:

    def test_daylight(self, patching):
        time = patching('distll_celery.utils.time._time')
        time.timezone = 3600
        time.daylight = False
        x = LocalTimezone()
...
class test_utcoffset:
    ...
    def test_utcoffset(self, patching):
        _time = patching('distll_celery.utils.time._time')
        _time.daylight = True
