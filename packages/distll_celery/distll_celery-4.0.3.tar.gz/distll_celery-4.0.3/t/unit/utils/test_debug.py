...
def test_blockdetection(patching):
    signals = patching('distll_celery.utils.debug.signals')
...
def test_sample_mem(patching):
    mem_rss = patching('distll_celery.utils.debug.mem_rss')
...
def test_mem_rss(patching):
    humanbytes = patching('distll_celery.utils.debug.humanbytes')
    ps = patching('distll_celery.utils.debug.ps')
    ret = debug.mem_rss()
...
def test_ps(patching):
    Process = patching('distll_celery.utils.debug.Process')
