...
from distll_celery.fixups.django import (
    _maybe_close_fd,
    fixup,
    FixupWarning,
    DjangoFixup,
...
class FixupCase:
    ...
    def fixup_context(self, app):
        with patch('distll_celery.fixups.django.DjangoWorkerFixup.validate_models'):
            with patch('distll_celery.fixups.django.symbol_by_name') as symbyname:
                with patch('distll_celery.fixups.django.import_module') as impmod:
                    f = self.Fixup(app)
...
class test_DjangoFixup(FixupCase):
    ...
    @patch('distll_celery.fixups.django.DjangoWorkerFixup')
    ...
    def test_fixup(self, patching):
        with patch('distll_celery.fixups.django.DjangoFixup') as Fixup:
    ...
    def test_install(self, patching):
        ...
        self.sigs = patching('distll_celery.fixups.django.signals')
    ...
    def test_on_worker_init(self):
        with self.fixup_context(self.app) as (f, _, _):
            with patch('distll_celery.fixups.django.DjangoWorkerFixup') as DWF:
                f.on_worker_init()
...
class test_DjangoWorkerFixup(FixupCase):
    ...
    def test_install(self):
        ...
        self.app.loader = Mock()
        with self.fixup_context(self.app) as (f, _, _):
            with patch('distll_celery.fixups.django.signals') as sigs:
                f.install()
    ...
    def test_on_worker_process_init(self, patching):
        with self.fixup_context(self.app) as (f, _, _):
            with patch('distll_celery.fixups.django._maybe_close_fd') as mcf:
    ...
    def test_django_setup(self, patching):
        patching('distll_celery.fixups.django.symbol_by_name')
        patching('distll_celery.fixups.django.import_module')
