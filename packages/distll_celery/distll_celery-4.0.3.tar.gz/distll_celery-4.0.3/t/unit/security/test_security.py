...
from distll_celery.exceptions import ImproperlyConfigured, SecurityError
from distll_celery.five import builtins
from distll_celery.security import disable_untrusted_serializers, setup_security
from distll_celery.security.utils import reraise_errors
...
class test_security(SecurityCase):
    ...
    @patch('distll_celery.security._disable_insecure_serializers')
    ...
    @patch('distll_celery.current_app')
    def test_setup_security__default_app(self, current_app):
        setup_security()

    @patch('distll_celery.security.register_auth')
    @patch('distll_celery.security._disable_insecure_serializers')
    def test_setup_registry_complete(self, dis, reg, key='KEY', cert='CERT'):
        ...
        with mock.open(side_effect=effect):
            with patch('distll_celery.security.registry') as registry:
                store = Mock()
