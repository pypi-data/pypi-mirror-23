...
from distll_celery.app.trace import (
    trace_task,
    _trace_task_ret,
    TraceInfo,
    mro_lookup,
...
from distll_celery.five import monotonic
from distll_celery.signals import task_revoked
from distll_celery.worker import request as module
from distll_celery.worker.request import (
...
class test_trace_task(RequestCase):
    ...
    def test_process_cleanup_fails(self, patching):
        _logger = patching('distll_celery.app.trace.logger')
...
class test_Request(RequestCase):
    ...
    def test_valid_expires_with_utc_makes_aware(self):
        with patch('distll_celery.worker.request.maybe_make_aware') as mma:
    ...
    def test_on_timeout(self, patching):
        warn = patching('distll_celery.worker.request.warn')
        error = patching('distll_celery.worker.request.error')
...
class test_create_request_class(RequestCase):
    ...
    def test_execute_using_pool(self):
        from distll_celery.app.trace import trace_task_ret as trace
