...
from distll_celery.events.receiver import CLIENT_CLOCK_SKEW
...
class test_EventReceiver:
    ...
    def test_catch_all_event(self):
        ...
        connection = Mock()
        connection.transport_cls = 'memory'
        r = self.app.events.Receiver(connection, node_id='distll_celery.tests')
    ...
    def test_event_from_message_localize_disabled(self):
        r = self.app.events.Receiver(Mock(), node_id='distll_celery.tests')
        r.adjust_clock = Mock()
        ts_adjust = Mock()
