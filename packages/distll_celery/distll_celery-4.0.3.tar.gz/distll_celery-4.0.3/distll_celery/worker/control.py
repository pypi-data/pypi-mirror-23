...
from distll_celery.exceptions import WorkerShutdown
from distll_celery.five import UserDict, items, string_t, text_t
from distll_celery.platforms import signals as _signals
from distll_celery.utils.functional import maybe_list
from distll_celery.utils.log import get_logger
from distll_celery.utils.serialization import jsonify, strtobool
from distll_celery.utils.time import rate
...
def rate_limit(state, task_name, rate_limit, **kwargs):
    ...
    See Also:
        :attr:`distll_celery.task.base.Task.rate_limit`.
...
def registered(state, taskinfoitems=None, builtins=False, **kwargs):
    ...
    tasks = reg if builtins else (
        task for task in reg if not task.startswith('distll_celery.'))
...
def memsample(state, **kwargs):
    ...
    from distll_celery.utils.debug import sample_mem
    return sample_mem()
...
def memdump(state, samples=10, **kwargs):  # pragma: no cover
    ...
    from distll_celery.utils import debug
    out = io.StringIO()
