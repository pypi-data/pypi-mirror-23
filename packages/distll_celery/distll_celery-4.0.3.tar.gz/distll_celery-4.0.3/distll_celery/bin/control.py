...
from distll_celery.five import items, string_t
from distll_celery.bin.base import Command
from distll_celery.platforms import EX_UNAVAILABLE, EX_USAGE
...
class _RemoteControl(Command):
    ...
    def _choices_by_group(cls, app):
        from distll_celery.worker.control import Panel
...
class inspect(_RemoteControl):
    ...
    Examples:
        .. code-block:: console
            ...
            $ distll_celery inspect active --timeout=5
            $ distll_celery inspect scheduled -d worker1@example.com
            $ distll_celery inspect revoked -d w1@e.com,w2@e.com
    """
...
class control(_RemoteControl):
    ...
    Examples:
        .. code-block:: console
            ...
            $ distll_celery control enable_events --timeout=5
            $ distll_celery control -d worker1@example.com enable_events
            $ distll_celery control -d w1.e.com,w2.e.com enable_events

            $ distll_celery control -d w1.e.com add_consumer queue_name
            $ distll_celery control -d w1.e.com cancel_consumer queue_name

            $ distll_celery control add_consumer queue exchange direct rkey
    """
