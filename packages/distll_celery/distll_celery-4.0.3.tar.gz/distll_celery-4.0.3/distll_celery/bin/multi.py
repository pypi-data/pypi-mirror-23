...
.. code-block:: console
    ...
    $ distll_celery multi start Leslie -E

    $ # Pidfiles and logfiles are stored in the current directory
    $ # by default.  Use --pidfile and --logfile argument to change
    $ # this.  The abbreviation %n will be expanded to the current
    $ # node name.
    $ distll_celery multi start Leslie -E --pidfile=/var/run/distll_celery/%n.pid
                                   --logfile=/var/log/distll_celery/%n%I.log


    $ # You need to add the same arguments when you restart,
    $ # as these aren't persisted anywhere.
    $ distll_celery multi restart Leslie -E --pidfile=/var/run/distll_celery/%n.pid
                                     --logfile=/var/run/distll_celery/%n%I.log

    $ # To stop the node, you need to specify the same pidfile.
    $ distll_celery multi stop Leslie --pidfile=/var/run/distll_celery/%n.pid

    $ # 3 workers, with 3 processes each
    $ distll_celery multi start 3 -c 3
    distll_celery worker -n celery1@myhost -c 3
    distll_celery worker -n celery2@myhost -c 3
    distll_celery worker -n celery3@myhost -c 3

    $ # start 3 named workers
    $ distll_celery multi start image video data -c 3
    distll_celery worker -n image@myhost -c 3
    distll_celery worker -n video@myhost -c 3
    distll_celery worker -n data@myhost -c 3

    $ # specify custom hostname
    $ distll_celery multi start 2 --hostname=worker.example.com -c 3
    distll_celery worker -n celery1@worker.example.com -c 3
    distll_celery worker -n celery2@worker.example.com -c 3

    $ # specify fully qualified nodenames
    $ distll_celery multi start foo@worker.example.com bar@worker.example.com -c 3

    $ # fully qualified nodenames but using the current hostname
    $ distll_celery multi start foo@%h bar@%h

    $ # Advanced example starting 10 workers in the background:
    $ #   * Three of the workers processes the images and video queue
    $ #   * Two of the workers processes the data queue with loglevel DEBUG
    $ #   * the rest processes the default' queue.
    $ distll_celery multi start 10 -l INFO -Q:1-3 images,video -Q:4,5 data
        -Q default -L:4,5 DEBUG

    $ # You can show the commands necessary to start the workers with
    $ # the 'show' command:
    $ distll_celery multi show 10 -l INFO -Q:1-3 images,video -Q:4,5 data
        -Q default -L:4,5 DEBUG

    $ # Additional options are added to each distll_celery worker' comamnd,
    $ # but you can also modify the options for ranges of, or specific workers

    $ # 3 workers: Two with 3 processes, and one with 10 processes.
    $ distll_celery multi start 3 -c 3 -c:1 10
    distll_celery worker -n celery1@myhost -c 10
    distll_celery worker -n celery2@myhost -c 3
    distll_celery worker -n celery3@myhost -c 3

    $ # can also specify options for named workers
    $ distll_celery multi start image video data -c 3 -c:image 10
    distll_celery worker -n image@myhost -c 10
    distll_celery worker -n video@myhost -c 3
    distll_celery worker -n data@myhost -c 3

    $ # ranges and lists of workers in options is also allowed:
    $ # (-c:1-3 can also be written as -c:1,2,3)
    $ distll_celery multi start 5 -c 3  -c:1-3 10
    distll_celery worker -n celery1@myhost -c 10
    distll_celery worker -n celery2@myhost -c 10
    distll_celery worker -n celery3@myhost -c 10
    distll_celery worker -n celery4@myhost -c 3
    distll_celery worker -n celery5@myhost -c 3

    $ # lists also works with named workers
    $ distll_celery multi start foo bar baz xuzzy -c 3 -c:foo,bar,baz 10
    distll_celery worker -n foo@myhost -c 10
    distll_celery worker -n bar@myhost -c 10
    distll_celery worker -n baz@myhost -c 10
    distll_celery worker -n xuzzy@myhost -c 3
"""
...
from distll_celery import VERSION_BANNER
from distll_celery.apps.multi import Cluster, MultiParser, NamespacedOptionParser
from distll_celery.platforms import EX_FAILURE, EX_OK, signals
from distll_celery.utils import term
from distll_celery.utils.text import pluralize
...
class TermLogger(object):
    ...
    splash_text = 'distll_celery multi v{version}'
...
class MultiTool(TermLogger):
    """The ``distll_celery multi`` program."""
    ...
    def __init__(self, env=None, cmd=None,
        ...
        self.fh = self.stdout
        self.prog_name = 'distll_celery multi'
        self.commands = {
            'start': self.start,
            'show': self.show,
            'stop': self.stop,
