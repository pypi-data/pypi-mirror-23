...
from distll_celery import worker
from distll_celery.result import allow_join_result, _set_task_join_will_block
from distll_celery.utils.dispatch import Signal
from distll_celery.utils.nodenames import anon_nodename
...
def start_worker(app,
    ...
    Yields:
        distll_celery.app.worker.Worker: worker instance.
    """
...
def _start_worker_thread(app,
    ...
    Yields:
        distll_celery.worker.Worker: worker instance.
    """
    setup_app_for_worker(app, loglevel, logfile)
    assert 'distll_celery.ping' in app.tasks
...
def _start_worker_process(app,
    ...
    Yields:
        distll_celery.app.worker.Worker: worker instance.
    """
    from distll_celery.apps.multi import Cluster, Node
