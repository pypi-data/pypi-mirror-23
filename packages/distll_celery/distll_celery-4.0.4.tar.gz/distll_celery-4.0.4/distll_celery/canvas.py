...
.. seealso:
    ...
    You should import these from :mod:`distll_celery` and not this module.
"""
...
from distll_celery.five import python_2_unicode_compatible
from distll_celery.local import try_import
from distll_celery.result import GroupResult
from distll_celery.utils import abstract
from distll_celery.utils.functional import (
    maybe_list, is_list, _regen, regen, chunks as _chunks,
    seq_concat_seq, seq_concat_item,
)
from distll_celery.utils.objects import getitem_property
from distll_celery.utils.text import truncate, remove_repeating_from_task
...
class Signature(dict):
    ...
    Note:
        You should use :func:`~distll_celery.signature` to create new signatures.
...
class _chain(Signature):
    ...
    def __init__(self, *tasks, **options):
        ...
        Signature.__init__(
            self, 'distll_celery.chain', (), {'tasks': tasks}, **options
        )
...
class chain(_chain):
    ...
    Returns:
        ~distll_celery.chain: A lazy signature that can be called to apply the first
...
class xstarmap(_basemap):
    ...
    _task_name = 'distll_celery.starmap'
...
class group(Signature):
    ...
    Returns:
        ~distll_celery.group: signature that when called will then call all of the
    ...
    def __init__(self, *tasks, **options):
        if len(tasks) == 1:
            ...
            if not isinstance(tasks, _regen):
                tasks = regen(tasks)
        Signature.__init__(
            self, 'distll_celery.group', (), {'tasks': tasks}, **options
        )
...
class chord(Signature):
    ...
    def __init__(self, header, body=None, task='distll_celery.chord',
    ...
    def _get_app(self, body=None):
        ...
        if app is None:
            try:
                ...
                tasks = self.tasks
            app = tasks[0]._app if len(tasks) else None
