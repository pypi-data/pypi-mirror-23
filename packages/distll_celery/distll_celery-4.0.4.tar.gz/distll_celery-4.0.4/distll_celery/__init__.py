...
if STATICA_HACK:  # pragma: no cover
    from distll_celery.app import shared_task                   # noqa
    from distll_celery.app.base import Celery                   # noqa
    from distll_celery.app.utils import bugreport               # noqa
    from distll_celery.app.task import Task                     # noqa
    from distll_celery._state import current_app, current_task  # noqa
    from distll_celery.canvas import (                          # noqa
        chain, chord, chunks, group,
        signature, maybe_signature, xmap, xstarmap, subtask,
    )
    from distll_celery.utils import uuid                        # noqa
...
old_module, new_module = local.recreate_module(  # pragma: no cover
    __name__,
    by_module={
        'distll_celery.app': ['Celery', 'bugreport', 'shared_task'],
        'distll_celery.app.task': ['Task'],
        'distll_celery._state': ['current_app', 'current_task'],
        'distll_celery.canvas': [
            'Signature', 'chain', 'chord', 'chunks', 'group',
            'signature', 'maybe_signature', 'subtask',
            'xmap', 'xstarmap',
        ],
        'distll_celery.utils': ['uuid'],
    },
    direct={'task': 'distll_celery.task'},
    __package__='distll_celery', __file__=__file__,
