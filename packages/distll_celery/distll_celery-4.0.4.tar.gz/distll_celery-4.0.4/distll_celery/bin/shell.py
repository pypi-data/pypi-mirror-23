...
from distll_celery.five import values
from distll_celery.bin.base import Command


class shell(Command):  # pragma: no cover
    """Start shell session with convenient access to distll_celery symbols.

    The following symbols will be added to the main globals:

        - ``distll_celery``:  the current application.
    ...
    def _run(self, ipython=False, bpython=False,
        ...
        if eventlet:
            import_module('distll_celery.concurrency.eventlet')
        if gevent:
            import_module('distll_celery.concurrency.gevent')
        import distll_celery
        import distll_celery.task.base
        self.app.loader.import_default_modules()

        # pylint: disable=attribute-defined-outside-init
        self.locals = {
            'app': self.app,
            'distll_celery': self.app,
            'Task': distll_celery.Task,
            'chord': distll_celery.chord,
            'group': distll_celery.group,
            'chain': distll_celery.chain,
            'chunks': distll_celery.chunks,
            'xmap': distll_celery.xmap,
            'xstarmap': distll_celery.xstarmap,
            'subtask': distll_celery.subtask,
            'signature': distll_celery.signature,
        }

        if not without_tasks:
            self.locals.update({
                task.__name__: task for task in values(self.app.tasks)
                if not task.name.startswith('distll_celery.')
            })
