...
import os
import sys
from distll_celery.platforms import EX_FAILURE, detached
from distll_celery.utils.log import get_logger
from distll_celery.utils.nodenames import default_nodename, node_format
from distll_celery.bin.base import daemon_options
...
class detached_celeryd(object):
    """Daemonize the distll_celery worker process."""

    usage = '%(prog)s [options] [celeryd options]'
    version = distll_celery.VERSION_BANNER
    description = ('Detaches Celery worker nodes.  See `distll_celery worker --help` '
                   'for the list of supported worker arguments.')
    command = sys.executable
    execv_path = sys.executable
    execv_argv = ['-m', 'distll_celery', 'worker']
    ...
    def _extract_command_line_config(self, argv):
        ...
        #    distll_celery worker -l info -- worker.prefetch_multiplier=10
