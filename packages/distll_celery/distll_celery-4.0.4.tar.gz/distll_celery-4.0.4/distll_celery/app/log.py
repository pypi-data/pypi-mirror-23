...
from distll_celery.platforms import isatty
from distll_celery.utils.log import (
    get_logger, mlevel,
    ColorFormatter, LoggingProxy, get_multiprocessing_logger,
    reset_multiprocessing_logger,
)
from distll_celery.utils.nodenames import node_format
from distll_celery.utils.term import colored
...
class Logging(object):
    ...
    def redirect_stdouts(self, loglevel=None, name='distll_celery.redirected'):
    ...
    def setup_logging_subsystem(self, loglevel=None, logfile=None, format=None,
        ...
        if not receivers:
            ...
            if self.app.conf.worker_hijack_root_logger:
                root.handlers = []
                get_logger('distll_celery').handlers = []
                get_logger('distll_celery.task').handlers = []
                get_logger('distll_celery.redirected').handlers = []
    ...
    def setup_logger(self, name='distll_celery', *args, **kwargs):
        """Deprecated: No longer used."""
        self.setup_logging_subsystem(*args, **kwargs)
        return logging.root

    def get_default_logger(self, name='distll_celery', **kwargs):
