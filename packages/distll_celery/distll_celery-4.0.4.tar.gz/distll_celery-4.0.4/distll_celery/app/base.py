...
from distll_celery import platforms
from distll_celery import signals
from distll_celery._state import (
...
from distll_celery.exceptions import AlwaysEagerIgnored, ImproperlyConfigured
from distll_celery.five import (
    UserDict, bytes_if_py2, python_2_unicode_compatible, values,
)
from distll_celery.loaders import get_loader_cls
from distll_celery.local import PromiseProxy, maybe_evaluate
from distll_celery.utils import abstract
from distll_celery.utils.collections import AttributeDictMixin
from distll_celery.utils.dispatch import Signal
from distll_celery.utils.functional import first, maybe_list, head_from_fun
from distll_celery.utils.time import timezone
from distll_celery.utils.imports import gen_task_name, instantiate, symbol_by_name
from distll_celery.utils.log import get_logger
from distll_celery.utils.objects import FallbackContext, mro_lookup
...
BUILTIN_FIXUPS = {
    'distll_celery.fixups.django:fixup',
}
...
class Celery(object):
    ...
    Keyword Arguments:
        ...
        fixups (List[str]): List of fix-up plug-ins (e.g., see
            :mod:`distll_celery.fixups.django`).
    ...
    amqp_cls = 'distll_celery.app.amqp:AMQP'
    backend_cls = None
    events_cls = 'distll_celery.app.events:Events'
    loader_cls = None
    log_cls = 'distll_celery.app.log:Logging'
    control_cls = 'distll_celery.app.control:Control'
    task_cls = 'distll_celery.app.task:Task'
    ...
    def _get_default_loader(self):
        ...
        return (
            ...
            self.loader_cls or
            'distll_celery.loaders.app:AppLoader'
        )
    ...
    def start(self, argv=None):
        """Run :program:`distll_celery` using `argv`.

        Uses :data:`sys.argv` if `argv` is not specified.
        """
        return instantiate(
            'distll_celery.bin.distll_celery:CeleryCommand', app=self
        ).execute_from_commandline(argv)

    def worker_main(self, argv=None):
        """Run :program:`distll_celery worker` using `argv`.

        Uses :data:`sys.argv` if `argv` is not specified.
        """
        return instantiate(
            'distll_celery.bin.worker:worker', app=self
        ).execute_from_commandline(argv)
    ...
    def config_from_object(self, obj,
        ...
        Example:
            >>> distll_celery.config_from_object('myapp.celeryconfig')

            >>> from myapp import celeryconfig
            >>> distll_celery.config_from_object(celeryconfig)
    ...
    def config_from_envvar(self, variable_name, silent=False, force=False):
        ...
        Example:
            ...
            >>> distll_celery.config_from_envvar('CELERY_CONFIG_MODULE')
        """
    ...
    def config_from_cmdline(self, argv, namespace='distll_celery'):
        self._conf.update(
    ...
    def setup_security(self, allowed_serializers=None, key=None, cert=None,
        ...
        """
        from distll_celery.security import setup_security
    ...
    def signature(self, *args, **kwargs):
        """Return a new :class:`~distll_celery.Signature` bound to this app."""
        kwargs['app'] = self
    ...
    def Worker(self):
        ...
        See Also:
            :class:`~@Worker`.
        """
        return self.subclass_with_self('distll_celery.apps.worker:Worker')
    ...
    def WorkController(self, **kwargs):
        ...
        """
        return self.subclass_with_self('distll_celery.worker:WorkController')

    @cached_property
    def Beat(self, **kwargs):
        """:program:`distll_celery beat` scheduler application.

        See Also:
            :class:`~@Beat`.
        """
        return self.subclass_with_self('distll_celery.apps.beat:Beat')
    ...
    def AsyncResult(self):
        ...
        See Also:
            :class:`distll_celery.result.AsyncResult`.
        """
        return self.subclass_with_self('distll_celery.result:AsyncResult')

    @cached_property
    def ResultSet(self):
        return self.subclass_with_self('distll_celery.result:ResultSet')
    ...
    def GroupResult(self):
        ...
        See Also:
            :class:`distll_celery.result.GroupResult`.
        """
        return self.subclass_with_self('distll_celery.result:GroupResult')
    ...
    @cached_property
    def _canvas(self):
        from distll_celery import canvas
        return canvas
