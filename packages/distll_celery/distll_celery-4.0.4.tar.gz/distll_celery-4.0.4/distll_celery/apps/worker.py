...
This module is the 'program-version' of :mod:`distll_celery.worker`.
...
from distll_celery.exceptions import WorkerShutdown, WorkerTerminate
from distll_celery.five import string, string_t
from distll_celery.loaders.app import AppLoader
from distll_celery.platforms import EX_FAILURE, EX_OK, check_privileges, isatty
from distll_celery.utils import static
from distll_celery.utils import term
from distll_celery.utils.debug import cry
from distll_celery.utils.imports import qualname
from distll_celery.utils.log import get_logger, in_sighandler, set_in_sighandler
from distll_celery.utils.text import pluralize
from distll_celery.worker import WorkController
...
class Worker(WorkController):
    ...
    def startup_info(self, artlines=True):
        ...
        if not isinstance(app.loader, AppLoader):
            ...
            if loader.startswith('distll_celery.loaders'):  # pragma: no cover
                loader = loader[14:]
    ...
    def macOS_proxy_detection_workaround(self):
        """See https://github.com/distll_celery/distll_celery/issues#issue/161."""
...
def install_worker_restart_handler(worker, sig='SIGHUP'):
    ...
    def restart_worker_sig_handler(*args):
        ...
        safe_say('Restarting distll_celery worker ({0})'.format(' '.join(sys.argv)))
        import atexit
...
def install_rdb_handler(envvar='CELERY_RDBSIG',
    ...
    def rdb_handler(*args):
        ...
        with in_sighandler():
            from distll_celery.contrib.rdb import set_trace, _frame
