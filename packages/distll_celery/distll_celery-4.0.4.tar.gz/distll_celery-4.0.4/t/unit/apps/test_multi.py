...
from distll_celery.apps.multi import (
...
class test_multi_args:
    ...
    @patch('distll_celery.apps.multi.gethostname')
    def test_parse(self, gethostname):
        ...
        for i, worker in enumerate(nodes3[1:]):
            assert worker.name == 'distll_celery%s@example.com' % (i + 2)
            node_i = 'distll_celery%s' % (i + 2,)
            assert worker.argv == (
                'COMMAND',
...
class test_Node:
    ...
    @patch('distll_celery.apps.multi.Popen')
...
class test_Cluster:
    ...
    def setup(self):
        self.Popen = self.patching('distll_celery.apps.multi.Popen')
        self.kill = self.patching('os.kill')
        self.gethostname = self.patching('distll_celery.apps.multi.gethostname')
        self.gethostname.return_value = 'example.com'
        self.Pidfile = self.patching('distll_celery.apps.multi.Pidfile')
    ...
    def test_getpids(self):
        ...
        nodes = p.getpids('distll_celery worker')
