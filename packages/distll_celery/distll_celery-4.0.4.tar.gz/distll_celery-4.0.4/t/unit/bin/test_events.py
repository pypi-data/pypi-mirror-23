...
from distll_celery.bin import events
...
class test_events:
    ...
    @_old_patch('distll_celery.events.dumper', 'evdump',
                lambda **kw: 'me dumper, you?')
    @_old_patch('distll_celery.bin.events', 'set_process_title', proctitle)
    def test_run_dump(self):
        assert self.ev.run(dump=True), 'me dumper == you?'
        assert 'distll_celery events:dump' in proctitle.last[0]

    @skip.unless_module('curses', import_errors=(ImportError, OSError))
    def test_run_top(self):
        @_old_patch('distll_celery.events.cursesmon', 'evtop',
                    lambda **kw: 'me top, you?')
        @_old_patch('distll_celery.bin.events', 'set_process_title', proctitle)
        def _inner():
            assert self.ev.run(), 'me top == you?'
            assert 'distll_celery events:top' in proctitle.last[0]
        return _inner()

    @_old_patch('distll_celery.events.snapshot', 'evcam',
                lambda *a, **k: (a, k))
    @_old_patch('distll_celery.bin.events', 'set_process_title', proctitle)
    def test_run_cam(self):
        ...
        assert 'distll_celery events:cam' in proctitle.last[0]

    @patch('distll_celery.events.snapshot.evcam')
    @patch('distll_celery.bin.events.detached')
    def test_run_cam_detached(self, detached, evcam):
        self.ev.prog_name = 'distll_celery events'
    ...
    @_old_patch('distll_celery.bin.events', 'events', MockCommand)
    def test_main(self):
