...
from distll_celery import shared_task, current_app
from distll_celery import app as _app
from distll_celery import _state
from distll_celery.app import base as _appbase
from distll_celery.app import defaults
from distll_celery.exceptions import ImproperlyConfigured
from distll_celery.five import items, keys
from distll_celery.loaders.base import unconfigured
from distll_celery.platforms import pyimplementation
from distll_celery.utils.collections import DictAttribute
from distll_celery.utils.serialization import pickle
from distll_celery.utils.time import timezone
from distll_celery.utils.objects import Bunch
...
class test_task_join_will_block:
    ...
    def test_task_join_will_block(self, patching):
        patching('distll_celery._state._task_join_will_block', 0)
...
class test_App:
    ...
    def test_registry_autofinalize_disabled(self):
        with self.Celery('xyzibari', autofinalize=False) as app:
            with pytest.raises(RuntimeError):
                app.tasks['distll_celery.chain']
            app.finalize()
            assert app.tasks['distll_celery.chain']
    ...
    def test_autodiscover_tasks_lazy(self):
        with patch('distll_celery.signals.import_modules') as import_modules:
            def lazy_list():
                return [1, 2, 3]
    ...
    def test_task_not_shared(self):
        with patch('distll_celery.app.base.connect_on_app_finalize') as sh:
            @self.app.task(shared=False)
            def foo():
                pass
    ...
    def test_task_sets_main_name_MP_MAIN_FILE(self):
        from distll_celery.utils import imports as _imports
    ...
    def test_annotate_decorator(self):
        from distll_celery.app.task import Task
    ...
    def test_apply_async_has__self__(self):
        ...
        with patch('distll_celery.app.amqp.AMQP.create_task_message') as create:
            with patch('distll_celery.app.amqp.AMQP.send_task_message') as send:
    ...
    def test_apply_async_adds_children(self):
        from distll_celery._state import _task_stack
    ...
    def test_worker_main(self):
        from distll_celery.bin import worker as worker_bin
    ...
    def test_config_from_object__namespace_lowercase(self):
        ...
        self.app.config_from_object(Config(), namespace='distll_celery')
        assert self.app.conf.task_always_eager == 44
    ...
    @patch('distll_celery.bin.distll_celery.CeleryCommand.execute_from_commandline')
    ...
    @patch('distll_celery.app.base.logger')
    ...
    def test_Beat(self):
        from distll_celery.apps.beat import Beat
        beat = self.app.Beat()
