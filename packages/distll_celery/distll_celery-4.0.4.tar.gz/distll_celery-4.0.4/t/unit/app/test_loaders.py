...
from distll_celery import loaders
from distll_celery.exceptions import NotConfigured
from distll_celery.five import bytes_if_py2
from distll_celery.loaders import base
from distll_celery.loaders import default
from distll_celery.loaders.app import AppLoader
from distll_celery.utils.imports import NotAPackage
...
class test_LoaderBase:
    ...
    def test_autodiscovery(self):
        with patch('distll_celery.loaders.base.autodiscover_tasks') as auto:
...
class test_DefaultLoader:
    ...
    @patch('distll_celery.loaders.base.find_module')
    ...
    def test_import_from_cwd(self):
        ...
        try:
            ...
            pass
        celery = sys.modules.pop('distll_celery', None)
        sys.modules.pop('distll_celery.local', None)
        try:
            assert l.import_from_cwd('distll_celery')
            sys.modules.pop('distll_celery', None)
            sys.modules.pop('distll_celery.local', None)
            sys.path.insert(0, os.getcwd())
            assert l.import_from_cwd('distll_celery')
        finally:
            sys.path = old_path
            sys.modules['distll_celery'] = celery
...
class test_autodiscovery:
    ...
    def test_autodiscover_tasks(self):
        ...
        with patch('distll_celery.loaders.base.find_related_module') as frm:
            base.autodiscover_tasks(['foo'])
            frm.assert_called()
