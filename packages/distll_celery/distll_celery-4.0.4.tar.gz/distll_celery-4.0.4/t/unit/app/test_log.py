...
from distll_celery import uuid
from distll_celery.app.log import TaskFormatter
from distll_celery.five import python_2_unicode_compatible
from distll_celery.utils.log import LoggingProxy
...
class test_ColorFormatter:
    ...
    @patch('distll_celery.utils.log.safe_str')
...
class test_default_logger:
    ...
    def test_get_logger_sets_parent(self):
        logger = get_logger('distll_celery.test_get_logger')
    ...
    def test_setup_logger_no_handlers_file(self, *args):
        tempfile = mktemp(suffix='unittest', prefix='distll_celery')
...
class test_task_logger(test_default_logger):
    ...
    def setup(self):
        logger = self.logger = get_logger('distll_celery.task')
        logger.handlers = []
    ...
    def test_renaming_base_logger(self):
        with pytest.raises(RuntimeError):
            get_task_logger('distll_celery')

    def test_renaming_task_logger(self):
        with pytest.raises(RuntimeError):
            get_task_logger('distll_celery.task')
