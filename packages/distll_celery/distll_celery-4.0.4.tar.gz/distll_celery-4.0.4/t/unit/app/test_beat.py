...
from distll_celery import beat
from distll_celery import uuid
from distll_celery.five import keys, string_t
from distll_celery.schedules import schedule
...
class test_ScheduleEntry:
    ...
    def create_entry(self, **kwargs):
        entry = dict(
            name='distll_celery.unittest.add',
...
class test_Scheduler:
    ...
    @patch('distll_celery.app.base.Celery.send_task')
    ...
    def test_install_default_entries(self):
        ...
        assert 'distll_celery.backend_cleanup' not in s.data
        self.app.backend.supports_autoexpire = False

        self.app.conf.result_expires = 30
        s = mScheduler(app=self.app)
        s.install_default_entries({})
        assert 'distll_celery.backend_cleanup' in s.data
    ...
    @patch('distll_celery.beat.error')
...
class test_EmbeddedService:
    ...
    def xxx_start_stop_process(self):
        ...
        with patch('distll_celery.platforms.close_open_fds'):
            s.run()
