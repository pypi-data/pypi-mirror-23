...
from distll_celery.app.defaults import (
...
class test_defaults:
    ...
    def setup(self):
        self._prev = sys.modules.pop('distll_celery.app.defaults', None)

    def teardown(self):
        if self._prev:
            sys.modules['distll_celery.app.defaults'] = self._prev
    ...
    def test_find(self):
        ...
        assert find('default_queue')[2].default == 'distll_celery'
        assert find('task_default_exchange')[2] is None

    @property
    def defaults(self):
        return import_module('distll_celery.app.defaults')
