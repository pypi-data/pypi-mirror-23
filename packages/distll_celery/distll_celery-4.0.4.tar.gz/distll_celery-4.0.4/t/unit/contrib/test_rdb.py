...
from distll_celery.contrib.rdb import (
    Rdb,
    debugger,
    set_trace,
)
...
class test_Rdb:
    ...
    @patch('distll_celery.contrib.rdb.Rdb')
    def test_debugger(self, Rdb):
        x = debugger()
        assert x
        assert x is debugger()

    @patch('distll_celery.contrib.rdb.debugger')
    @patch('distll_celery.contrib.rdb._frame')
    def test_set_trace(self, _frame, debugger):
        assert set_trace(Mock())
        assert set_trace()
        debugger.return_value.set_trace.assert_called()

    @patch('distll_celery.contrib.rdb.Rdb.get_avail_port')
    @skip.if_pypy()
    def test_rdb(self, get_avail_port):
        ...
        with Rdb(out=out) as rdb:
            ...
            # set_trace
            with patch('distll_celery.contrib.rdb.Pdb.set_trace') as pset:
                with patch('distll_celery.contrib.rdb._frame'):
                    rdb.set_trace()
                    rdb.set_trace(Mock())
    ...
    def test_get_avail_port(self, sock):
        ...
        with patch('distll_celery.contrib.rdb.current_process') as curproc:
