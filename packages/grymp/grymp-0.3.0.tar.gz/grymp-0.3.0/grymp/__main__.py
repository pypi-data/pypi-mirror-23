...
def main(args):
    ...
    try:
        for base in args.base:
            ...
            if args.feature:
                if not feature.Feature(fs).process(base, name, args.feature):
                    return 2
            if args.extras:
                if not extras.Extras(fs).process(base, name, args.extras):
                    return 3

            # this *must* only execute if both the feature and extras succeeded
