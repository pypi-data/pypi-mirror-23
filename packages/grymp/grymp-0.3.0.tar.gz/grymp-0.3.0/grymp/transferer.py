...
class Transferer(object):
    ...
    def _validate(self, src, dest, message):
        """
        ...
        if os.path.exists(dest) and \
            ...
            if not util.binary_prompt(
                    'Overwrite {0} ({1} {2})?'.format(
                        dest,
                        util.format_bytes(abs(src_size - dest_size)),
                        'smaller' if src_size > dest_size else 'larger'),
                    src_size > dest_size):
                return False
    ...
    def move(self, src, dest):
        ...
        """
        if not self._validate(src, dest, 'Move {0} to {1}?'.format(src, dest)):
            return False
    ...
    def copy_delete(self, src, dest):
        ...
        """
        if not self._validate(src, dest, 'Copy {0} to {1}?'.format(src, dest)):
            return False
