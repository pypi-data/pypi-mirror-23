...
class Basecall2DTools(Basecall1DTools):
    ...
    def add_called_sequence(self, section="requires: '2D'", name=None, sequence=None, qstring=None):
        ...
        """
        if section != "2D":
            warnings.warn("Basecall2DTools.add_called_sequence() should specify section='2D'", DeprecationWarning)
            # Backwards compatibilty to 0.3.3, if no "2D" section, bump args by 1 and pass to super
            if section == "requires: '2D'":
                # We assume that a named args were given
                return super(Basecall2DTools, self).add_called_sequence("2D", name, sequence, qstring)
            # We assume ordered args were given off-by-1
            return super(Basecall2DTools, self).add_called_sequence("2D", section, name, sequence)
        return super(Basecall2DTools, self).add_called_sequence(section, name, sequence, qstring)
