...
def read_or_init_config(file, profile):
    ...
    try:
        ...
        with open(file, 'w', 0o600) as config_file:
            mfa_name = raw_input("[%s] Please enter MFA device name: " % profile)
            data[profile] =  {
                "mfa_name": mfa_name
            }
...
def session_expiration_diff(d1, d2):
...
def print_environment(aws_access_key, aws_secret_access_key, aws_session_token):
        ...
        print ("export AWS_ACCESS_KEY_ID=%s" % (pipes.quote(str(aws_access_key))))
        print ("export AWS_SECRET_ACCESS_KEY=%s" % (pipes.quote(str(aws_secret_access_key))))
        print ("export AWS_SESSION_TOKEN=%s" % (pipes.quote(str(aws_session_token))))
...
def print_environment_disabled(aws_access_key_id, aws_secret_access_key):
        ...
        if aws_access_key_id and aws_secret_access_key:
            print ("export AWS_ACCESS_KEY_ID=%s" % (pipes.quote(str(aws_access_key_id))))
            print ("export AWS_SECRET_ACCESS_KEY=%s" % (pipes.quote(str(aws_secret_access_key))))
        else:
...
def main():
    ...
    if args.disable:
        if args.profile in credentials:
            ...
            logger.error("Profile does not exist in: %s" % CREDENTIALS_FILE)
            sys.exit(1)

    data = read_or_init_config(CONFIG_FILE, args.profile)
    if data is None:
        logger.error("Incorrect/empty config file %s" % CONFIG_FILE)
        sys.exit(1)

    if args.profile not in data: 
            mfa_name = raw_input("[%s] Please enter MFA device name: " % args.profile)
            data[args.profile] =  {
                "mfa_name": mfa_name
            }
    ...
    if validate_mfa_name(data[args.profile]['mfa_name']) is None:
        logger.error("MFA name validation error %s" % CONFIG_FILE)
        sys.exit(1)
    ...
    if aws_access_key_id and aws_secret_access_key and awsauth_enabled != 1:
        ...
        credentials[args.profile]['aws_access_key_id'] = aws_access_key_id
        credentials[args.profile]['aws_secret_access_key'] = aws_secret_access_key
    
    if args.profile in credentials and args.show_expiration:
        s = session_expiration_diff(credentials[args.profile]['sts_aws_session_expiration'], datetime.utcnow())
        if s < 0:
            logger.info("Session token has expired")
        else:
            logger.info("Session token is expiring in %s seconds" % s)
        sys.exit(1)

    if args.profile not in credentials or session_expiration_diff(credentials[args.profile]['sts_aws_session_expiration'], datetime.utcnow())  < 10 or args.refresh:
        ...
        try:
            ...
            if args.token:
                ...
                my_token = raw_input("[%s] Please type in your MFA token: " % args.profile)
            ...
            credentials[args.profile]['sts_aws_access_key_id'] = aws_access_key_id
            credentials[args.profile]['sts_aws_secret_access_key'] = aws_secret_access_key
            credentials[args.profile]['sts_aws_session_token'] = aws_session_token
            credentials[args.profile]['sts_aws_session_expiration'] = aws_session_expiration
