...
def instantaneous_waiting_time(dataframe):
    '''
    Compute the instantaneous waiting time at each events in the dataframe.
    '''
    # Avoid side effect
    df = pd.DataFrame.copy(dataframe)
    df['starting_time'] = df['submission_time'] + df['waiting_time']
    ...
    wt_sum = df_sorted_by_starting_time.waiting_time.sum()
    wt_sum.name = "instantaneous_waiting_time"
    # Sort by starting time
    wt_sum.index = df_sorted_by_starting_time['starting_time']
