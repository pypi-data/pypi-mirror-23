...
def distance_pack(t0, matrix):
    res = [(*matrix[t0.index, i], i) for i in np.where(matrix['reltype'][t0.index, :] != b'none')[0]]
    res = sorted(res, key=lambda t: list(t[1]))

    return [(key, [t0.dictionary.index[t[3]] for t in v]) for key, v in groupby(res, key=lambda t: t[2])]
