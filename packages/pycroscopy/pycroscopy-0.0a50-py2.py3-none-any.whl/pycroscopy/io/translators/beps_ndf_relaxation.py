...
class BEPSndfRelaxationTranslator(Translator):
    """
    ...
    def translate(self, data_filepath, show_plots=True, save_plots=True, do_histogram=False, debug=False):
        ...
        if debug:
            print('BEndfRelaxationTranslator: Getting file paths')

        parm_filepath, udvs_filepath, parms_mat_path = self._parse_file_path(data_filepath)
        if debug:
            print('BEndfRelaxationTranslator: Reading Parms text file')

        isBEPS, self.parm_dict = parmsToDict(parm_filepath)
        self.parm_dict['data_type'] = 'BEPSData'
        expt_type = self.parm_dict['VS_mode']

        is_relax = expt_type == 'load user defined VS Wave from file'
        if not isBEPS and is_relax:
            warn('This is NOT a BEPS new-data-format relaxation dataset!')
            return None
        ...
        if debug:
            print('BEndfRelaxationTranslator: Preparing to read parms.mat file')
        self.BE_wave, self.BE_wave_rev, self.BE_bin_inds = self.__get_excit_wfm(parms_mat_path)

        if debug:
            print('BEndfRelaxationTranslator: About to read UDVS file')
        ...
        self.udvs_mat, self.udvs_labs, self.udvs_units = trimUDVS(self.udvs_mat, self.udvs_labs, self.udvs_units,
                                                                  ignored_plt_grps)
        if debug: print('BEndfRelaxationTranslator: Read UDVS file')
        ...
        if debug: print('BEndfRelaxationTranslator: Preparing to set up parsers')
