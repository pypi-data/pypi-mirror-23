...
def hook_settings(run_hook, get_agent_func=None, timer=None,
    ...
    # Figure out what to hook based on which Django version we're on
    if hasattr(django.conf, "BaseSettings"):
        # Django < 1.11
        hook_target = "BaseSettings"
    else:
        # Django >= 1.11
        hook_target = "LazySettings"

    # Hook the settings object to catch attribute changes
    orig_set_attr = getattr(django.conf, hook_target).__setattr__

    def new_set_attr(bound_self, name, value):
        log.debug(
            "django.conf.%(hook_target)s.__setattr__(%(name)s, %(value)s)", {
    ...
    setattr(getattr(django.conf, hook_target), "__setattr__", new_set_attr)
