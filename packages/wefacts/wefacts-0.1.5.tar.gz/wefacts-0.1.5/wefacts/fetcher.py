...
def fetch_raw_lite(year, station_id, dir_raw='%s/csv_raw/' % util.base_dir, retry=1):
    while retry > 0:
        try:
...
def fetch_raw_severe_weather(category, date_start, date_end):
    ...
    for year in xrange(date_start.year, date_end.year+1):
        if year < current_year:
            if not os.path.isfile('%s/raw/%s-%4d.csv' % (util.base_dir, category, year)):
                _fetch_raw_sw(category, year)
        elif year == current_year:
            m1 = 1 if date_start.year < current_year else date_start.month
            m2 = date_end.month
            for month in xrange(m1, m2+1):
                if month == m2 or not os.path.isfile('%s/raw/%s-%4d%2d.csv' % (util.base_dir, category, year, month)):
    ...
    while retry > 0 and len(file_names) > 0:
        try:
            ...
            for file_name in file_names:
                ftp.retrbinary('RETR %s.gz' % file_name, open('%s/raw/%s.gz' % (util.base_dir, file_name), 'wb').write)
            ...
            for f in file_names:
                if os.path.isfile('%s/raw/%s.gz' % (util.base_dir, f)):
                    os.remove('%s/raw/%s.gz' % (util.base_dir, f))

    # unzip the file
    for file_name in fetched:
        with gzip.open('%s/raw/%s.gz' % (util.base_dir, file_name), 'rb') as f:
            with open('%s/raw/%s' % (util.base_dir, file_name), 'w') as f2:
                file_content = f.read()
                f2.write(file_content)
                f2.close()
            f.close()
        os.remove('%s/raw/%s.gz' % (util.base_dir, file_name))
    return list(fetched)
