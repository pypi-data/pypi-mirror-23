...
def search_nearby_stations(
        ...
        search_option='usaf_wban', dir_raw='%s/csv_raw/' % util.base_dir):
    ...
    local_time_zone = tz.gettz(_get_time_zone(gps, date_start_local))
    date_end_utc = date_end_local.replace(tzinfo=local_time_zone).astimezone(tz.gettz('UTC'))

    station2location = searcher.search_stations(gps, country, state, date_end_utc,
                                                search_station_num, search_radius_miles,
                                                search_option, dir_raw)
    ...
    return station2location, gps


def download_raw_weather(stations, gps, date_start_local, date_end_local,
                         dir_raw='%s/csv_raw/' % util.base_dir):
    if isinstance(date_start_local, int):
        date_start_local = datetime.datetime.strptime(str(date_start_local), '%Y%m%d')
    if isinstance(date_end_local, int):
        date_end_local = datetime.datetime.strptime(str(date_end_local), '%Y%m%d')
    ...
    for station_usaf_wban in stations:
        for year in xrange(year_start, year_end+1):
            ...
            elif fetcher.fetch_raw_lite(year, station_usaf_wban, retry=2):
                station_year_list.append('%s-%d' % (station_usaf_wban, year))
            else:
...
def dump_weather_result(
        station2location, gps, address, date_start_local, date_end_local,
        dir_result='%s/csv_result/' % util.base_dir,
        severe_weather='plsr'):
...
def get_weather(address, date_start, date_end,
                dump_csv=False, result_dir='%s/result/' % util.base_dir,
                search_station_num=5, search_radius_miles=15, search_option='usaf_wban',
                severe_weather='plsr'):

    station2location, gps = search_nearby_stations(
...
def _get_weather_lite(date_start_utc, date_end_utc, station_usaf_wban, station_location='',
                      dir_raw='%s/csv_raw/' % util.base_dir,):
    date_end_utc += datetime.timedelta(hours=23)
    year_start, year_end = date_start_utc.year, date_end_utc.year
    df, stations = None, {}
...
def _get_weather_severe(severe_weather, date_start, date_end, gps):
...
def _get_time_zone(gps, dt):
    api_key = json.load(open('%slocal/accounts.json' % os.path.dirname(util.base_dir)))['google_map_api_key']
...
def test_pipeline(address, start_date_local=20161202, end_date_local=20170201):
    station2location, gps = search_nearby_stations(address, start_date_local, end_date_local)
    download_raw_weather(station2location.keys(), gps, start_date_local, end_date_local)
    station2csv, _ = dump_weather_result(station2location, gps, address, start_date_local, end_date_local)
