...
def process_social_login_and_get_token(request, username, email, **options):
	...
	if user is None:
		...
		ret, error = signup(request, params)
		if error:
...
def facebook_login_and_get_token(request, params):
	...
	username = email
	return process_social_login_and_get_token(request, username, email)
...
def google_plus_login_and_get_token(request, params):
	...
	try:
		r = requests.get("https://www.googleapis.com/plus/v1/people/me/", params = {'access_token': p["access_token"]})
	except Exception as e:
