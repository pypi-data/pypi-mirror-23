...
class Character:
    def __init__(self, char_id, **kwargs):
        self.id = char_id
        self.raw_animeography = kwargs.pop('animeography', None)
        self.raw_mangaography = kwargs.pop('mangaography', None)
        self.animeography = [Client().get_anime(parse_id(a.link)) for a in self.raw_animeography]
        self.mangaography = [Client().get_manga(parse_id(m.link)) for m in self.raw_mangaography]
