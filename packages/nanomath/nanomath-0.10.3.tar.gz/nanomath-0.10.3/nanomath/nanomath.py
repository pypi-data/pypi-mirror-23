...
def readstats(readlengths, qualities):
	...
	res["MaxLengthsAndQ"] = [zip(list(readlengths[indices_top_L]),  list(qualities[indices_top_L]))]
	indices_top_Q = np.argpartition(qualities, -5)[-5:]
	res["MaxQualsAndL"] = [zip(list(readlengths[indices_top_Q]),  list(qualities[indices_top_Q]))]
