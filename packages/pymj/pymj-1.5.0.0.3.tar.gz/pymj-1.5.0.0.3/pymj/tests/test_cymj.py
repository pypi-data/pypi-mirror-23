...
from pymj import (MjSim, MjSimPool, load_model_from_xml, load_model_from_mjb,
...
def test_multiprocess():
    '''
    Tests for importing pymj from multiple processes.
    '''
    # First enforce recompilation.
    path = os.path.dirname(os.path.abspath(__file__))
    cymj_path = os.path.join(path, "..", "cymj.c")
    if os.path.exists(cymj_path):
        print("Removing %s" % cymj_path)
        os.remove(cymj_path)
    generated = glob.glob(os.path.join(path, "..", "generated", "_pyxbld*"))
    for dirname in generated:
        print("Removing %s" % dirname)
        shutil.rmtree(dirname)
    generated = glob.glob(os.path.join(path, "..", "cythonlock_*.pyc"))
    for fname in generated:
        print("Removing %s" % fname)
        os.remove(fname)

    # Start processes without access to imported libraries.
