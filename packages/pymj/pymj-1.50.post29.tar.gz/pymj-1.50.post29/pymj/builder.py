...
The easy solution is to "import pymj" _before_ "import glfw".

More info: https://github.com/openai/pymj/issues/8
'''

def exit_handler(file):
...
def load_cython_ext(mjpro_path):
    ...
    if 'glfw' in sys.modules and \
       abspath(sys.modules["glfw"].__file__).find("mujoco") == -1:
        print(GLFW_WARNING)
    try:
        lockpath = join(dirname(__file__), 'cythonlock_%s.pyc' % (sys.platform))
        file = open(lockpath, 'a')
        atexit.register(exit_handler, file)
        max_attempts = 15
        for idx in range(max_attempts):
            try:
                portalocker.lock(file, portalocker.LOCK_EX | portalocker.LOCK_NB)
                break
            except (portalocker.exceptions.LockException, BlockingIOError):
                print("My pid = %s. Failed to acquire lock. Waiting." % os.getpid())
                import time
                time.sleep(10)
