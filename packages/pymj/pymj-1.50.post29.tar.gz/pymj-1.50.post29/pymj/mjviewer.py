...
from pymj.utils import rec_copy, rec_assign
...
class MjViewer(MjViewerBasic):
    ...
    def _create_full_overlay(self):
        ...
        self.add_overlay(const.GRID_TOPLEFT, "[R]eference frames", "Off" if self.vopt.frame == 1 else "On")
        ...
        if action != glfw.RELEASE:
            ...
            if not self._record_video and len(self._video_frames) > 0:
                ...
                process = Process(target=save_video,
                                 args=(frames, self._video_path % self._video_idx))
                process.start()
                self._video_frames = []
                self._video_idx += 1
