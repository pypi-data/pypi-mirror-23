...
class SISaModel(FSM):
    ...
    def neutral(self):
        ...
        # Infected
        discontent_neighbors = self.get_neighboring_agents(state_id=self.discontent)
        if random.random() < len(discontent_neighbors) * self.neutral_discontent_infected_prob:
            return self.discontent
        content_neighbors = self.get_neighboring_agents(state_id=self.content.id)
    ...
    def content(self):
        ...
        # Superinfected
        discontent_neighbors = self.get_neighboring_agents(state_id=self.discontent.id)
