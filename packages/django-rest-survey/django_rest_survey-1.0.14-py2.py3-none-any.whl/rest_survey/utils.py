...
def answer_question(survey,
    ...
    if question.other_token:
        if isinstance(value, basestring) and value.upper() == question.other_token.upper():
            create_other_answer(question, value)

        if isinstance(value, list): 
            upper_values = [v.upper() for v in value]
            if question.other_token.upper() in upper_values:
                create_other_answer(question, question.value.other_token)
