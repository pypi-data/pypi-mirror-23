...
cdef class mujoco_pyStatistic(object):
...
cdef mujoco_pyStatistic WrapMjStatistic(mjStatistic* p):
    cdef mujoco_pyStatistic o = mujoco_pyStatistic()
    o._set(p)
    return o

cdef class mujoco_pyVisual_rgba(object):
...
cdef mujoco_pyVisual_rgba WrapMjVisual_rgba(mjVisual_rgba* p):
    cdef mujoco_pyVisual_rgba o = mujoco_pyVisual_rgba()
    o._set(p)
    return o

cdef class mujoco_pyrContext(object):
    cdef mjrContext* ptr
...
cdef mujoco_pyrContext WrapMjrContext(mjrContext* p):
    cdef mujoco_pyrContext o = mujoco_pyrContext()
    o._set(p)
    return o

cdef class mujoco_pyVisual_scale(object):
...
cdef mujoco_pyVisual_scale WrapMjVisual_scale(mjVisual_scale* p):
    cdef mujoco_pyVisual_scale o = mujoco_pyVisual_scale()
    o._set(p)
    return o

cdef class mujoco_pyVisual_global_(object):
...
cdef mujoco_pyVisual_global_ WrapMjVisual_global_(mjVisual_global_* p):
    cdef mujoco_pyVisual_global_ o = mujoco_pyVisual_global_()
    o._set(p)
    return o

cdef class mujoco_pyTimerStat(object):
...
cdef mujoco_pyTimerStat WrapMjTimerStat(mjTimerStat* p):
    cdef mujoco_pyTimerStat o = mujoco_pyTimerStat()
    o._set(p)
    return o

cdef class mujoco_pyvGeom(object):
    cdef mjvGeom* ptr
...
cdef mujoco_pyvGeom WrapMjvGeom(mjvGeom* p):
    cdef mujoco_pyvGeom o = mujoco_pyvGeom()
    o._set(p)
    return o

cdef class mujoco_pyVisual_headlight(object):
...
cdef mujoco_pyVisual_headlight WrapMjVisual_headlight(mjVisual_headlight* p):
    cdef mujoco_pyVisual_headlight o = mujoco_pyVisual_headlight()
    o._set(p)
    return o

cdef class mujoco_pyContact(object):
    cdef mjContact* ptr
...
cdef mujoco_pyContact WrapMjContact(mjContact* p):
    cdef mujoco_pyContact o = mujoco_pyContact()
    o._set(p)
    return o

cdef class mujoco_pyrRect(object):
    cdef mjrRect* ptr
...
cdef mujoco_pyrRect WrapMjrRect(mjrRect* p):
    cdef mujoco_pyrRect o = mujoco_pyrRect()
    o._set(p)
    return o

cdef class mujoco_pyvLight(object):
    cdef mjvLight* ptr
...
cdef mujoco_pyvLight WrapMjvLight(mjvLight* p):
    cdef mujoco_pyvLight o = mujoco_pyvLight()
    o._set(p)
    return o

cdef class mujoco_pyvCamera(object):
    cdef mjvCamera* ptr
...
cdef mujoco_pyvCamera WrapMjvCamera(mjvCamera* p):
    cdef mujoco_pyvCamera o = mujoco_pyvCamera()
    o._set(p)
    return o
...
cdef class mujoco_pyVisual_quality(object):
...
cdef mujoco_pyVisual_quality WrapMjVisual_quality(mjVisual_quality* p):
    cdef mujoco_pyVisual_quality o = mujoco_pyVisual_quality()
    o._set(p)
    return o

cdef class mujoco_pyvFigure(object):
    cdef mjvFigure* ptr
...
cdef mujoco_pyvFigure WrapMjvFigure(mjvFigure* p):
    cdef mujoco_pyvFigure o = mujoco_pyvFigure()
    o._set(p)
    return o

cdef class mujoco_pyVFS(object):
    cdef mjVFS* ptr
...
cdef mujoco_pyVFS WrapMjVFS(mjVFS* p):
    cdef mujoco_pyVFS o = mujoco_pyVFS()
    o._set(p)
    return o

cdef class mujoco_pyOption(object):
    cdef mjOption* ptr
...
cdef mujoco_pyOption WrapMjOption(mjOption* p):
    cdef mujoco_pyOption o = mujoco_pyOption()
    o._set(p)
    return o

cdef class mujoco_pySolverStat(object):
...
cdef mujoco_pySolverStat WrapMjSolverStat(mjSolverStat* p):
    cdef mujoco_pySolverStat o = mujoco_pySolverStat()
    o._set(p)
    return o

cdef class mujoco_pyvOption(object):
    cdef mjvOption* ptr
...
cdef mujoco_pyvOption WrapMjvOption(mjvOption* p):
    cdef mujoco_pyvOption o = mujoco_pyvOption()
    o._set(p)
    return o

cdef class mujoco_pyvPerturb(object):
    cdef mjvPerturb* ptr
...
cdef mujoco_pyvPerturb WrapMjvPerturb(mjvPerturb* p):
    cdef mujoco_pyvPerturb o = mujoco_pyvPerturb()
    o._set(p)
    return o

cdef class mujoco_pyvScene(object):
    cdef mjvScene* ptr
...
cdef mujoco_pyvScene WrapMjvScene(mjvScene* p):
    cdef mujoco_pyvScene o = mujoco_pyvScene()
    o._set(p)
    return o

cdef class mujoco_pyWarningStat(object):
...
cdef mujoco_pyWarningStat WrapMjWarningStat(mjWarningStat* p):
    cdef mujoco_pyWarningStat o = mujoco_pyWarningStat()
    o._set(p)
    return o
...
cdef class PyMjModel(object):
    ...
    cdef mujoco_pyOption _opt
    cdef mujoco_pyVisual _vis
    cdef mujoco_pyStatistic _stat
...
cdef class mujoco_pyvGLCamera(object):
...
cdef mujoco_pyvGLCamera WrapMjvGLCamera(mjvGLCamera* p):
    cdef mujoco_pyvGLCamera o = mujoco_pyvGLCamera()
    o._set(p)
    return o

cdef class mujoco_pyVisual(object):
    cdef mjVisual* ptr


    cdef mujoco_pyVisual_global_ _global_
    cdef mujoco_pyVisual_quality _quality
    cdef mujoco_pyVisual_headlight _headlight
    cdef mujoco_pyVisual_map _map
    cdef mujoco_pyVisual_scale _scale
    cdef mujoco_pyVisual_rgba _rgba
...
cdef mujoco_pyVisual WrapMjVisual(mjVisual* p):
    cdef mujoco_pyVisual o = mujoco_pyVisual()
    o._set(p)
    return o

cdef class mujoco_pyVisual_map(object):
...
cdef mujoco_pyVisual_map WrapMjVisual_map(mjVisual_map* p):
    cdef mujoco_pyVisual_map o = mujoco_pyVisual_map()
    o._set(p)
    return o
...
def _mj_defaultVFS(mujoco_pyVFS vfs):
    mj_defaultVFS(vfs.ptr)

def _mj_addFileVFS(mujoco_pyVFS vfs, str directory, str filename):
    return mj_addFileVFS(vfs.ptr, directory.encode(), filename.encode())

def _mj_makeEmptyFileVFS(mujoco_pyVFS vfs, str filename, int filesize):
    return mj_makeEmptyFileVFS(vfs.ptr, filename.encode(), filesize)

def _mj_findFileVFS(mujoco_pyVFS vfs, str filename):
    return mj_findFileVFS(vfs.ptr, filename.encode())

def _mj_deleteFileVFS(mujoco_pyVFS vfs, str filename):
    return mj_deleteFileVFS(vfs.ptr, filename.encode())

def _mj_deleteVFS(mujoco_pyVFS vfs):
    mj_deleteVFS(vfs.ptr)

def _mj_loadXML(str filename, mujoco_pyVFS vfs, str error, int error_sz):
...
def _mj_addContact(PyMjModel m, PyMjData d, mujoco_pyContact con):
...
def _mjv_defaultCamera(mujoco_pyvCamera cam):
    mjv_defaultCamera(cam.ptr)

def _mjv_defaultPerturb(mujoco_pyvPerturb pert):
    mjv_defaultPerturb(pert.ptr)

def _mjv_room2model(np.ndarray[np.float64_t, mode="c", ndim=1] modelpos, np.ndarray[np.float64_t, mode="c", ndim=1] modelquat, np.ndarray[np.float64_t, mode="c", ndim=1] roompos, np.ndarray[np.float64_t, mode="c", ndim=1] roomquat, mujoco_pyvScene scn):
    mjv_room2model(&modelpos[0], &modelquat[0], &roompos[0], &roomquat[0], scn.ptr)

def _mjv_model2room(np.ndarray[np.float64_t, mode="c", ndim=1] roompos, np.ndarray[np.float64_t, mode="c", ndim=1] roomquat, np.ndarray[np.float64_t, mode="c", ndim=1] modelpos, np.ndarray[np.float64_t, mode="c", ndim=1] modelquat, mujoco_pyvScene scn):
    mjv_model2room(&roompos[0], &roomquat[0], &modelpos[0], &modelquat[0], scn.ptr)

def _mjv_cameraInModel(np.ndarray[np.float64_t, mode="c", ndim=1] headpos, np.ndarray[np.float64_t, mode="c", ndim=1] forward, np.ndarray[np.float64_t, mode="c", ndim=1] up, mujoco_pyvScene scn):
    mjv_cameraInModel(&headpos[0], &forward[0], &up[0], scn.ptr)

def _mjv_cameraInRoom(np.ndarray[np.float64_t, mode="c", ndim=1] headpos, np.ndarray[np.float64_t, mode="c", ndim=1] forward, np.ndarray[np.float64_t, mode="c", ndim=1] up, mujoco_pyvScene scn):
    mjv_cameraInRoom(&headpos[0], &forward[0], &up[0], scn.ptr)

def _mjv_frustumHeight(mujoco_pyvScene scn):
...
def _mjv_moveCamera(PyMjModel m, int action, float reldx, float reldy, mujoco_pyvScene scn, mujoco_pyvCamera cam):
    mjv_moveCamera(m.ptr, action, reldx, reldy, scn.ptr, cam.ptr)

def _mjv_movePerturb(PyMjModel m, PyMjData d, int action, float reldx, float reldy, mujoco_pyvScene scn, mujoco_pyvPerturb pert):
    mjv_movePerturb(m.ptr, d.ptr, action, reldx, reldy, scn.ptr, pert.ptr)

def _mjv_moveModel(PyMjModel m, int action, float reldx, float reldy, np.ndarray[np.float64_t, mode="c", ndim=1] roomup, mujoco_pyvScene scn):
    mjv_moveModel(m.ptr, action, reldx, reldy, &roomup[0], scn.ptr)

def _mjv_initPerturb(PyMjModel m, PyMjData d, mujoco_pyvScene scn, mujoco_pyvPerturb pert):
    mjv_initPerturb(m.ptr, d.ptr, scn.ptr, pert.ptr)

def _mjv_applyPerturbPose(PyMjModel m, PyMjData d, mujoco_pyvPerturb pert, int flg_paused):
    mjv_applyPerturbPose(m.ptr, d.ptr, pert.ptr, flg_paused)

def _mjv_applyPerturbForce(PyMjModel m, PyMjData d, mujoco_pyvPerturb pert):
    mjv_applyPerturbForce(m.ptr, d.ptr, pert.ptr)

def _mjv_select(PyMjModel m, PyMjData d, mujoco_pyvOption vopt, float aspectratio, float relx, float rely, mujoco_pyvScene scn, np.ndarray[np.float64_t, mode="c", ndim=1] selpnt):
    return mjv_select(m.ptr, d.ptr, vopt.ptr, aspectratio, relx, rely, scn.ptr, &selpnt[0])

def _mjv_defaultOption(mujoco_pyvOption opt):
    mjv_defaultOption(opt.ptr)

def _mjv_defaultFigure(mujoco_pyvFigure fig):
    mjv_defaultFigure(fig.ptr)

def _mjv_makeConnector(mujoco_pyvGeom geom, int type, float width, float a0, float a1, float a2, float b0, float b1, float b2):
    mjv_makeConnector(geom.ptr, type, width, a0, a1, a2, b0, b1, b2)

def _mjv_makeScene(mujoco_pyvScene scn, int maxgeom):
    mjv_makeScene(scn.ptr, maxgeom)

def _mjv_freeScene(mujoco_pyvScene scn):
    mjv_freeScene(scn.ptr)

def _mjv_updateScene(PyMjModel m, PyMjData d, mujoco_pyvOption opt, mujoco_pyvPerturb pert, mujoco_pyvCamera cam, int catmask, mujoco_pyvScene scn):
    mjv_updateScene(m.ptr, d.ptr, opt.ptr, pert.ptr, cam.ptr, catmask, scn.ptr)

def _mjv_addGeoms(PyMjModel m, PyMjData d, mujoco_pyvOption opt, mujoco_pyvPerturb pert, int catmask, mujoco_pyvScene scn):
    mjv_addGeoms(m.ptr, d.ptr, opt.ptr, pert.ptr, catmask, scn.ptr)

def _mjv_updateCamera(PyMjModel m, PyMjData d, mujoco_pyvCamera cam, mujoco_pyvScene scn):
    mjv_updateCamera(m.ptr, d.ptr, cam.ptr, scn.ptr)

def _mjr_defaultContext(mujoco_pyrContext con):
    mjr_defaultContext(con.ptr)

def _mjr_makeContext(PyMjModel m, mujoco_pyrContext con, int fontscale):
    mjr_makeContext(m.ptr, con.ptr, fontscale)

def _mjr_freeContext(mujoco_pyrContext con):
    mjr_freeContext(con.ptr)

def _mjr_uploadTexture(PyMjModel m, mujoco_pyrContext con, int texid):
    mjr_uploadTexture(m.ptr, con.ptr, texid)

def _mjr_uploadMesh(PyMjModel m, mujoco_pyrContext con, int meshid):
    mjr_uploadMesh(m.ptr, con.ptr, meshid)

def _mjr_uploadHField(PyMjModel m, mujoco_pyrContext con, int hfieldid):
    mjr_uploadHField(m.ptr, con.ptr, hfieldid)

def _mjr_setBuffer(int framebuffer, mujoco_pyrContext con):
    mjr_setBuffer(framebuffer, con.ptr)

def _mjr_blitBuffer(mujoco_pyrRect src, mujoco_pyrRect dst, int flg_color, int flg_depth, mujoco_pyrContext con):
    mjr_blitBuffer(src.ptr[0], dst.ptr[0], flg_color, flg_depth, con.ptr)

def _mjr_overlay(int font, int gridpos, mujoco_pyrRect viewport, str overlay, str overlay2, mujoco_pyrContext con):
    mjr_overlay(font, gridpos, viewport.ptr[0], overlay.encode(), overlay2.encode(), con.ptr)

def _mjr_figure(mujoco_pyrRect viewport, mujoco_pyvFigure fig, mujoco_pyrContext con):
    mjr_figure(viewport.ptr[0], fig.ptr, con.ptr)

def _mjr_render(mujoco_pyrRect viewport, mujoco_pyvScene scn, mujoco_pyrContext con):
