...
def conv(x, w, axis_order=None, conv_dim=None, x_shape=None, w_shape=None,
    ...
    border_mode: str
        ...
        * ``'same'``: Zero-pads image to generate ouput of the same shape as
          the input (only supported with uneven kernel sizes).
    stride: tuple
        | (tuple of len 2)
    ...
    if border_mode=='same':
        ...
        assert np.all(np.remainder(w_shape[-conv_dim:], 2)==1), '"same" mode requires uneven kernel sizes.'
        border_mode='half'
        crop_full = True
    else:
        crop_full = False
    ...
    if crop_full:  # TODO: This is never reached. Remove if it turns out to be completely unneeded.
        cropper = []
