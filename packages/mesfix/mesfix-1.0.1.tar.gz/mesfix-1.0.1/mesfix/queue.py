...
def end_auction(user_account_object, offer_object, auction_object, document_objects, 
    investor_entity_object, company_object, link_object):
    ...
    message = {
        ...
        'acceptor': 'Jacobo Sanin',
        'seller': 'Margarita Vallejo',
...
def signup(user_account_object, profile_object, entity_object, user_object):
...
class MesfixRabbitMQConnector(object):
    ...
    def message_publisher_factory(self, routing_key, **kargs):
        ...
        if routing_key == 'acceptContracts':
            message = accept_contracts(user_account_object, entity_object)
        ...
        if routing_key == 'ofacUser':
            message = ofac_user(entity_object)

        if routing_key == 'serviceError':
            message = service_error(error_object)

        if routing_key == 'missing-documents':
            message = missing_documents(user_account_object, entity_object, company_object, document_objects)
        ...
        if routing_key == 'confirmed-payment-seller':
            message = confirmed_payment_seller(user_account_object, entity_object, company_object, auction_object)

        if routing_key == 'confirmed-payment': 
            message = confirmed_payment(user_account_object, entity_object, company_object, auction_object)

        if routing_key == 'update-bid': 
            message = update_bid(user_account_object, entity_object, company_object, auction_object)

        if routing_key == 'auction-bid': 
            message = auction_bid(user_account_object, entity_object, company_object, auction_object)

        if routing_key == 'investment-certificate':
            message = investment_certificate(user_account_object, entity_object, company_object, link_object, string_object)

        if routing_key == 'end-auction':
            message = end_auction(user_account_object, offer_object, auction_object, document_objects, 
                investor_entity_object, company_object, link_object)
        ...
        if routing_key == 'signup':
            message = signup(user_account_object, profile_object, entity_object, user_object)
