...
def urlencode_sorted(params_dict, quote=False):
	if quote:
		return urlencode({k: params_dict[k] for k in sorted(params_dict)})
	return urlencode({k: params_dict[k] for k in sorted(params_dict)}, quote_via=lambda s, *args: s)
...
class Scrapper:
	...
	def save_url(self, path, params=None, data=None, overwrite=False, sep='==', save=True):
		''' sep: concaternator path and querysting for save filename
		'''
		url = self.root + path
		# print(params)
		tokken = [self.save_root+path]
		tokken.append(urlencode_sorted(data or params or {}) or '')
		_file = sep.join(tokken)
		
		if save and os.path.exists(_file) and not overwrite:
			return 
	...
	def save_urls_with_params(self, src_path, overwrite=False, sep='==', save=True, **params_set):
		...
		for param in self._product_params(**params_set):
			args = src_path, param, None, overwrite, sep, save
			args_list.append(args)

		content_list = []
		with ThreadPoolExecutor(min(len(args_list), self.max_worker)) as executor:
			todo_list = []
