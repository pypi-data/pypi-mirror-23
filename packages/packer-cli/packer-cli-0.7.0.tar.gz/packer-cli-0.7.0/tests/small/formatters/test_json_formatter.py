...
class TestJSONFormatter(unittest.TestCase):
    ...
    def test_indent(self):
        """
        Tests that custom indentation can be specified
        """

        data = {'key': 'value'}
        io_stream = self._prepare_stream(data)

        formatter = packer.formatters.JSONFormatter(io_stream, indent=2)

        self.assertEqual(
            json.dumps(data, indent=2),
            formatter.format_pretty(),
            'Data was pretty formatted with the right indentation',
        )
