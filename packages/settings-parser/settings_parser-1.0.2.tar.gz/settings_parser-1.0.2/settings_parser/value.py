...
class DictValue():
        ...
        if isinstance(values, Dict):
            ...
            for key, value in values.items():
                if isinstance(value, DictValue):
                    self.values_list.append(NamedValue(key, value, kind=value.kind))
    ...
    def validate(self, config_dict: Dict) -> Dict:
        ...
        for val in self.values_list:
            ...
            if val.kind is not Value.mandatory and (val.key not in config_dict or config_dict[val.key] is None):
                continue
