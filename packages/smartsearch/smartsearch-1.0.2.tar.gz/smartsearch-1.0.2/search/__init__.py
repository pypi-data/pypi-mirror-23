...
# Internal imports
from search import exceptions, clients
...
class Searcher:
    ...
    def __init__(self, keys=None, conf_file=None):
        ...
        print ("Have keys {}".format(self.keys))
        for client_name, client_baseclass in _client_classes.items():
            print(client_name, client_baseclass, client_baseclass.key_required)
            if client_baseclass.key_required:
                ...
                if client_name in self.keys.keys():
                    ...
                    print("Passing {0} to {1}".format(self.keys[client_name], client_name))
