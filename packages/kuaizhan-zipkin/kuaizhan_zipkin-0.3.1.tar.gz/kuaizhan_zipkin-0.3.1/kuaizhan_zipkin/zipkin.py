...
def long_to_hex(num):
    return format(num, 'x').zfill(8)


def add_result_to_binary_annotation(zipkin_context, result):
    try:
        if result.__dict__:
            zipkin_context.update_binary_annotations({"result": json.dumps(result.__dict__)})
        else:
            zipkin_context.update_binary_annotations({"result": json.dumps(result)})
    except TypeError:
        pass
