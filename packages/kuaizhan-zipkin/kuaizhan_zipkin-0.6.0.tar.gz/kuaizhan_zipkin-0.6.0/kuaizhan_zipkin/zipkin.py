...
def init_service(name, bootstrap_servers, port=None, topic_name="kuaizhan_zipkin", log_span_enabled=False, retries=3):
    ...
    producer = KafkaProducer(bootstrap_servers=bootstrap_servers, retries=retries, compression_type="gzip",
                             value_serializer=lambda m: json.dumps(m, default=lambda x:
                             dict((k, v) for k, v in x.__dict__.iteritems() if v is not None)))
    global topic
    topic = topic_name
    global log_span
...
def add_result_to_binary_annotation(zipkin_context, url, result):
    try:
        zipkin_context.update_binary_annotations({"url": url})
...
def transport_handler(message):
    ...
    for annotation in span.get('annotations', []):
        annotation['endpoint'] = dict((k, v) for k, v in annotation.pop('host').iteritems() if v is not None)
    for binary_annotation in span.get('binary_annotations', []):
        binary_annotation['endpoint'] = dict(
            (k, v) for k, v in binary_annotation.pop('host').iteritems() if v is not None)
    span = dict((k, v) for k, v in span.iteritems() if v is not None)
    ...
    def wapper(handler, *args, **kw):
        ...
        if existing_zipkin_attrs:
            with zipkin_span(
                ...
                add_result_to_binary_annotation(zipkin_context, handler.request.uri, result)
                return result
        else:
            with zipkin_span(
                    service_name=service_name,
    ...
    def wrapper(*args, **kwargs):
        ...
        if existing_zipkin_attrs:
            with zipkin_span(
                ...
                add_result_to_binary_annotation(zipkin_context, request.url, result)
                return result
        else:
            with zipkin_span(
                    service_name=service_name,
...
def none_root_span(func):
    ...
    def wapper(*args, **kw):
        with zipkin_span(
            ...
            result = func(*args, **kw)
            add_result_to_binary_annotation(zipkin_context, result)
            return result
