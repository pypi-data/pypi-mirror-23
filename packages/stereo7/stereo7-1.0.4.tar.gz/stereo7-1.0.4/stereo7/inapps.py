...
class Inapp(object):
    ...
    def validate(self):
        ...
        if self.tier == 0:
            return False, 'tier have to be 1+'
...
def parse_google_doc(app, google_doc_id):
    ...
    for row in inapps:
        ...
        iap.tier = int(get_param('Price (tier)', header, row))
...
<package xmlns="http://apple.com/itunes/importer" version="software5.2">
    ...
    inapps_str = ''
    for i in app.inapps:
        image = fileutils.root_dir + '/proj.ios/inapps.itmsp/{}'.format(i.screenshot)
    ...
    fileutils.write(fileutils.root_dir + '/proj.ios/inapps.itmsp/machine-local-data.xml', local_data)
    fileutils.write(fileutils.root_dir + '/proj.ios/inapps.itmsp/metadata.xml', metadata)
...
def create_csv_android(app):
    ...
    for i in app.inapps:
        rate = 60 * 1000 * 1000
        diff = 1000 * 1000
        inapps_str = pattern.format(app.package, i.id, i.tier * rate - diff, i.localisations[0].title, i.localisations[0].description)
        csv += '\n' + inapps_str
    fileutils.write(fileutils.root_dir + '/proj.android/android_inapps.csv', csv)


def run(app_package='com.stereo7games.syndicate3',
        app_name='Syndicate III',
        app_version='1.0',
        google_spreedsheet_id='1WvxSOHiH-AnoMhPek-mbIqkVgwboGYKrecjueGvT5jE'
        ):
