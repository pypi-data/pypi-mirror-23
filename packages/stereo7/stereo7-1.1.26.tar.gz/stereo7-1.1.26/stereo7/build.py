...
'''
    ...
    def build(self, configuration):
        ...
        if not self.arg_parser.upload:
            result = result and 0 == os.system(PlatformIos.upload_shell)
        else:
            print 'Missing upload build to iTunesConnect'
    ...
    def set_version(self, packagename, app_version, build_version):
        ...
        version = '{}.{}'.format(app_version, build_version)
        result = True
...
def run(package_name, app_version, arg_parser):
    ...
    return build.set_version(package_name, app_version, build_version) and \
