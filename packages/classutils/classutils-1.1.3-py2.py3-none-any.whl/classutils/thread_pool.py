...
class ThreadPool(object):
    ...
    def create(self):
        ...
        if self._pool is None:
            logging.info(u'Creating thread pool ({uuid}) with {n} threads...'.format(uuid=self.id,
                                                                                     n=self.num_threads))
            ...
            logging.warning(u'Not creating pool, there is already an active instance!')
    ...
    def destroy(self):
        ...
        logging.info(u'Destroying thread pool ({uuid}), '
        ...
        logging.info(u'Thread pool ({uuid}) destroyed!'.format(uuid=self.id))
