...
from localstack.utils.common import load_file, save_file
...
def fetch_key():
    ...
    try:
        ...
        result = requests.post('%sactivate' % API_URL, json.dumps(data))
    finally:
        logging.getLogger('py.warnings').setLevel(logging.WARNING)
    key_base64 = json.loads(result.content)['key']
    key = AES.new(str(base64.b64decode(key_base64)), AES.MODE_CBC, '\0' * 16)
    return key
