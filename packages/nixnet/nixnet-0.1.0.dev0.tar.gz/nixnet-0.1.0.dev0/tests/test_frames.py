...
@pytest.fixture
def nixnet_interface(request):
    interface = request.config.getoption("--nixnet-interface")
    return interface
...
def test_stream_loopback(nixnet_interface):
    ...
    with nixnet.FrameInStreamSession(nixnet_interface) as input_session:
        with nixnet.FrameOutStreamSession(nixnet_interface) as output_session:
            ...
            expected_frame = types.CanFrame(0, False, constants.FrameType.CAN_DATA, bytearray(payload_list))
