...
class Main:
    ...
    @staticmethod
    def predict(pred_set, name, img_conf, count_conf, maybe_conf):
        Main.__setup()
        print ('Loading Model')
        model = NeuralNetwork("load", file_path=os.path.join(MODELS_DIR, name))
        print ('Done Loading Model')
        ...
        Predictor.predict_wells(os.path.join(PRED_DIR,pred_set), 
                    [Constants.ABNORMAL,
                        Constants.NORMAL,
                        Constants.WEIRD],
                    model, img_conf, count_conf, maybe_conf)

    @staticmethod
    def test_wells(test_set, name,  img_conf, count_conf, maybe_conf):
        Main.__setup()
        print ('Loading Model')
        model = NeuralNetwork("load", file_path=os.path.join(MODELS_DIR,name))
        print ('Done Loading Model')
