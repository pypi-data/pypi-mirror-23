...
class MacacaWorker(object):
    def __init__(self, macaca_dir, data):
        self.macaca_task = json.load(data)
        self.macaca_type = self.__post_get('macaca_type', self.macaca_task)
        self.macaca_dir = macaca_dir
        self.app_name = self.__post_get('app_name', self.macaca_task)
        self.version_name = self.__post_get('version_name', self.macaca_task)
        self.module_name = self.__post_get('module_name', self.macaca_task)
        self.case_name = self.__post_get('case_name', self.macaca_task)
        self.case_url = self.__post_get('case_url', self.macaca_task)
