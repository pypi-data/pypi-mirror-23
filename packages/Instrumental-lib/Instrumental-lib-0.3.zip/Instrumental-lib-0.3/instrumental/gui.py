...
class CroppableCameraView(QGraphicsView):
    ...
    def __init__(self, camera, **settings):
        ...
        self.imageSet.connect(self._set_image)
    ...
    def set_image(self, image_array):
        """Thread-safe way to set current image from an array"""
        self.imageSet.emit(image_array)

    def _set_image(self, image_array):
        """Directly set the image. Should only be called from the GUI thread"""
        if self.persist and self.disp_array is not None:
            p = self.persist_factor
            q = 1. - p
            self.disp_array = np.asarray(self.disp_array * p + image_array * q, dtype=np.uint16)
        else:
            self.disp_array = image_array

        # Save underlying data
        self.latest_array = image_array

        pixmap = QPixmap(self._array_to_qimage(self.disp_array))
        ...
        self.imageDisplayed.emit(image_array)
    ...
    def _wait_for_frame(self):
        frame_ready = self.cam.wait_for_frame(timeout=0)
        if frame_ready:
