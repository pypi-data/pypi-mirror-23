...
from fundamentals import fft, ccfnpl
import gc
...
from statistics import fsc, ccc
from utilities import model_circle, model_blank, model_gauss_noise
...
#         Fnew_amp = self.randomize_quant_beyond_circle(F_amp, circ_np, amp=True)
    ...
    def generate_refinement_grid_to_be_evaluated_fixed_inc(self, helical_symmetry, grid_count=25):
        ...
        array([[(10.0, 50.0), (9.98613037447989, 49.930651872399444)],
               [(10.013888888888888, 50.0), (10.0, 49.930651872399444)]], dtype=object)
        ...
        sym_comb_grid = SegClassReconstruct().generate_rise_rotation_or_pitch_unitnumber_pairs_for_symmetry_grid(
            [pitch - int((pitch_count - 1 ) / 2.0) * pitch_inc, pitch + round((pitch_count - 1)/ 2.0) * pitch_inc], pitch_inc, 
            [no_unit - int((nut_count - 1) / 2.0) * no_unit_inc, no_unit + round((nut_count - 1) / 2.0) * no_unit_inc], no_unit_inc)
        sym_comb_rise_grid = SegClassReconstruct().convert_pitch_unit_grid_to_rise_rotation_grid(sym_comb_grid)
