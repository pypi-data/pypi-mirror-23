...
def watch(config):
    ...
    while True:
        ...
        if events:
            ...
            click.echo("Failed to query recent events...")
            return None
...
def create_project(config,
    ...
    if link:
        ...
        if projects:
            ...
            click.echo(
                info("You don't have any project to link to this repository."))
    else:
        valid = False
        if name is None:
        ...
        six.print_("Creating project (it may take up to a few minutes) %s " %
                   project_name, end='')
    ...
    try:
        ...
        click.echo(info(
            "This repository already has a tensorport remote. You can delete the remote using 'git remote remove tensorport'"))
        return
...
def create_dataset(config,
    ...
    if link:
        ...
        if datasets:
            ...
            click.echo(
                info("You don't have any dataset to link to this repository."))
    else:
        valid = False
        if name is None:
        ...
        six.print_("Creating dataset (it may take up to a few minutes) %s " %
                   dataset_name, end='')
    ...
    click.echo(info("Dataset %s/%s is created.\nMatrix: %s\n" % (username,
                                                                 dataset_name,
...
def get_job(config, job_id=None):
    ...
    if job_id is None:
        ...
        if jobs:
            job_id, job_name = select_job(jobs)
        else:
            click.echo(
                info(
        ...
        try:
            ...
            if project:
                ...
                if projects:
                    ...
                    click.echo(info("No project to delete."))
                    return
        except:
    ...
    try:
        ...
        try:
            ...
            if dataset:
                ...
                click.echo(info("Droject %s/%s deleted" %
                ...
                if datasets:
                    ...
                    click.echo(info("No dataset to delete."))
                    return
...
def get_datasets(config, owner=None):
    ...
    if datasets:
        ...
        click.echo("Failed to query owned datasets...")
        return None
...
def _create_job(config,
    ...
    if not project:
        ...
        if projects:
            ...
            click.echo(
                info(
                    "You don't have any projects! You can create one using 'tport create project'."))
            return None
    else:
        try:
            ...
            if not project:
                click.echo(
                    info("Can not find %s/%s" % (username, project_name)))
                return None
            if commit:
            ...
            click.echo(info("Can not find %s/%s" % (username, project_name)))
            return None
    ...
    if commit is None:
        try:
            ...
            commit_to_use = valid_commits[commit_id]
        except Exception as e:
    ...
    if (not training_mode) or (not training_mode in training_modes):
    ...
    if training_mode == 'single-node':
        ...
        if (worker_type is None) or (not worker_type in [it.get('type') for it in valid_it]):
            ...
            click.echo(
                info("%s selected for workers' instance type" %
                     worker_instance_type))

        if ps_replicas is None or ps_replicas < 1:
            # Specify number of workers
            ps_replicas = click.prompt(
                text=question('Select number of paramter servers'), default=1)
        ...
        if (ps_type is None) or (not ps_type in [it.get('type') for it in valid_it]):
            ...
            click.echo(
                info("%s selected for parameter servers' instance type" %
                     ps_instance_type))
    ...
    parameters = {
        ...
        "datasets": datasets,
        "code": project.get('id'),
        "code_commit": commit_to_use.get('id')
    }
...
@click.option(
    '--worker-replicas',
    help='Nubmer of workers (for distributed training only)',
    type=int)
@click.option(
    '--worker-type',
...
@click.option(
    '--description', help='Job description [optional]', required=False)
@click.option('--current_env', is_flag=True, default=False, help='Use current enviroment when running locally.')
@click_log.simple_verbosity_option()
@click_log.init(__name__)
@pass_config
def run(config,
    ...
    try:
        ...
        if not job_id:
            ...
            if jobs:
                ...
                click.echo(
                    info(
                        "You do not have any jobs to start. Use 'tport create job' or 'tport run' to run a job."
                    ))
                return
        else:
            try:
    ...
    try:
        ...
        if not job_id:
            ...
            if jobs:
                ...
                click.echo(
                    info(
                        "You do not have any running jobs. Use 'tport run' to run a job."
                    ))
                return
        else:
            job = client.get_job(params={'job_id': job_id})
