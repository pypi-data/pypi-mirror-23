...
def get_data_path(dataset_name, local_root, local_repo, path):
    """
    Depending on the environemt (tensorport cloud vs local) update path to data.
    Args:
...
def get_logs_path(root):
    """
    Depending on the environemt (tensorport cloud vs local) update path to logs.
    Args:
...
class TensorportClient(object):
    ...
    def open_dashboard(self):
        """
        Opens a Dashboard in a browser
        :return:
        """
        try:
            ...
            print("Matrix works best in Chrome, consider installing it.")
    ...
    def get_projects(self, owner=None, reader=None, writer=None):
        """
        ...
        try:
            response = self.client_action(
                ['projects', 'owned', 'list'], params={}, validate=False)
            return response
        except Exception as e:
            print("Couldn't fetch a project, error: %s" % str(e))
            raven_client.captureException()
