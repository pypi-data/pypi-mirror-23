...
class SshManager(object):
    ...
    def uploadDir(self, scpClient, local_dir, remote_dir):
        """
        """
        LOGGER.debug("try to upload from %s to  %s", local_dir, remote_dir )
        
        for filename in local_dir:
            localfilePath = os.path.join(local_dir, filename)
            LOGGER.debug("try to upload %s", localfilePath)
            if os.path.isdir(localfilePath):
                # uses '/' path delimiter for remote server
                self.uploadDir(scpClient, localfilePath + '/', os.path.join(remote_dir, filename))
            else:
                LOGGER.debug("try to upload %s", localfilePath)
                if os.path.isfile(localfilePath):
                    scpClient.put(localfilePath, os.path.join(remote_dir, filename))
                        
    def scpPutDir(self, localDir, host, remoteDir, user=None):

        """
        upload localDir to a remoteDir 

        :param str host: IP address of the host
        :param str localDir: the full path in local to store the file,
        :param str user: the user to login on remote. default is 'root'
        ...
        if not localDir:
            raise SshManagerError("localdir %s not exist", localDir)         
        userSsh = 'root' if user is None else user
        client = self.getClient(host, user=userSsh)
        scpClient = SCPClient(client.get_transport())
#         scpClient.socket_timeout = 1000.0
        try:
            #todo:check
            scpClient.stat(remoteDir)
        except BaseException:
            LOGGER.error("remotedir %s not exist", remoteDir)
            raise SshManagerError("remote %s not exist", remoteDir)

        self.uploadDir(scpClient, localDir, remote_dir)
        return remoteDir
