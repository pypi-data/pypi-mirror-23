        ...
        if labpar['SPA'] is not None and labpar['SPA'] != "" :
            ...
            for spa in labpar['SPA'].split(","):
                ...
                proc=subprocess.Popen([os.path.dirname(os.path.abspath(__file__))+'/resources/copy.sftp',
                                       self.variablePlugin.get('user','Build:USER:xxx'),
                                       self.variablePlugin.get('pass','Build:USER:xxx'),
                                       self.variablePlugin.get('buildServer','Build:USER:xxx'),
                                       'download',
                                       self.variablePlugin.get('path','Build:USER:xxx')+"/"+spa+'.full.tar',
                                       os.getcwd()+'/install/'+self.openstack['username']], 
        ...
        if vmpar['CustomScript'] == 'heatinstall1':
            ...
            with open(os.getcwd()+'/install/'+self.openstack['username']+'/siteinfo','a') as outfile:
                for k,v in siteinfo.items():
                    outfile.write(k+'='+v+'\n')
            location=self.variablePlugin.get('serverType','Build:USER:xxx')
            for pkg in vmpar['Package'].split(","):
                if location=='Local':
                    proc=subprocess.Popen([os.path.dirname(os.path.abspath(__file__))+'/resources/copy.sftp',
                                           ...
                                           self.variablePlugin.get('buildServer','Build:USER:xxx'),
                                           'download',
                                           self.variablePlugin.get('path','Build:USER:xxx')+"/"+pkg,
                                           os.getcwd()+'/install/'+self.openstack['username']], 
                                           stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
                elif location=='NEXUS':
