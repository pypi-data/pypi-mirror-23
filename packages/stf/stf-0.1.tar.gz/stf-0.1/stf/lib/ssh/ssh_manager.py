...
from stf.lib.common.multi_tasks_manager import MultiTasksManager
...
class SshManager(object):
    ...
    def scpGetDir(self, host, source, user=None, localDir=None):
        """
        get a remote file dir and copy in localDir(default is cache Dir)
    ...
    def downloadDir(self, sftp_client, remote_dir, local_dir):
        """
        """
        LOGGER.debug("try to copy %s to local %s", remote_dir, local_dir)
        ...
        for filename in sftp_client.listdir(remote_dir):
            ...
            if stat.S_ISDIR(sftp_client.stat(filePath).st_mode):
                ...
                self.downloadDir(sftp_client, filePath + '/', os.path.join(local_dir, filename))
            else:
                LOGGER.debug("try to copy %s", filePath)
                if os.path.isfile(filePath):
                    sftp_client.get(filePath, os.path.join(local_dir, filename))
        ...
        try:
            ...
            if useKey:
                ...
                try:
                    ...
                    LOGGER.trace(' No access, no SSH key already installed on ' + host + \
                                 ' with user ' + finalUser)
                    LOGGER.trace(' Now trying SSH access via user + password')
                    useKey = False
                except Exception as err:
                    msg = "Cannot connect to " + host + " as " + finalUser
                    LOGGER.error(msg)
    ...
    def run(self, host, cmd, user=None, ddmStation=None, ignoreStdStreams=False, bladeRcsHostname=None,
        ...
        :param str user: login
        :param str ddmStation: ddm blade letter ("A", "B", "C", etc.) target if any,
                               if None target is active pilot
        ...
        >>> sshManager.run('hp31oam',ddmStation='C',cmd)   #executes cmd on station C
        ...
        if ddmStation:
            ...
            command = "ssh -q STATION_" + ddmStation + " \"" + command + "\""
        if bladeRcsHostname:
    ...
    def multiRun(self, hostList, cmd, user=None, bladeList=None, timeout=SSH_CMD_TIMEOUT, env=None):
        """
        Executes a command via paramiko exec_command on a list of host (maybe also on a list of blade of the host).

        :param List host: a list of IP address of the target lab
        :param str cmd: the command to execute
        :param str user: login
        :param list bladeRcsHostname: list of mcas rack-chassis-slot blade hostname, eg:0-0-2,
        :param int timeout: in minutes. Interrupt the function if timeout reached, default value is 20 min.
        :return: the tuple (exit_status, stdout) stdout is a unique string with all EOL except trailing EOL
        :rtype: tuple

        >>> sshManager.multiRun(['hp31oam', 'hp32oam'],cmd)              #executes cmd on pilot active
        >>> sshManager.multiRun(['hp31oam', 'hp32oam'],cmd, bladeList=['0-0-2', '0-0-3'])
        """
        multiTasksManager = MultiTasksManager()
        for host in hostList:
            if bladeList:
                for blade in bladeList:
                    multiTasksManager.register(self.run, host, cmd, user, ignoreStdStreams=False, bladeRcsHostname=blade, timeout=timeout)
            else:
                multiTasksManager.register(self.run, host, cmd, user, ignoreStdStreams=False, timeout=timeout, env=env)
        return multiTasksManager.runMultiTasks(MultiTasksManager.TASK_RETURN_VALUES)
