...
class Manager(object):
    ...
    def getInstance(self, name):
        ...
        logger.info('try to find %s [%s] from directory [%s]' %(self.kind, name, self.factory_folder) )
        ...
        if not os.path.isfile(file_path):
            logger.info('try to find %s [%s] from %s' %(self.kind, name, [os.path.join(x, '%s/%s' %(external_module_name,file_name)) for x in sys.path]) )
            try:
                file_ins = __import__('%s.%s' % (external_module_name, external_module_name), globals(), locals(),[external_module_name], 0)
            except ImportError:
                try:
                    logger.info('Not found')
                    logger.info('try to find %s [%s] from %s' % (self.kind, name, [os.path.join(x, file_name) for x in sys.path]))
            ...
            try:
                logger.info('try to find %s [%s] from %s' %(self.kind, name, file_path))
