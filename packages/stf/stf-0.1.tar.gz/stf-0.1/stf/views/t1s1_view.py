...
class STFT1S1View(STFBaseView):
    def __init__(self, plugins):
        super(STFT1S1View, self).__init__(plugins)
    ...
    def _findTestWithSteps(self, root, d, test_suite, direct=False):
        ...
        logger.info('found test case: %s (tags: %s, tms ids: %s)' % (path, tags, tms_ids))
        if tags:
        ...
        test_id = 'TESTCASE-%s-%s' % (str(self.global_test_id), d)
    ...
    def _findTestWithoutSteps(self, root, f, test_suite, direct=False):
        ...
        logger.info('found test case: %s (module: %s, tags: %s, tms ids: %s)' % (path, module, tags, tms_ids))
        if tms_ids:
            tms_ids = tms_ids.split('__', 1)[1].split('~')
        ...
        test_ins = TestCase(path, f, tags, tms_ids, 'TESTCASE-%s-%s' % (str(self.global_test_id), f))
