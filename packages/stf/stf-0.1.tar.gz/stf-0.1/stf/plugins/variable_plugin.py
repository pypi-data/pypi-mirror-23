...
class STFVariablePlugin(STFBasePlugin):
    ...
    def getBuild(self, option):
        self.audit()
        return self.get(option, 'Build')

    def getBuildFile(self):
        self.audit()
        fd,path = tempfile.mkstemp()
        self.parser.dumpSectionToFile('Build:MCAS', path)
        return path
    ...
    def getTestValueAsList(self, option):
        ...
        if l is None:
            logger.warning("%s list is %s", option, l)
            return None
    ...
    def getLabInfo(self, lab, account):
        lab_list = self.getTestValueAsList(lab)
        logger.debug("%s list is %s", lab, lab_list)
        if lab_list is None:
            logger.warning("lab list is %s", lab_list)
            return None
        ...
        try:
            for lab in lab_list:
                ...
                for k in lab_ins.getHosts():
                    ...
                    if ins is None:
                        continue
                    lab_info.setAccount(ins.getVV(account))
                    lab_info.setAccount(ins.getVV(account + '_become'), True)
