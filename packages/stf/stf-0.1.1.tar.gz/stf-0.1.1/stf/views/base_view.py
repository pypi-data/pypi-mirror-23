...
class STFBaseView(object):
    ...
    def prepareCaseSourceList(self):
        ...
        if l is None:
            logger.warning("You should define 'source' in [Test] section in %s" % self.variables.ini_file)
            #dir = os.path.abspath(os.path.join(self.variables.ini_file, os.pardir))
            #self.case_dir_list.append(dir)
