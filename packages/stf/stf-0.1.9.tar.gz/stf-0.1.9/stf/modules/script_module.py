...
class STFScriptModule(STFBaseModule):
    ...
    def checkModuleParameter(self, module, jumphost_info, lab_info):
        ...
        if lab_info:
            if not jumphost_info:
                labs = self.sshPlugin.getLabList(lab_info)
                if not labs:
                    logger.error("there is no such lab %s", lab_info)
                    raise STFScriptModuleError("there is no such labs %s", lab_info)
    ...
    def run(self, caseInfo):
        ...
        if not lab_info:
            ...
            if not mode:
                self.sshPlugin.getAllLabClinet(lab_info)
