...
def noisy_bagging(X,Y, samples):
    ...
    mean_var = [(np.mean(X[:,i]),np.var(X[:,i])) for i in range(m)]
    
    noise = np.zeros(shape=(n,m))
    ...
    for i in range(samples):
        for j in range(m):
            mu, sigma = mean_var[j]
            noise[:,j] = np.random.normal(mu,sigma,n)
            
        indices = np.array(range(len(Y)))
        sample = np.random.choice(indices,len(indices))
            
        samples.append([X[sample]+noise, Y[sample]])
