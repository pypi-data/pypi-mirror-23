...
class TestAPI:
    ...
    def test_status(self):
        ...
        self.status.reply(str(time.time()))
        self.status.reply(str(time.time()), photo=io.BytesIO(base64.decodebytes(PHOTO)))
        self.status.repost(str(time.time()))
        self.status.repost(str(time.time()), photo=io.BytesIO(base64.decodebytes(PHOTO)))
