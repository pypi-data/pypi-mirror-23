...
def load_classifier():
    ...
    with warnings.catch_warnings():
        if CLASSIFIER == None:
            d = joblib.load(pkg_resources.resource_filename(__name__,'data/site_clf.sk'))
...
def predict_probability(text, desc=True):
    ...
    out_s = sorted(out.items(), key=lambda x: x[1],  reverse=True)
    ans = [{LABEL_ENCODER.inverse_transform(x[0]):x[1]} for x in out_s]
    ans = [[{'naics_6':x,'naics_5':x[0:5],'naics_4':x[0:4],'naics_3':x[0:3],'naics_2':x[0:2],'value':y} for x,y in z.iteritems()][0] for z in ans]
    df = pd.DataFrame(ans)

    naics_2_prob = df.groupby('naics_2').sum().sort_values(['value'])
    naics_3_prob = df.groupby('naics_3').sum().sort_values(['value'])
    naics_4_prob = df.groupby('naics_4').sum().sort_values(['value'])
    naics_5_prob = df.groupby('naics_5').sum().sort_values(['value'])
    naics_6_prob = df.groupby('naics_6').sum().sort_values(['value'])

    out_records = df.sort_values('value',ascending=False).iloc[0:3][['naics_2','naics_3','naics_4','naics_5','naics_6']].to_dict('records')
    _ = [x.update({
        'naics_2_prob':naics_2_prob.ix[x['naics_2']].value,
        'naics_3_prob':naics_3_prob.ix[x['naics_3']].value,
        'naics_4_prob':naics_4_prob.ix[x['naics_4']].value,
        'naics_5_prob':naics_5_prob.ix[x['naics_5']].value,
        'naics_6_prob':naics_6_prob.ix[x['naics_6']].value,
