...
DATA_CLASSIFIER = pkg_resources.resource_filename(__name__,'data/site_clf.sk')
DATA_NAICS_MAP = pkg_resources.resource_filename(__name__,'data/naics_codes_2012.csv')
FILE_STOP_WORDS = pkg_resources.resource_filename(__name__,'data/stop_words.txt')

stop_words = open(FILE_STOP_WORDS).read().splitlines()
...
df_map = [x for x in csv.DictReader(open(DATA_NAICS_MAP))]
...
def spell_check(blob):
    d2 = DictWithPWL("en_US",pkg_resources.resource_filename(__name__, 'data/google_100k.txt'))
