...
def run(**kwargs):
  N=kwargs.pop('N',10)
  L=kwargs.pop('L',120)# in mm L=89.24 for 440hz
  r=kwargs.pop('r',10)# in mm
  R=kwargs.pop('R',40)# in mm
  l=kwargs.pop('l',100)# in mm
 ...
 # assert np.max(np.abs(eigVal.imag))<1e-15 and np.min(eigVal.real)>=0, "Eigenvalues must be real positive"
  ...
  res={'bvp':bvp,'eigVal':eigVal,'eigVec':eigVec,'freq':freq,'N':N,'R':R,'L':L,'l':l,'r':r,'E':E,'rho':rho,'nu':nu}
  if graphics and isModuleFound('fc_simesh_mayavi'):
    kwargs['max_displacement']=kwargs.get('max_displacement',2*r) 
    if verbose>0:
...
def plot(res,**kwargs):
  ...
  max_displacement=kwargs.pop('max_displacement',L/10) 
