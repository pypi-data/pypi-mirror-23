...
from azure.ml.api.schema.schemaObjects import InternalSchema
...
class SchemaUtilTest(SchemaUnitTests):
    ...
    def test_parse_input_json_bad_inputs(self):
        ...
        self._validate_input_parsing_error_messages(input_json,
                                                    service_schema.input,
                                                    "Input request is not in format as specified by swagger")
    ...
    def test_parse_input_json_mismatch_arguments(self):
        ...
        try:
            ...
            if "Argument mismatch: Run function takes in an argument" not in be.message:
                self.fail("Expected error message not found, instead got {0}".format(be.message))
        except Exception as e:
            self.fail()

    def test_parse_input_json_handles_defaults(self):
        input_json = json.dumps({"a": [i for i in range(10)]})
        defaults = {"b":11}
        service_schema = load_service_schema(self.test_schema_path)
        inputs = parse_service_input(input_json, service_schema.input, defaults)
        if "a" not in inputs:
            self.fail("Argument a was not found in the input dictionary")
        if "b" in inputs:
            self.fail("Default argument found in input dictionary")

    @patch('azure.ml.api.schema.numpyUtil.NumpyUtil.get_input_object_from_file')
    @patch('azure.ml.api.schema.sparkUtil.SparkUtil.get_input_object_from_file')
    @patch('azure.ml.api.schema.pandasUtil.PandasUtil.get_input_object_from_file')
    @patch('azure.ml.api.schema.pythonUtil.PythonUtil.get_input_object_from_file')
