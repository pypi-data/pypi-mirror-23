...
class Router(TemplateView):
    ...
    def route(self, request: HttpRequest):
        ...
        indent = 2 if re.match("[Mozilla]{7}", headers['HTTP_USER_AGENT']) else 0
        ...
        try:
            ...
            return HttpResponse(json.dumps(response, indent=indent), content_type="application/json", status=403)
        except BaseException as e:
            response = {"status": False, "message": str(e)}
            return HttpResponse(json.dumps(response, indent=indent), content_type="application/json", status=500)
