...
from onyx.core import Structure, GetObj, GetVal, InvalidateNode, EvalBlock
from onyx.core import OnyxTestCase
from onyx.system import trade_api, transaction_api
...
class UnitTest(OnyxTestCase):
    ...
    def setUp(self):
        super().setUp()
        transaction_api.prepare_for_test()
    ...
    # -------------------------------------------------------------------------
    def test_BookAmendDelete(self):
        cash_trade = GetObj(self.cash_trade)
        cfd_trade = GetObj(self.cfd_trade)
        assert cash_trade.Party == cfd_trade.Party
        book = cash_trade.Party

        cash_trade = trade_api.CommitTrade(cash_trade)
        cfd_trade = trade_api.CommitTrade(cfd_trade)

        self.assertAlmostEqual(GetVal(book, "MktValUSD"), 300.0, 8)

        # --- trying deleting a trade without permissions should raise a
        #     TradeError exception. See below for the correct approach.
        with self.assertRaises(trade_api.TradeError):
            trade_api.DeleteTrade(cash_trade.Name)

        with EvalBlock() as eb:
            eb.change_value("Database", "TradesDeletable", True)
            trade_api.DeleteTrade(cash_trade.Name)

        # --- NB: Children needs to be invalidated manually because Book's
        #         Children are loaded directly from PosEffects table via
        #         ChildrenByBook
        InvalidateNode(book, "Children")
        self.assertAlmostEqual(GetVal(book, "MktValUSD"), 150.0, 8)

        with EvalBlock() as eb:
            eb.change_value("Database", "TradesDeletable", True)
            trade_api.DeleteTrade(cfd_trade.Name)

        # --- see comment above
        InvalidateNode(book, "Children")
        self.assertAlmostEqual(GetVal(book, "MktValUSD"), 0.0, 8)
