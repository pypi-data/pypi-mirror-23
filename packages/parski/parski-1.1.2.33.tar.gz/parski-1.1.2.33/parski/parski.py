...
class NodeSet(object):
    def __init__(self, fname=None, rawjson=None):
        ...
        if rawjson is not None:
            self.entries = self.loadj(rawjson)
    ...
    def loadj(self, rawjson):
        ...
        for key in rawjson:
            ...
            props['public_dns_name'] = key.get('DnsName')
            ...
            try:
                for tag in key['Tags']:
                    ...
                    if tag.get('Key') == 'Name':
                        props['name'] = tag.get('Value')
            except:
                pass
            try:
                for tag, val in key['InstanceState'].iteritems():
                    if tag == 'Name':
                        props['state'] = val
            except:
                pass
