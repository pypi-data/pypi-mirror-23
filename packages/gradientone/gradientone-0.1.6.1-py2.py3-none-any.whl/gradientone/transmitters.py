...
class Transmitter(BaseClient):
    ...
    def test_complete(self):
        ...
        self.metadata = self._get_metadata_only(self.results_dict)
...
class CANOpenTransmitter(Transmitter):
    ...
    def __init__(self, data=None, units=None, measurements=None,
    ...
    def _get_metadata_only(self, results_dict={}, handles=[]):
        """Returns metadata object

        Since the CANOpenTransmitter has no channels traces to remove,
        this method simply returns the results_dict. If it there is a
        handles object that needs to be cleaned up and assigned to
        metadata, the handles kwarg can be used.
        """
        metadata = {}
...
class ScopeTransmitter(Transmitter, ScopeClient):
    ...
    def generate_results_dict(self):
        results_page_link = BASE_URL + '/result/' + str(self.test_run_id)
        screenshot_url = (BASE_URL + '/download?file_key=screenshot-' +
                          str(self.test_run_id))
        self.results_dict = {
            'results_link': results_page_link,
            'scope_screenshot_url': screenshot_url,
    ...
    def generate_thumbnail_data(self):
        try:
            ...
            for ch in trace_data['channels']:
                channels.append(self.shrink(ch['y_values'], ch['time_step']))
    ...
    def post_result(self):
        ...
        PAYLOAD = {
            ...
            'fields': [
                ...
                {
                    'name': 'record_length',
                    'value': cfg_ex['acquisition']['record_length'],
                    'type': 'number'
                },
                {
