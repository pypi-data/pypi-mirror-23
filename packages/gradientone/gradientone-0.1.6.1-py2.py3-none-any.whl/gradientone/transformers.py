...
class ScopeTransformer(IviTransformer, IviScopeClient):
    ...
    def check_any_channel_enabled(self):
        ...
        if channels_enabled:
            ...
            data['instruments'] = {instrument_type: instrument_settings}
            self.put(BASE_URL + '/gateway', data=json.dumps(data))
            return True
        else:
            # if none enabled
            data['state'] = 'No Channels Enabled'
            logger.warning(data['state'])
            self.put(BASE_URL + '/gateway', data=json.dumps(data))
            return False
    ...
    def fetch_measurements(self):
        ...
        self.metadata = self._get_metadata_only(channels=self.channels)
...
class TransformerMSO2302A(ScopeTransformer):
    ...
    def load_raw_setup(self, try_count=0):
        ...
        try:
            ...
            if try_count > 10:
                ...
                self.instr = gateway_helpers.get_instrument(self.test_run)
    ...
    def _fetch_waveform(self, channel_name):
        ...
        super(TransformerMSO2302A, self)._fetch_waveform(channel_name)
        trigger_level = 0
        try:
    ...
    def check_instrument_ready(self):
        """Simply returns True since the MSO2302A has no busy? check"""
        return True
