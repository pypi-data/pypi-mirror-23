    ...
    def status(self, register="status_word", skip_format=False):
        ...
        for i in range(len(status_formatted)):
            ...
            if status_formatted[i]:
                if status_flag not in self.current_flags:
                    self.current_flags.append(status_flag)
                    self.flag_timestamps.append([timestamp, status_flag, True])
            else:
                if status_flag in self.current_flags:
                    del self.current_flags[self.current_flags.index(status_flag)]
                    self.flag_timestamps.append([timestamp, status_flag, False])
        if skip_format:
            return ""
        return ",".join(self.current_flags)
