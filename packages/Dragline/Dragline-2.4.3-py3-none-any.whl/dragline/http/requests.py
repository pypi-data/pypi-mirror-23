...
class Request(object):
    ...
    def get_args(self, keys=None):
        args = deepcopy(runtime.settings.DEFAULT_REQUEST_ARGS)
        request_id = None
        ...
        if args['proxies']:
            ...
            args['proxies']['http'] = args['proxies']['http'].replace("sessionid", request_id)
            args['proxies']['https'] = args['proxies']['https'].replace("sessionid", request_id)
        for i in args:
            if callable(args[i]):
                args[i] = args[i]()
        if keys:
    ...
    def send(self):
        ...
        try:
            args = self.get_args()
            response = runtime.request_processor.get_response(**args)
            response.meta = self.meta
            if args['proxies']:
                response.ids = urlparse(args['proxies']['http'])
            return response
