...
class Insertion_distribution():
    ...
    def plot_plt(self):
        datalist = self.data_processing2()
        fig, ax_list = plt.subplots(nrows=len(datalist))
        ax_list[0].set_title('%s/%s_insertcounts_distribution'%(self.tag, self.tag))
        font = {'family': 'serif', 
            'color':  'darkred', 
            'weight': 'normal', 
            'size': 16, 
        ...
        for index, data in enumerate(datalist): #datalist.reverse() [::-1]
            ...
            ax_list[index].set_xlim(xmax=len(df['count2']))#chromosome']))
    ...
    def data_processing2(self):
        list = []
        df = pd.read_table('%s/%s_all_insertions.xls'%(self.tag, self.tag), usecols=[0, 1, 2, 5])

        def chro_proc(chro, accession):
            df_chro = df[df['chromosome']==chro]
            # df_chro = df_chro.groupby(['insertions', 'insert_direct'], as_index=False).count()
            df_chro['F'] = np.where(df_chro['insert_direct']=='+', df_chro['count2'], 0)
            df_chro['R'] = np.where(df_chro['insert_direct']=='-', -1 * df_chro['count2'], 0)
