...
default_logging_verbosity = logging.INFO

logging_level_verbosity = default_logging_verbosity

# logging setup
logger = logging.getLogger(__name__)
logger.setLevel(logging_level_verbosity)
...
console.setLevel(logging_level_verbosity)
...
def dumpToCSV(dataframe, filename, dir):
    """dumps the dataframe into a csv file
    accepts dataframe and filename as argument"""
    filename = filename + ".csv"
    fullfilepath = dir + "\\" + filename
    logger.info("generating %s to %s" %(filename, fullfilepath))
    dataframe.to_csv(fullfilepath)
...
def config_loader(filepath):
    ...
    with open(filepath, 'r') as yamlfile:
        ...
        logger.info("configuration file %s read" %(filepath))
    return cfg
...
def main():
    ...
    parser.add_argument("cfg", help="configuration file(.yaml) to specify Oracle database connection parameters, "
                                    "SQL query parameters, golden list selection fields, date and time "
                                    "of the two data sets to sample and"
                                    " number of hours to process, Note: by default 'config.yaml' is used. See config.yaml"
                                    "file provided as sample for parameters", type=str,
                        )
    ...
    parser.add_argument("-v", "--verbose", help="detail message output", action="store_true")
    ...
    if args.verbose: # change verbosity if user specifies
        logging_level_verbosity = logging.DEBUG
        logger.setLevel(logging_level_verbosity)
        console.setLevel(logging_level_verbosity)

    cfg = config_loader(args.cfg) # read configuration .yaml file
    ...
    logger.debug("database connection details: %s" %db_connection)
    ...
    logger.debug("data dump taken from table: %s" %(cfg['OracleDBConnectionParameters']['table']))
    logger.debug("timeperiod: day1 %s, day2 %s, hrs %s" %(cfg['TimePeriod']['Day1'],cfg['TimePeriod']['Day2'],cfg['TimePeriod']['Hrs'] ))
    ...
    if args.imageinfo:  # if image option is selected
        if args.rawdata:
            ...
            logger.debug("timeperiod: day1 %s, day2 %s, hrs %s" % (
            cfg['TimePeriod']['Day1'], cfg['TimePeriod']['Day2'], cfg['TimePeriod']['Hrs']))
            ...
            if args.rawdata:
                dumpToCSV(df_raw1_image, miscfuncs.appendTimeInfo("rawDataDumpSet1withImg", cfg['TimePeriod']['Day1'],
                                                                                                            cfg['TimePeriod']['Hrs']), userhomedir)
                dumpToCSV(df_raw2_image, miscfuncs.appendTimeInfo("rawDataDumpSet2withImg", cfg['TimePeriod']['Day2'],
                                                                                                            cfg['TimePeriod']['Hrs']), userhomedir)
            dumpToCSV(df1_filt_df2_using_golden, args.goldenoutputfile, userhomedir)
            conn.close()
            sys.exit(0)
        ...
        if args.rawdata:
            dumpToCSV(df_raw1,
                      miscfuncs.appendTimeInfo("rawDataDumpSet1", cfg['TimePeriod']['Day1'], cfg['TimePeriod']['Hrs']), userhomedir)
            dumpToCSV(df_raw2,
                      miscfuncs.appendTimeInfo("rawDataDumpSet2", cfg['TimePeriod']['Day2'], cfg['TimePeriod']['Hrs']), userhomedir)
        dumpToCSV(df1_filt_df2_using_golden, args.goldenoutputfile, userhomedir)
        conn.close()
        sys.exit(0)
