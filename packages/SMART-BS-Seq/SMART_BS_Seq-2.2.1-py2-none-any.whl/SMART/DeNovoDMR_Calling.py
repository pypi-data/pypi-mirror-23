...
#                 print str(math.fabs(PreviousSpecificity-CurrentSpecificity))+":"+CurrentState+PreviousState+":"+str(SimilarityEntropyvalue)+":"+str(EucDistance)+":"+str((int(list0[j][1])-int(list0[0][2])))+":"+str(IntraCpGNum)
    ...
    def DeNovoFinalResult_MultiProgress(self,tmpFolder,BedOutFile,MergedSegmentOutFile,DMR_methyl_OutFile,DMR_Specificity_OutFile,SegmentCellTypeMethymarkPvalue,ChromeArray,SampleNames,Groups,Column_Key_Group,MSthreshold,SCthreshold,SLthreshold,p_DMR,p_MethylMark,statisticout):
        '''
...
#         Methyentropy=NewEntropy.Entropy()
    ...
    def DM_ROI(self, tmpDMCfolder,GenomeRegions,GenomeRegionFolder,BedOutFile,MergedSegmentOutFile,DMR_methyl_OutFile,DMR_Specificity_OutFile,SegmentCellTypeMethymarkPvalue,ChromeArray,SampleNames,Groups,Column_Key_Group,MSthreshold,SCthreshold,SLthreshold,p_DMR,p_MethylMark,statisticout):
        '''
    ...
    def runDMROI(self,UserArgslist,MethyData,ProjectName,MissReplace,GenomeRegions,MSthreshold,EDthreshold,SMthreshold,CDthreshold,SCthreshold,SLthreshold,p_DMR,p_MethylMark,OutFolder):
        ...
        SegmentCellTypeMethymarkPvalue.write("##Methylation marks of each group. p value is the one sample t test and represents the significance of difference between specific group methylation level and the baseline methylation levels.\nChrome\tStart\tEnd\tDMR_State\tGroup\tMethylMark_Type\tMethylMark_pValue\n")    
        TotalRegionNum,Numdec,CellTypeHypoNum,CellTypeHyperNum=Searchregion.DM_ROI(tmpDMCfolder,GenomeRegions,GenomeRegionFolder,BedOutFile,MergedSegmentOutFile,DMR_methyl_OutFile,DMR_Specificity_OutFile,SegmentCellTypeMethymarkPvalue,ChromeArray,SampleNames,Groups,Column_Key_Group,MSthreshold,SCthreshold,SLthreshold,p_DMR,p_MethylMark,statisticout)
        #print Numdec
        shutil.rmtree(tmpFolder) #Delete tmp files
...
if __name__ == '__main__':
    ...
    #MethyDataFolderName='/Users/hongbo.liu/Desktop/MethylMatrix_chr0.txt'
    ...
    Starttime=time.strftime('%Y-%m-%d %X',time.localtime(time.time()))
    print Starttime+" Start to check data and fill missing value..."
    statisticout.write(Starttime+" Start to check data and fill missing value...\n")
    
    DMCmethyfile=open(FinalResultFolder+"1_DifferMethlCpGs.txt",'wb') #Built a txt file 
    ChromeArray,SampleNames,Group_Key,Column_Key_Group,TotalCpGNum,AvailableCpGNum,DMC_Num,FillMissNum = Searchregion.DataCheck_and_MissReplace(MethyDataFolderName,MissReplace,AvailableGroup,MSthreshold,tmpDMCfolder,DMCmethyfile,p_DMR,statisticout)
    Groups=Group_Key.keys()
    #Genome segmentation based on methylation similarity between neighboring C...
    Starttime=time.strftime('%Y-%m-%d %X',time.localtime(time.time()))
    ...
    #BedOutFile.write("track name=\"MergedMethySegment\" description=\"Merged Methylation Segment of Human Cells/Tissues (0.4&0.2&500)\" visibility=2 itemRgb=\"On\"\n")
    ######Bed9 for Visualization in Hub of UCSC genome browser
    MergedSegmentOutFile=open(FinalResultFolder+"3_MergedSegment.txt",'wb') #Build a bed file    
    ...
    ######SegmentwithMethylation data
    DMR_methyl_OutFile=open(FinalResultFolder+"4_MergedSegmentwithmethylation.txt",'wb') #Build  a bed file 
    DMR_methyl_OutFile.write("#Mean methylation level of merged segments in each group.\nChrome\tStart\tEnd\tSpecificityState\tMethyState\tMethySpecificity\tNAOVA_pvalue\tMeanMethy\tCNum\tLength\tMethylData:"+','.join(Groups)+"\n")    
    ######Cell-type-specificity
    DMR_Specificity_OutFile=open(FinalResultFolder+"5_DMR_Group_Specificity.txt",'wb') #Build a bed file    
    DMR_Specificity_OutFile.write("#Methylation specificity of merged segments in each group.\nChrome\tStart\tEnd\tSpecificityState\tMethyState\tMethySpecificity\tNAOVA_pvalue\tMeanMethy\tCNum\tLength\tMethylMarkPvalue:"+','.join(Groups)+"\n")    
    ######Cell-type-specificity2
    SegmentCellTypeMethymarkPvalue=open(FinalResultFolder+"6_DMR_GroupSpecific_Methylmark.txt",'wb') #Build a bed file  
