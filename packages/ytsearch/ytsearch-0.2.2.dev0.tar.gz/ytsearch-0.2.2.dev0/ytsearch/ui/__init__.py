...
class Interface:
    ...
    def load_playlists(self):
        ...
        if os.path.exists('{}/playlists.json'.format(CONF_DIR)):
            with lock.Open('{}/playlists.json'.format(CONF_DIR)) as f:
                if f is None:
                    return None
                data = f.read()
            try:
    ...
    def save_playlists(self):
        ...
        if self.playlists == {} or self.playlists is None:
            return None
        with lock.Open('{}/playlists.json'.format(CONF_DIR), 'w') as f:
            if f is None:
                return None
    ...
    def load_playlist(self, playlist_name):
        ...
        """
        videos = []
        for data in list(self.playlists[playlist_name]):
    ...
    def event_SHUFFLE_QUEUE(self, _):
