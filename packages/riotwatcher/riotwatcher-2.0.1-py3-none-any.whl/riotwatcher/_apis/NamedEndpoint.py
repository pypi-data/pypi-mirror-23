...
class NamedEndpoint:
    """
    Helper class to inject endpont name into calls to a BaseApi instance without
    ...
    def __init__(self, base_api, endpoint_name):
        ...
        :param endpoint_name string: the name of the inheiriting endpoint
        """
    ...
    def _request(self, method_name, region, url_ext, **kwargs):
        """
        Sends a request through the BaseApi instance provided, injecting the
        provided endpoint_name into the method call, so the caller doesn't have to

        :param method_name string: the name of the calling method
        :param region string:   The region to execute this request on
        :param url_ext string:  the URL of the request, after the domain. This means
                                the part after the .com extension.
        """
