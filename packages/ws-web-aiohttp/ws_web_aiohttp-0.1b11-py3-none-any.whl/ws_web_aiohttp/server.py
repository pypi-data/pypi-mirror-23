...
async def setup_app(conf_mod, conf_dir, d, vargs, port=None):
    print(vargs)
    ...
    conf = modconf.import_class(conf_mod, 'Conf', ('DEVELOP' if d else 'DEPLOY', port, conf_dir), 
            kwargs=kwargs,
            folder=conf_dir)
...
def runserver(args):
    ...
    app = loop.run_until_complete(setup_app(args.conf_mod, args.conf_dir, args.d, vars(args), args.p))
