...
class RWSims:
    def __init__(self, obj = 100, length = 100, bound_x=(-np.Inf, np.Inf), bound_y=(-np.Inf, np.Inf), speed_range=range(5,10)):
        self.rw_paths = self.rwsim(obj, length, bound_x=bound_x, bound_y=bound_y, speed_range=speed_range)
        self.brw_paths = self.biased_rwsim(obj, length, bound_x=bound_x, bound_y=bound_y, speed_range=speed_range)
    ...
    def biased_rwsim(self, obj, length, speed_range = range(5,16), bound_x=(-np.Inf, np.Inf), bound_y=(-np.Inf, np.Inf)):
        '''
        ...
        for speed in speed_range:
            ...
            i = 0
            while i < obj:
                model_path = self.biased_random_walk(N = length, speed_mu = speed, bias_prop = 0.5, origin = (0,0), bound_x=bound_x, bound_y=bound_y)
...
class LevySims:
    def __init__(self, obj = 100, length = 100, bound_x=(-np.Inf, np.Inf), bound_y=(-np.Inf, np.Inf), p_scale = range(25,30), levy=False, levy_scale = range(1,5)):
        self.power_paths = self.power_sim(obj, length, bound_x=bound_x, bound_y=bound_y, scale_range = p_scale)
        if levy:
    ...
    def power_sim(self, obj, length, scale_range = range(25,35), bound_x=(-np.Inf, np.Inf), bound_y=(-np.Inf, np.Inf) ):
        '''
        ...
        for scale in scale_range:
            ...
            i = 0
            while i < obj:
                model_path = self.power_flier(N = length, scale_coeff=scale, bound_x=bound_x, bound_y=bound_y)
