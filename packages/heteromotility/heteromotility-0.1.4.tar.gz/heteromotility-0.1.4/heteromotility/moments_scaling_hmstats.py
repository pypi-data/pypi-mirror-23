...
class MSDFeatures:
    '''
    Calculated mean squared displacement features for a provided dict
    of trajectories in cell_ids
    ...
    def __init__(self, cell_ids):
        self.msd_distributions, self.log_distributions = self.msd_dist_all_cells(cell_ids)
    ...
    def calc_moment(self, path, tau, p):
        '''
        Calculates moment of order P from a cell path
        ...
        p    : order of the moment to be calculated
        ...
        while (t + tau) < len(path):
            dist_mom = ( distance(path[ t + tau ], path[ t ]) )**p
            distances.append(dist_mom)
            t += 1

        moment = np.mean(distances)

        return moment

    def calc_moment_distribution(self, path, max_tau, max_order = 6):
        '''
        Calculates the scaling exponent for a range of time
        lag values tau and moment orders [1,p]

        Parameters
        -------------------
        path : list of tuples containing sequential XY coordinates
        max_tau : maximum time lag to consider for MSD calculation
        max_order : maximum moment order to calculate

        Returns
        -------------------
        distribution : P x maxTau array of moment values for displacements of
                       size tau (columns), where P is the order of the moment
        ...
        Here -- returns both distributions for that moment as -1 if
        MSD calc is 0 for a given range tau
        '''
        distribution = np.zeros( ( int(max_order), int(max_tau) ) )
        log_distribution = np.zeros( ( int(max_order), int(max_tau) ) )

        for p in range(1, max_order+1):
            tau = 1
            while tau < max_tau:
                mom = self.calc_moment(path, tau, p)
                distribution[p-1,tau-1] = mom
                if mom > 0:
                    log_distribution[p-1,tau-1] = math.log(mom)
                else:
                    distribution[p-1,:] = -1
                    log_distribution[p-1, :] = -1
                    tau = max_tau
                tau += 1

        return distribution, log_distribution

    def msd_dist_all_cells(self, cell_ids, max_order = 6):
        '''
        Calculates moment distributions for all cells in dict cell_ids
        ...
        Returns
        -------------------
        mom_distributions : dict keyed by cell_id containing P x maxTau array
                            of MSD distributions, indexed by tau
        log_distributions : dict with log transformed arrays of
                            MSD distributions

        Notable behavior
        -------------------
        checks for moment distributions == -1 and sets values for
        that cell as -1, which is checked to provide an alpha of 0
        This is done manually, as stats.linregress fails ungracefully
        if points are identical, as happens for some taus with simulated data

        Considers time lags (tau) in the interval [1, path_length/4] to ensure
        adequate sampling of each time lag

        As with all MSD calculations, a path length of ~>=20 units it required
        ...
        for u in cell_ids:
            cell = cell_ids[u]
            max_tau = np.floor(len(cell)/4)
            dist, log_dist = self.calc_moment_distribution(cell, max_tau, max_order)

            mom_distributions[u] = dist
            log_distributions[u] = log_dist

        return mom_distributions, log_distributions

    def calc_alphas(self, log_distributions):
        '''
        Calculate the alpha coefficient value as the slope of a
        log(Moment) v log(tau) plot

        Parameters
        -----------------
        log_distributions : dict keyed by cell_id containing P x maxTau arrays
        of log transformed moment values, where P is the moment order

        Returns
        ----------------
        alphas : dict of dicts keyed moment order then by cell_id,
            with values as the alpha coeff ~ log(Moment(tau)) = alpha*log(tau)

        Notable Behavior
        ----------------
        Checkes if log_distributions[cell_id][P,:] is == -1 for all
        If so, sets alpha at the appropriate "flat" slope of 0
        '''

        alphas = {}
        max_order = log_distributions[log_distributions.keys()[0]].shape[0]
        max_tau = log_distributions[log_distributions.keys()[0]].shape[1]
        ...
        for p in range(1, max_order+1):
            ...
            for u in log_distributions:
                if sum(log_distributions[u][p-1,:] == -1) > 0:
                    alphas[p][u] = 0
                else:
                    slope, intercept, r_val, p_val, SE = stats.linregress( log_tau, log_distributions[u][p-1,:] )
                    alphas[p][u] = slope
    ...
    def make_displacement_dist(self, cell_ids, tau):
        ...
        Parameters
        ----------
        cell_ids : dict of trajectories keyed by unique cell identifier, valued
                   by lists of tuples of sequential XY coordinates
        tau      : time lag to consider when calculating displacements

        Returns
        ----------
        displacement_dist : dict of displacement series keyed by unique cell
                            identifiers, valued by lists of sequential
                            displacements
        '''
    ...
    # Possibly refactor to perform vectorized calculations
    def moment_scaling(self, dispArray):
        '''
        Calculates the exponent describing the scaling of each moment
        of the displacement distribution with variable time lags (tau)

        Scaling exponent Gamma calculated as the slope of relationship between
        log(moment) vs log(tau)

        mu_p(tau) = 2*N*D_p*tau^gamma_p

        where p is the moment being calculated, N is the dimensionality,

        Parameters
        -----------
        dispArray : M x T array of displacements, where M is the number of cells
                    and T is the number of observed displacements
        keyVector : M x 1 vector of unique cell identifiers, labeling rows in
                    dispArray

        Returns
        -----------
        momentScaling : dict of dicts keyed by moment, keyed by cell identifiers
                        valued with the exponents of moment scaling

                mS = {
                moment1 : {cell_id1 : moment1Gamma,...}
                moment2 : {cell_id2 : moment2Gamma,...}
                ...
                momentP : ...
                }
        '''
...
class RWFeatures:
    ...
    * estimating self-similarity metrics
    ...
    '''
    def __init__(self, cell_ids, gf, pacorr_max = False, autocorr_max_tau = 11): # gf = GeneralFeatures
        ...
        self.autocorr, self.qstats, self.pvals = self.autocorr_all(cell_ids, max_tau = autocorr_max_tau)
        #if pacorr_max != False:
    ...
    def compare_to_randwalk(self, cell, u, cell_linearity, cell_net_dist, cell_avg_speed):
        ...
        Parameters
        -----------
        cell : list of (x,y) tuple-coordinates denoting the cell path
        u    : key of the cell in all data dicts
        cell_linearity : dict keyed by unique cell ids valued with floats of
                         cell linearity (pearson's r^2)
        cell_net_dist  : dict keyed by unique cell ids valued with floats of
                         a cell's net distance from it's starting location
        cell_avg_speed : dict keyed by unique cell ids valued with floats of
                         a cell's average speed

        Returns
        -----------
        diff_linearity : dict keyed by cell ids of the difference between a cell's
                         observed linearity and the theoretical linearity if the
                         cell were random walking
        diff_net_dist  : dict keyed by cell ids of the difference between a cell's
                         observed net distance and the theoretical net distance
                         if the cell were random walking
        '''
        origin = cell[0]
        N = len(cell)
        rate = cell_avg_speed
