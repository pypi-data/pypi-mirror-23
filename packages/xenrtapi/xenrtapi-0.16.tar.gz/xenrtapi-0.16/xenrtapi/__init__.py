...
class XenRT(object):
    ...
    def update_machine(self, name, params=None, broken=None, status=None, resources=None, addflags=None, delflags=None, prio=None, private=None):
        """
        ...
        `private`: boolean - Whether the fields set in params are private  
        """
    ...
    def get_group_from_sid(self, sid):
        """
        Get a group name from a SID  

        Parameters:  
        `sid`: string - Group SID  
        """
        path = "%s/groupfromsid" % (self.base)
        paramdict = {}
        files = {}
        if sid != None:
            paramdict['sid'] = self.__serializeForQuery(sid)
        payload = {}
        r = requests.get(path, params=paramdict, headers=self.customHeaders)
        if r.status_code in (502, 503):
            time.sleep(30)
    ...
    def get_machine_offering(self, offeringid):
        ...
        Parameters:  
        `offeringid`: integer - Machine offering ID to get  
        """
        path = "%s/machineofferings/{offeringid}" % (self.base)
        path = path.replace("{offeringid}", self.__serializeForPath(offeringid))
        paramdict = {}
    ...
    def delete_machine_offering(self, offeringid):
        ...
        Parameters:  
        `offeringid`: integer - Machine offering ID to delete  
        """
        path = "%s/machineofferings/{offeringid}" % (self.base)
        path = path.replace("{offeringid}", self.__serializeForPath(offeringid))
        paramdict = {}
    ...
    def get_machines(self, status=None, site=None, pool=None, cluster=None, user=None, machine=None, resource=None, flag=None, aclid=None, limit=None, offset=None, pseudohosts=None, search=None, group=None, include_forbidden=None, only_restricted=None, only_available_now=None, only_available_now_preemptable=None, leasegroup=None, mine=None):
        """
        ...
        `leasegroup`: list of string - Filter on lease group - can specify multiple  
        `mine`: boolean - Filter on my machines - either leased to me or one of my groups  
        """
        path = "%s/machines" % (self.base)
        paramdict = {}
        files = {}
        ...
        if leasegroup != None:
            paramdict['leasegroup'] = self.__serializeForQuery(leasegroup)
        if mine != None:
            paramdict['mine'] = self.__serializeForQuery(mine)
        payload = {}
        r = requests.get(path, params=paramdict, headers=self.customHeaders)
    ...
    def get_user_from_sid(self, sid):
        """
        Get a username from a SID  

        Parameters:  
        `sid`: string - User SID  
        """
        path = "%s/userfromsid" % (self.base)
        paramdict = {}
        files = {}
        if sid != None:
            paramdict['sid'] = self.__serializeForQuery(sid)
    ...
    def update_job(self, id, params=None, complete=None, private=None):
        """
        Update job details  
        ...
        `private`: boolean - Whether the params being update are private  
        """
    ...
    def new_deployment_spec(self, name, deployment, params=None, min_hosts=None, max_hosts=None, flags=None, variables=None, default=None):
        """
        Create a new saved deployment spec  

        Parameters:  
        `name`: string - Name of the Deployment Spec  
        `deployment`: dictionary - Deployment Spec definition  
        `params`: dictionary - Key/value pair of parameters when this deployment spec is used  
        `min_hosts`: ['integer', 'null'] - Minimum hosts for this deployment  
        `max_hosts`: ['integer', 'null'] - Maximum hosts for this deployment spec  
        `flags`: list of string - List of flags when this deployment spec is used. Can negate by prefixing a flag with '!'  
        `variables`: dictionary - Variables for this job. Format is {"variable": {"friendlyname": <friendlyname>, "options": {"option1": {"flags": ["flag1", "flag2"]}}}}  
        `default`: boolean - Whether to make this the default deployment spec  
        """
        path = "%s/deploymentspecs" % (self.base)
        paramdict = {}
        files = {}
        payload = {}
        j = {}
        ...
        if default != None:
            j['default'] = default
        if min_hosts != None:
            j['min_hosts'] = min_hosts
        if variables != None:
            j['variables'] = variables
        if max_hosts != None:
            j['max_hosts'] = max_hosts
        if flags != None:
            j['flags'] = flags
        payload = json.dumps(j)
        myHeaders = {'content-type': 'application/json'}
    ...
    def get_deployment_specs(self, deploymentspecid):
        """
        Get a list of Saved Deployment Specs  
    ...
    def new_job(self, machines=None, pools=None, flags=None, resources=None, specified_machines=None, sequence=None, custom_sequence=None, params=None, deployment=None, job_group=None, email=None, inputdir=None, lease_machines=None, preemptable=None, testcases=None, expiry=None, run_timeout=None, group=None):
        """
        Submits a new job  
        ...
        `group`: string - Group to run this job as  
        """
    ...
    def lease_machine(self, name, duration, reason, force=None, besteffort=None, preemptable=None, admin_override=None, user=None, group=None):
        """
        Lease a machine  
        ...
        `group`: string - Group to borrow a machine as (you must be a member of this group)  
        """
    ...
    def update_deployment_spec(self, deploymentspecid, name=None, deployment=None, params=None, min_hosts=None, max_hosts=None, flags=None, variables=None, default=None):
        """
        Update an existing Saved Deployment Spec  

        Parameters:  
        `deploymentspecid`: integer - Deployment Spec ID to update  
        `name`: string - Name of the Deployment Spec  
        `deployment`: dictionary - Deployment Spec definition  
        `params`: dictionary - Key/value pair of parameters when this deployment spec is used  
        `min_hosts`: ['integer', 'null'] - Minimum hosts for this deployment  
        ...
        `default`: boolean - Whether to make this the default deployment spec  
        """
        path = "%s/deploymentspecs/{deploymentspecid}" % (self.base)
        path = path.replace("{deploymentspecid}", self.__serializeForPath(deploymentspecid))
        paramdict = {}
        files = {}
        payload = {}
        j = {}
    ...
    def get_deployment_spec(self, deploymentspecid):
        """
        Get a Saved Deployment Spec  

        Parameters:  
        `deploymentspecid`: integer - Saved Deployment Spec ID to get  
        """
        path = "%s/deploymentspecs/{deploymentspecid}" % (self.base)
        path = path.replace("{deploymentspecid}", self.__serializeForPath(deploymentspecid))
        paramdict = {}
    ...
    def delete_deployment_spec(self, deploymentspecid):
        """
        Delete a Saved Deployment Spec  

        Parameters:  
        `deploymentspecid`: integer - Saved Deployment Spec ID to delete  
        """
        path = "%s/deploymentspecs/{deploymentspecid}" % (self.base)
        path = path.replace("{deploymentspecid}", self.__serializeForPath(deploymentspecid))
        paramdict = {}
    ...
    def get_job_log(self, id, logid):
        """
        Gets the tail for the job log  

        Parameters:  
        `id`: integer - Job ID to get log for  
        `logid`: integer - Log ID to start from  
        """
        path = "%s/job/{id}/log" % (self.base)
        path = path.replace("{id}", self.__serializeForPath(id))
        paramdict = {}
        files = {}
        if logid != None:
            paramdict['logid'] = self.__serializeForQuery(logid)
        payload = {}
        r = requests.get(path, params=paramdict, headers=self.customHeaders)
        if r.status_code in (502, 503):
            time.sleep(30)
    ...
    def job_can_run(self, machines=None, pools=None, flags=None, resources=None, specified_machines=None, params=None, preemptable=None, group=None):
        """
    ...
    def get_job_tcs(self, machines=None, pools=None, flags=None, resources=None, specified_machines=None, sequence=None, custom_sequence=None, params=None, deployment=None, job_group=None, email=None, inputdir=None, lease_machines=None, preemptable=None, testcases=None, expiry=None, run_timeout=None, group=None):
        """
    ...
    def start_suite_run(self, suite, branch, version, skus=None, params=None, seqs=None, rerun=None, rerunall=None, rerunifneeded=None, xenrtbranch=None, devrun=None, delay=None, suiteruntag=None, sku=None):
        """
        Start suite run  
        ...
        `skus`: list of string - Ordered list of SKUs to apply to suite run  
        ...
        `sku`: string - Suite run SKU (DEPRECATED - use skus parameter instead)  
        """
    ...
    def rerun_suite_run(self, id, seqs, xenrtbranch=None):
        """
        Rerun sequence(s) from suite run  

        Parameters:  
        `id`: string - id of the suiterun to rerun  
        `seqs`: list of string - Sequences to run  
        `xenrtbranch`: string - XenRT branch to use to submit suite (note that XRTBRANCH also needs to be set in the suite)  
        """
        path = "%s/suiterun/rerun/{id}" % (self.masterbase)
        path = path.replace("{id}", self.__serializeForPath(id))
        paramdict = {}
        files = {}
        payload = {}
