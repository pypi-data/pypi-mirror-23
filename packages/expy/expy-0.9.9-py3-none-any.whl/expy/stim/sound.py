...
# The experiment procedure wouldn't be blocked by this function
def playFreeSound(sound, playing_track=None):
    '''
    Play a sound array
...
def change_volume(raw, index):
    return raw * index

import scipy
import resampy

def change_pitch(raw, scale):
    size = len(raw)
...
def playAlterableSound(sound, effect=change_volume, 
        key_up=key_.RIGHT, key_down=key_.LEFT, key_confirm=key_.ENTER):
    '''
    Play a sound array
    ...
    sound: array
        The sound data
    effect: function (default: change_volume)
    ...
    chunk = 1024
    if effect==change_pitch: chunk = 1024*8
    ...
    while 1:
        ...
        if shared.pressing==key_up:
            if effect==change_volume and changed_index>1:
                changed_index = 1
            else:
                changed_index *= 1.01
