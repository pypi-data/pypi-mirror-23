...
def remote_present(runner, remote_url):
    runner("conan remote list")
    return remote_url in str(runner.output)


def add_remote_safe(runner, name, url, insert=False):
    if remote_present(runner, url):
        return

    if insert:
        if runner("conan remote add %s %s --insert" % (name, url)) != 0:
            logger.info("Remote add with insert failed... trying to add at the end")
        else:
            return 0
    runner("conan remote add %s %s" % (name, url))  # Retrocompatibility
...
class ConanOutputRunner(ConanRunner):
    ...
    def __call__(self, command):
        return super(ConanOutputRunner, self).__call__(command, output=self.output)
...
''')
        ...
        if self.upload:
            if remote_present(self.runner, self.upload):
                pass
            else:
                add_remote_safe(self.runner, "upload_repo", self.upload, insert=False)
        ...
        if self.remotes:
            ...
            for counter, remote in enumerate(reversed(remotes)):
                ...
                add_remote_safe(self.runner, remote_name, remote, insert=True)
