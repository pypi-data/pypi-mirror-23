...
''')
        ...
        if self.upload:
            self.add_remote_safe(self.runner, "upload_repo", self.upload, insert=False)
        ...
        if self.remotes:
            ...
            for counter, remote in enumerate(reversed(remotes)):
                ...
                self.add_remote_safe(self.runner, remote_name, remote, insert=True)
    ...
    def add_remote_safe(self, name, url, insert=False):
        ...
        existing_name = self.get_remote(url)
        if existing_name:
            runner("conan remote remove %s" % existing_name)
