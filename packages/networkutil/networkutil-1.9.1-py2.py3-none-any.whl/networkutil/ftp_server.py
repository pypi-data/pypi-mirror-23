...
class FTPServer(object):
    ...
    def __init__(self,
                 interface='0.0.0.0',  # 0.0.0.0 must be used rather than enpty string otherwise windows breaks
                 port=2121):
    ...
    def add_user(self,
        ...
        logging.debug('Adding user: {un}'.format(un=username))
        self.authoriser.add_user(username, password, directory, perm=permission)

    def remove_user(self,
                    username):
        if self.authoriser.has_user(username):
            logging.debug('Removing user: {un}'.format(un=username))
    ...
    def set_max_connections(self,
        ...
        if self.server is not None:
            ...
            logging.error('Cannot set max connections to {num}.  Server not started yet!'.format(num=num_conns))
    ...
    def set_max_connections_per_ip(self,
        ...
        if self.server is not None:
            ...
            logging.error('Cannot set max connections per ip to {num}.  Server not started yet!'.format(num=num_conns))
