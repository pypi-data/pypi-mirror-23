...
class LibraryOrgGui:
    def __init__(self, root, parent, standalone=False):
        ...
        if C.MODEL_SELECT == 'MPV':
            ...
            def clip_selector(clip, layer=0):
                ...
                try:
                    ...
                    if C.DEBUG:
                        print('your external command doesnt work gg')
                    return
                self.perform_delayed_actions(clip)
        ...
        self.delete_but = ttk.Button(self.bottom_bar, text='(DEL)ete', padding=bot_but_pad, command=self.delete_prompt)
        self.root.bind("<Delete>", self.delete_prompt)

        self.rename_but = ttk.Button(self.bottom_bar, text='(R)ename', padding=bot_but_pad, command=self.rename_prompt)
        self.root.bind("r", self.rename_prompt)
        self.root.bind("<F2>", self.rename_prompt)

        self.tag_but = ttk.Button(self.bottom_bar, text='(T)ag', padding=bot_but_pad, command=self.tag_prompt)
        self.root.bind("t", self.tag_prompt)

        self.move_but = ttk.Button(self.bottom_bar, text='(M)ove', padding=bot_but_pad, command=self.move_prompt)
    ...
    def tag_prompt(self, *args):
        ...
        def gen_tag_list_callback(cliplist, itemlist):
            ...
            def gend_fun(tag_list):
                for i, clp in enumerate(clipz):
                    self.db.tagdb.update_clip_tags(tag_list, clp)
                    new_tags = clp.str_tags()
    ...
    def activate_clip(self, event=None):
        ...
        if clip_fname is None or len(clip_fname) < 1:
            return
        # print(clip_fname)
        if clip_fname in self.db.clips:
...
class ImportWizardGui:
    def __init__(self, top_frame, parent):
        ...
        self.delete_but = ttk.Button(self.bottom_bar, text='(DEL)ete', padding=bot_but_pad, command=self.delete_clip)

        self.root.bind("r", self.rename_clip)
        self.root.bind("<F2>", self.rename_clip)
        self.rename_but = ttk.Button(self.bottom_bar, text='(R)ename', padding=bot_but_pad, command=self.rename_clip)

        self.root.bind("t", self.tag_clip)
        self.tag_but = ttk.Button(self.bottom_bar, text='(T)ag', padding=bot_but_pad, command=self.tag_clip)

        self.root.bind("m", self.move_clip)
        self.move_but = ttk.Button(self.bottom_bar, text='(M)ove', padding=bot_but_pad, command=self.move_clip)
