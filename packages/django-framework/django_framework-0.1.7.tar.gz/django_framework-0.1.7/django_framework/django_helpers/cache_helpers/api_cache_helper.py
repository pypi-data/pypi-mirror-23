...
class APICache(BaseCache):
    ...
    def _clear(self, model_name = None, token = None):

        if model_name == None:
            # clear it all!
            self.redis_connection.flushall()
        
        elif model_name != None and token != None: # this is level 1

            self.redis_connection.delete(self.generate_key_name())
            self.redis_connection.hdel(self.modified_model_name, token)
            
        elif model_name != None and token == None: # this is level 2 sorta
            self.redis_connection.delete(self.modified_model_name)


    def _clear_level(self, level, model_name = None, token = None):
        
        if level == 0:
            return
        elif level == 1:
            self.clear(model_name = model_name, token = token)
        elif level == 2:
            self.clear(model_name = model_name, token = None)
