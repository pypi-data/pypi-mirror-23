...
def updateBank(amount):
    ...
    word = 'loss' if amount < 0 else 'winning'
...
def amountToCurrency(amount):
    ...
    except ValueError:  # If the local does not support currency formatting
...
def wheel():
    """
        Will define the winning number and return an array with the complete game
...
def game():
    """
        Spin the wheel and shows the ball position and returns the winning position
...
def getOutcome(betAmount, bet, specificChoice=None):
    """
        Initiate a game with a bet amount and a bet type and calculates the outcome
    """
...
def getColorIcon(color):
    """
        If unicode is supported, will return an icon for the number color.
    ...
    try:
        ...
        while valid == False:
            if previousBetNumber:
                ...
                betNumber = input('* Chose a bet number (just press [ENTER] to play again `%s`): ' % (previousBet['name']))

                # Default to previous bet
                if betNumber == '':
                    betNumber = previousBetNumber
            else:
                betNumber = input('* Chose a bet number: ')
