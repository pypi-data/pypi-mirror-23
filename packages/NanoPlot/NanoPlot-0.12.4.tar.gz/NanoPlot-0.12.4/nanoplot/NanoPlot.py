...
def getStats(datadf, outdir):
	...
	with open(os.path.join(outdir, "NanoStats.txt"), 'wt') as output:
		...
		for length, qual in sorted(statdict["MaxLengthsAndQ"], key=lambda x: x[0], descending=True):
			output.write("Length: {}bp\tQ: {}\n".format(length, round(qual, 2)))
		output.write("\n")
		output.write("Top 5 average basecall quality scores their read lengths:\n")
		for length, qual in sorted(statdict["MaxQualsAndL"], key=lambda x: x[1], descending=True):
			output.write("Length: {}bp\tQ: {}\n".format(length, round(qual, 2)))
		output.write("\n")
