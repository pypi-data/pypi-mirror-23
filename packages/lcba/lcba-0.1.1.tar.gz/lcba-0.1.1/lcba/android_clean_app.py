...
ANDROID_MANIFEST_FILE = config.ANDROID_MANIFEST_FILE
LAYOUT_IDS_PATH = config.LAYOUT_IDS_PATH
...
def remove_resource_file(issue, filepath, ignore_layouts):
    ...
    if os.path.exists(filepath) and (ignore_layouts is False or issue.elements[0][0] != 'layout'):
        ...
        if issue.elements[0][0] == 'layout':
            curPath = os.getcwd()
            ids_path = curPath + LAYOUT_IDS_PATH
...
# 删除资源，包括文件和重复的字符
###############################################
def deleteRes(lint_result_path, app_dir, ignore_layouts_file, ignore_layouts_value):
...
def main():
    ...
    deleteRes(curPath + "/JuMeiYouPinV1.0.0/build/reports/lint-results.xml", curPath + "/JuMeiYouPinV1.0.0/", True,
              True)
