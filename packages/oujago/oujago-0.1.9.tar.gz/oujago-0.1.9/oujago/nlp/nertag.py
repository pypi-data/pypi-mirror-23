...
class MoranNER(BaseNER):
    """Moran NER method. For internal use only.
    """
    ...
    def recognize(self, sentence=None, words=None, postags=None, **kwargs):
        # get results
        result = _moran(sentence)

        # format
        if oujago.utils.is_py3():
            return [(sentence[tag.offset // 3: (tag.offset + tag.length) // 3], tag.uniform)
...
def ner(sentence=None, mode=None, **kwargs):
    ...
    Examples:
        ...
        ('飞船', 'O'), ('成功', 'O'), ('登月', 'O'), ('的', 'O')]
        >>> ner("阿姆斯特朗是乘哪个飞船成功登月的", mode='moran')
        [('阿姆斯特朗', 'PER')]
    ...
    if mode == 'moran':
        ...
        if _moran_instance is None:
            _moran_instance = MoranNER()

        # NER
        return _moran_instance.recognize(sentence, **kwargs)
