...
class SnipsLocalMusic:
    ...
    def __init__(self, db_file_path, threading=None):
        """ Initialisation.

        :param db: the json music database.
        :param threading: a thread handler object for handling asynchronous
                          playback.
        """
    ...
    def play_file(self, filename):
        ...
        if self.threading:
            self.threading.run(target=AudioPlayer.play, args=(filename, None,))
        else:
...
class AudioPlayer:
    ...
    @staticmethod
    def play(filename, on_done, run_event=None):
        """ Play a file.

        :param filename: the path to the audio file to play.
        :param on_done: callback to execute when playing is done.
        :param run_event: a run event object to pass for background
                          playback.
        """
        pygame.mixer.init()
