...
public:
	...
	// Can we do a operator<>()? But we don't need it as in comparison to OpenCV, we know the type at compile time...
...
/*	static Image<T, num_channels> zeros(std::size_t rows, std::size_t cols)
	{
		...
		for (int c=0; c < image.cols; ++c) {
			for (int r = 0; r < image.rows; ++r) {
				image(r, c) = T{ 1 }; // Does this create an std::array with all zeros, like we expect? No it doesn't, it only sets the first component to 0, and leaves the rest.
			}
		}
		return image;
	}; */
...
//private:
	std::vector<T> data; // Maybe not too ideal, see the zeros(...) function. Should rather encode [RGB...] etc in here too, directly. Maybe.
