...
def ps(event):
    ...
    for up, thrname, obj in sorted(result, key=lambda x: x[0]):
        nr += 1
        res = psformat % (nr, elapsed(up), thrname[:24], obj._state.printable(nokeys=True), obj._counter.printable(nokeys=True), obj._error.printable(nokeys=True))
