...
from .utils import days, locked, stripped, tname, to_time, name
...
class Event(Default):
    ...
    def dispatch(self):
        for func in self._funcs:
            logging.info("! dispatch %s %s (%s)" % (name(func), self._parsed.rest, self.origin))
            try:
                func(self)
            except:
    ...
    def wait(self, sec=None):
        ...
        super().wait()
        for thr in self._thrs:
            logging.debug("join %s" % tname(thr))
            thr.join(sec)
