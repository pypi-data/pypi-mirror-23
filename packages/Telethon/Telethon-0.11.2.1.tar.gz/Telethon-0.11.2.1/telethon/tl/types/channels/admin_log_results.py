...
class AdminLogResults(MTProtoRequest):
    ...
    def __str__(self):
        return '(channels.adminLogResults (ID: 0xed8af74d) = (events={}, chats={}, users={}))'.format(None if not self.events else [str(_) for _ in self.events], None if not self.chats else [str(_) for _ in self.chats], None if not self.users else [str(_) for _ in self.users])
