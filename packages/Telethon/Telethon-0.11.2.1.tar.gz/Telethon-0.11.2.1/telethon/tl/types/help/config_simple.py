...
class ConfigSimple(MTProtoRequest):
    ...
    def on_send(self, writer):
        ...
        for ip_port_list_item in self.ip_port_list:
            ip_port_list_item.on_send(writer)
    ...
    def on_response(self, reader):
        ...
        ip_port_list_len = reader.read_int()
        for _ in range(ip_port_list_len):
            ip_port_list_item = reader.tgread_object()
            self.ip_port_list.append(ip_port_list_item)

    def __repr__(self):
        return 'help.configSimple#d997c3c5 date:int expires:int dc_id:int ip_port_list:Vector<ipPort> = help.ConfigSimple'

    def __str__(self):
        return '(help.configSimple (ID: 0xd997c3c5) = (date={}, expires={}, dc_id={}, ip_port_list={}))'.format(str(self.date), str(self.expires), str(self.dc_id), None if not self.ip_port_list else [str(_) for _ in self.ip_port_list])
