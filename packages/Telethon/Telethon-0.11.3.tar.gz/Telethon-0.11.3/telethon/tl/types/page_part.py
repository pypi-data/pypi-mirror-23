...
class PagePart(MTProtoRequest):
    ...
    def __str__(self):
        return '(pagePart (ID: 0x8e3f9ebe) = (blocks={}, photos={}, documents={}))'.format(None if not self.blocks else [str(_) for _ in self.blocks], None if not self.photos else [str(_) for _ in self.photos], None if not self.documents else [str(_) for _ in self.documents])
