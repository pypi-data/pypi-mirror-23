...
class LangPackDifference(MTProtoRequest):
    ...
    def on_send(self, writer):
        ...
        for strings_item in self.strings:
            strings_item.on_send(writer)
    ...
    def on_response(self, reader):
        ...
        strings_len = reader.read_int()
        for _ in range(strings_len):
            strings_item = reader.tgread_object()
            self.strings.append(strings_item)
    ...
    def __str__(self):
        return '(langPackDifference (ID: 0xf385c1f6) = (lang_code={}, from_version={}, version={}, strings={}))'.format(str(self.lang_code), str(self.from_version), str(self.version), None if not self.strings else [str(_) for _ in self.strings])
