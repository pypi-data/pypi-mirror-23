...
class PageFull(MTProtoRequest):
    ...
    def __str__(self):
        return '(pageFull (ID: 0x556ec7aa) = (blocks={}, photos={}, documents={}))'.format(None if not self.blocks else [str(_) for _ in self.blocks], None if not self.photos else [str(_) for _ in self.photos], None if not self.documents else [str(_) for _ in self.documents])
