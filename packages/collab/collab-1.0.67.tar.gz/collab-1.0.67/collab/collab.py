...
class FileObject:
    """
    represents a file that was given as an input argument, or created as an output, along with information about it.
    """
    def __init__(self, identifier, type, file_name, creation_step, creation_index, usage_index):
        if type == _OUTPUT_FILE:
            if usage_index is None:
                raise ValueError("output files must not have a usage_index set, since that wouldn't make sense")
        elif type == _INPUT_FILE:
            pass
        else:
            raise ValueError("invalid type: FileObject must be either an input or an output of this program")
    ...
    def to_json(self):
        ...
        NOT a counterpart to _parse_tag(): the information output here is for treating a file as a newly created object, not an existing object.
        """
        if self.type != _OUTPUT_FILE:
            raise IOError("this object represents an input file, not an output file.")
        if self.creation_step != get_current_step():
            raise IOError("can't output a file that wasn't created by this program in this step")
        res = {'identifier' : self.identifier.to_json(), "file" : file_name, "index" : creation_index }
        return res
...
def get_inputs():
    ...
    for c,a in enumerate(inputs):
        ...
        usage_index = c
        res.append(FileObject(identifier, type, file_name, creation_step, creation_index, usage_index))
    return res
...
def add_output_file():
    ...
    type = _OUTPUT_FILE
    file_name = "%d" % _output_counter
    creation_step = get_current_step()
    creation_index = _output_counter
    usage_index = None
    res = FileObject(identifier, type, file_name, creation_step, creation_index, usage_index)
    _output_counter += 1
