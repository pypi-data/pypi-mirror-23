...
import requests
import logging
from ..config import qualityplatform_api_url
...
def add_case_file(test_case_id, file_hash):
    try:
        url = qualityplatform_api_url + 'testcase/fuss_hash'
        info = {
...
def get_device_by_id(device_id):
    try:
        url = qualityplatform_api_url + 'devices/' + str(device_id)
        r = requests.get(url)
        return r.json()
    except requests.RequestException as e:
        logger.exception(e)
