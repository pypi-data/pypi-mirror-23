...
from featuretools.features import (Day, Hour, Diff, Compare, Not,
...
def test_arithmetic(es):
    ...
    features = []
    features.append(day.ADD(hour))
    features.append(day.SUB(hour))
    features.append(day.MUL(hour))
    features.append(day.DIV(hour))
...
def test_override_cmp_from_variable(es):
    count_lo = IdentityFeature(es['log']['value']).GT(1)
...
def test_override_cmp(es):
    ...
    gt_lo = count.GT(1)
    gt_other = count.GT(_sum)
    ge_lo = count.GE(1)
    ge_other = count.GE(_sum)
    lt_hi = count.LT(10)
    lt_other = count.LT(_sum)
    le_hi = count.LE(10)
    le_other = count.LE(_sum)
    ne_lo = count.NE(1)
    ne_other = count.NE(_sum)
