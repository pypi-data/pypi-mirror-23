...
from featuretools.features import (IdentityFeature, DirectFeature, Sum, Count,
                                   Min, Mode, Compare, Mean, And, TopNMostCommon)
...
def test_make_agg_feat_where_count(entityset, backend):
    agg_feat = Count(base_feature=entityset['log']['id'],
                     parent_entity=entityset['sessions'],
                     where=IdentityFeature(entityset['log']['product_id']).EQ('coke zero'))
...
def test_make_agg_feat_multiple_dtypes(entityset, backend):
    compare_prod = IdentityFeature(entityset['log']['product_id']).EQ('coke zero')
...
def test_make_compare_feat(entityset, backend):
    ...
    feat = log_count_feat.GT(mean_feat)
...
def test_make_agg_feat_where_count_and_device_type_feat(entityset, backend):
    ...
    compare_count = log_count_feat.EQ(1)
    compare_device_type = IdentityFeature(entityset['sessions']['device_type']).EQ(1)
...
def test_make_agg_feat_where_count_or_device_type_feat(entityset, backend):
    ...
    compare_count = log_count_feat.GT(1)
    compare_device_type = IdentityFeature(entityset['sessions']['device_type']).EQ(1)
...
def test_calc_feature_matrix(entityset):
    ...
    property_feature = IdentityFeature(entityset['log']['value']).GT(10)
