...
class Entity(BaseEntity):
    ...
    def query_by_values(self, instance_vals, variable_id=None, columns=None,
                        time_last=None, return_sorted=False, start=None,
                        end=None, random_seed=None, shuffle=False):
    ...
    def index_by_variable(self, variable_id):
        ...
        """
        ts = time.time()
        gb = self.df.groupby(variable_id)
        index = self.indexed_by[variable_id]
    ...
    def infer_variable_types(self, ignore=None, link_vars=None):
        ...
        for variable in df.columns:
            ...
            elif variable == time_index:
                if isinstance(df[variable].iloc[0], datetime):
            ...
            elif isinstance(df[variable].iloc[0], datetime):
                inferred_type = vtypes.Datetime
    ...
    def set_time_index(self, variable_id):
        ...
        if variable_id is not None:
            self.df.sort_values(variable_id, kind="mergesort", inplace=True)
    ...
    def set_id_variable(self, variable_id, unique=True):
        ...
        if unique:
            assert self.df.index.is_unique, "Index is not unqiue on dataframe"
