...
class BaseEntity(FTBase):
    ...
    def __init__(self, id, entityset, variable_types=None, name=None,
        ...
        Args:
            ...
            id_variable (Optional(str)) : Id of id variable. Ignored if entityset provided
        ...
        self.set_id_variable(id_variable)
        ...
        # todo check the logic of this. can id_var not be in variable types?
        if self.id_variable is not None and self.id_variable not in inferred_variable_types:
            self.variables.append(vtypes.Categorical(self.id_variable, self))
    ...
    def __eq__(self, other, deep=False):
        if not deep:
            ...
            if self.id_variable != other.id_variable:
                return False
    ...
    # def populate_info_from_entityset(self, entityset):
    #     self.id_variable = entityset.get_id_variable(self.id)
    #     self.time_index = entityset.get_time_index(self.id)
    #     self.secondary_time_index = entityset.get_secondary_time_index(self.id)
    #     inferred_variable_types = entityset.get_variable_types(self.id)
    #     existing = set([v.id for v in self.variables])
    #     for i, v in enumerate(self.variables):
    #         if v.id in inferred_variable_types and type(v) != inferred_variable_types[v.id]:
    #             self.variables[i] = inferred_variable_types[v.id](v.id, self)
    #     for v, vtype in inferred_variable_types.iteritems():
    #         if v not in existing:
    #             self.variables.append(vtype(v, self))

    #     self.add_variable_statistics_all(entityset)
    #     self.shape = entityset.get_dataframe(self.id).shape
    ...
    def get_all_link_vars(self):
        id_var = self.id_variable
        forward_rels = self.entityset.get_forward_relationships(self.id)
        forward_vars = [r.get_entity_variable(self.id) for r in forward_rels]
        backward_rels = self.entityset.get_backward_relationships(self.id)
        backward_vars = [r.get_entity_variable(self.id) for r in backward_rels]
        link_vars = set([id_var]) | set(forward_vars) | set(backward_vars)
        return link_vars

    # TODO dfs: used by add_parent_time_index but nowhere else
    def add_variable(self, new_id, type, entityset=None):
        """Add variable to entity

        Args:
            new_id (str) : id of variable to be added
            type (:class:`.Variable`) : class of variable
            entityset (:class:`.BaseEntitySet`) : EntitySet associated with this entity's Entityset
        """
        ...
        # self.variable_types[new_v] = type
