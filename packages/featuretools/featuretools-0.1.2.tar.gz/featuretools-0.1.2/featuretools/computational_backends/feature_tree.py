...
from featuretools.features import IdentityFeature, TransformFeature,\
    DirectFeature, AggregationFeature
from toolz import merge
from ..utils import gen_utils as utils
import logging
logger = logging.getLogger('featuretools.feature_tree')


class FeatureTree(object):
    def __init__(self, entityset, features):
        self.entityset = entityset
        self.prediction_eid = features[0].entity.id
    ...
    def _generate_feature_tree(self, features):
        """
        Given a set of features for a prediction entity, build a tree linking
        ...
        for f in features:
            all_features = merge(all_features,
                                 {g.hash(): g for g in f._get_deep_dependencies()})
        self.all_features = all_features.values()

        # find top-level features and index them by entity id.
        for f in self.all_features:
            _, num_forward = self.entityset.find_path(self.prediction_eid, f.entity.id,
                                                  include_num_forward=True)
            if num_forward or f.entity.id == self.prediction_eid:
    ...
    def _order_entities(self):
        ...
        self.ordered_entities = utils.topsort([self.prediction_eid],
    ...
    def _get_feature_depths(self, entity_id):
        ...
        while queue:
            ...
            # add all base features to the queue and continue the search
            for dep in f.base_features:
...
def _get_base_entity_id(f):
    if isinstance(f, IdentityFeature) or isinstance(f, TransformFeature):
        return f.entity.id
    else:
        # Assume all of f's base_features belong to the same entity
        return f.base_features[0].entity.id
