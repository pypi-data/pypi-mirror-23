from featuretools.features import AggregationFeature, TransformFeature, DirectFeature, IdentityFeature
from featuretools.features import Mean, Mode, Count
...
class DFSFilterBase(object):
    ...
    def post_instance_validate(self, f, e, prediction_entity_id):
        """
        Args:
            f (`.FeatureBase`): feature
            e (`.BaseEntity`): entity
            prediction_entity_id (str): id of prediction entity
        ...
        return self.apply_filter(f, e, prediction_entity_id)
    ...
    def is_valid(self, feature=None, entity=None,
                 prediction_entity_id=None, child_feature=None,
...
class TraverseUp(DFSFilterBase):
    ...
    def apply_filter(self, parent_entity, child_entity,
        ...
        if not forward:
            if (parent_entity.id == prediction_entity_id or
                    es.find_backward_path(parent_entity.id,
                                          prediction_entity_id) is None):
                return True
...
class AggFeatStack(DFSFilterBase):
    ...
    def apply_filter(self, f, e, prediction_entity_id):
