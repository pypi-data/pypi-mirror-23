...
logger = logging.getLogger('dfs.entityset')
...
class EntitySet(BaseEntitySet):
    ...
    #  Public API methods  ###################################################
    ###########################################################################
    def head(self, entity_id, n=10, variable_id=None):
        df = self.entity_stores[entity_id].head(n)
    ...
    def get_pandas_data_slice(self, filter_entity_ids, index_eid,
        ...
        for filter_eid in filter_entity_ids:
    ...
    def get_id_variable(self, entity_id):
        """
        Get name of the primary key ID column for this entity
        """
        return self.entity_stores[entity_id].id_variable
    ...
    ###########################################################################
    # TODO dfs: look for time_index_as_datetime
    def entity_from_csv(self, entity_id,
                        csv_path,
                        id_variable=None,
                        variable_types=None,
                        use_variables=None,
                        make_id_variable=False,
                        time_index=None,
                        secondary_time_index=None,
                        time_index_components=None,
                        time_index_as_datetime=None,
        ...
        Args:
            ...
            id_variable (str, optional): Name of the variable used to index the entity.
            ...
            make_id_variable (Optional(boolean)) : If True, assume id_variable does not exist as a column in
                csv, and create a new column of that name using integers the (0, len(dataframe)).
                Otherwise, assume id_variable exists in csv
            ...
            parse_date_cols (Optional(lst[str])) : list of column names to parse as datetimes
        ...
        self._import_from_dataframe(entity_id, df, id_variable=id_variable,
                                    make_id_variable=make_id_variable,
    ...
    def entity_from_dataframe(self,
        ...
        Args:
            ...
            make_id_variable (Optional(str)) : If True, assume id_variable does not exist as a column in
                csv, and create a new column of that name using integers the (0, len(dataframe)).
                Otherwise, assume id_variable exists in csv
        ...
        # TODO look into handling secondary_time_index here
        # _operations?
        self._import_from_dataframe(entity_id, dataframe.copy(), id_variable=id_variable,
                                    make_id_variable=make_id_variable,
                                    time_index=time_index,
                                    secondary_time_index=secondary_time_index,
                                    variable_types=variable_types,
    ...
    def _import_from_dataframe(self,
        ...
        Args:
            ...
            make_id_variable (Optional(str)) : If True, assume id_variable does not exist as a column in
                ...
                Otherwise, assume id_variable exists in dataframe
                An entity's variable_types dict maps string variable ids to types (:class:`.Variable`)
        ...
        if id_variable is None:
            assert not make_id_variable, "Must specify an id_variable name if make_id_variable is True"
            logger.warning(("Using first column as id variable. ",
                           "To change this, specify the id_variable parameter"))
        else:
            if id_variable not in variable_types:
                variable_types[id_variable] = vtypes.Categorical

        if make_id_variable:
            if id_variable in dataframe.columns:
                raise RuntimeError("Cannot make ID: ID variable already present")
            dataframe.insert(0, id_variable, xrange(0, len(dataframe)))
        elif id_variable is None:
            id_variable = dataframe.columns[0]
    ...
    def normalize_entity(self, base_entity_id, new_entity_id, id_variable,
        ...
        Args:
            ...
            id_variable (str): variable in old entity
                that will become id variable of new entity. Relationship
            ...
            convert_links_to_integers (bool) : If True,
                ...
                the old variable's name plus "_link"
        ...
        new_id_variable = id_variable

        if convert_links_to_integers:
            new_id_variable = self.make_id_variable_name(new_entity_id)

        transfer_types = {}
        transfer_types[new_id_variable] = type(base_entity[id_variable])
        ...
        if make_time_index:
            ...
            new_entity_df.sort_values(base_time_index, ascending=True, inplace=True)
        else:
            new_entity_time_index = None

        selected_variables = [id_variable] +\
            [v for v in additional_variables] +\
            [v for v in copy_variables]

        new_entity_df2 = new_entity_df. \
            drop_duplicates(id_variable, keep=time_index_reduce)[selected_variables]
        ...
        if make_secondary_time_index:
            ...
            secondary_variables = [id_variable, secondary_time_index] + make_secondary_time_index.values()[0]
            secondary_df = new_entity_df. \
                drop_duplicates(id_variable, keep='last')[secondary_variables]
            ...
            secondary_df.set_index(id_variable, inplace=True)
            new_entity_df = new_entity_df2.join(secondary_df, on=id_variable)
        else:
            new_entity_df = new_entity_df2

        if convert_links_to_integers:
            old_entity_df = self.get_dataframe(base_entity_id)
            link_variable_id = self.make_id_variable_name(new_entity_id)
            new_entity_df[link_variable_id] = np.arange(0, new_entity_df.shape[0])

            just_id_variable = old_entity_df[[id_variable]]
            id_as_int = just_id_variable.merge(new_entity_df,
                                               left_on=id_variable,
                                               right_on=id_variable,
                                               how='left')[link_variable_id]
            old_entity_df[id_variable] = id_as_int

            base_entity.update_data(old_entity_df)

            id_variable = link_variable_id
        ...
        transfer_types[id_variable] = vtypes.Categorical
        ...
        self.add_relationship(Relationship(new_entity[id_variable], base_entity[id_variable]))
    ...
    def add_parent_time_index(self, entity_id, parent_entity_id,
        ...
        if child_time_index_variable is None:
            child_time_index_variable = parent_time_index_variable

        path = self.find_forward_path(entity_id, parent_entity_id)
        ...
        parent_data = self.entity_stores[parent_entity_id].df[[rel.parent_variable.id, parent_time_index_variable]]
        col_map = {parent_time_index_variable: child_time_index_variable}
        ...
        parent_type = type(parent_entity[parent_time_index_variable])
        entity.add_variable(parent_time_index_variable, parent_type)
        entity.add_variable_statistics(child_time_index_variable, self)
    ...
    def add_interesting_values(self, max_values=5):
        ...
        for entity in self.entities:
            entity.add_interesting_values(max_values=max_values)
