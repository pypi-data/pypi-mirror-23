...
from featuretools.features.api import (IdentityFeature, BinaryFeature,
                                       TimeSince, DirectFeature, Compare,
                                       AggregationFeature)
import featuretools.features.api as ftypes
from .dfs_filters import (TraverseUp, AggFeatStack, LimitModeUniques,
                          WhereFeatsDifferentThanBaseFeats,
                          LimitWhereClausesByFeatureType,
                          LimitWhereClausesByVariableType,
                          DFeatWhere, WhereStack)

import logging
logger = logging.getLogger('dfs.entity')


class DeepFeatureSynthesis(object):
    """Automatically produce features for a prediction entity in an Entityset.

        Args:
            prediction_entity_id (str): id of entity to build features for
            ...
            filters (list[:class:`.DFSFilterBase`], optional) : list of dfs filters
                ...
                [:class:`synthesis.dfs_filters.TraverseUp`,
                 :class:`synthesis.dfs_filters.LimitModeUniques`,
                 :class:`synthesis.dfs_filters.LimitWhereClausesByVariableType`,
                 :class:`synthesis.dfs_filters.LimitWhereClausesByFeatureType`,
                 :class:`synthesis.dfs_filters.WhereFeatsDifferentThanBaseFeats`,
                 :class:`synthesis.dfs_filters.WhereStack`,
                 :class:`synthesis.dfs_filters.DFeatWhere`,
                 :class:`synthesis.dfs_filters.AggFeatStack`]

            agg_primitives (list[:class:`features.AggregationFeature`], optional):
            ...
            trans_primitives (list[:class:`features.TransformFeature`], optional):
            ...
            where_feat_aggs (list[:class:`features.AggregationFeature`], optional):
    ...
    def __init__(self,
        ...
        if ignore_entities is None:
            ...
            assert prediction_entity_id not in ignore_entities,\
                "Can't ignore prediction_entity!"
        ...
        self.prediction_entity_id = prediction_entity_id
        self.es = entityset

        if filters is None:
            if where_feat_aggs is None:
                where_feat_aggs = [ftypes.Count]

            filters = [TraverseUp(),
                       LimitModeUniques(),
                       LimitWhereClausesByVariableType([Categorical]),
                       LimitWhereClausesByFeatureType(where_feat_aggs),
                       WhereFeatsDifferentThanBaseFeats(),
                       WhereStack(),
                       DFeatWhere(),
                       AggFeatStack()]
        elif where_feat_aggs is not None:
            limit_where_filter = LimitWhereClausesByFeatureType(where_feat_aggs)
            if limit_where_filter not in filters:
                filters.append(limit_where_filter)

        self.pre_instance_filters = []
        self.post_instance_filters = []
        self.traversal_filters = []

        for f in filters:
            if f.filter_type == 'pre_instance':
                self.pre_instance_filters.append(f)
            elif f.filter_type == 'post_instance':
    ...
    def build_features(self, variable_types=None):
        """Automatically builds feature definitions for prediction
        ...
        Returns:
            list[:class:`features.BaseFeature`]: returns a list of
                features for prediction entity, sorted by feature depth
                (shallow first)
        """
        all_features = {}
        ...
        self._run_dfs(self.es[self.prediction_entity_id], [],
                      all_features, max_depth=self.max_depth)

        new_features = all_features[self.prediction_entity_id].values()
        ...
        if variable_types is not None:
            new_features = [f for f in new_features
                            if f.variable_type in variable_types]

        def check_secondary_index(f):
            secondary_time_index = self.es[self.prediction_entity_id].secondary_time_index
        ...
        def filt(f):
            # remove identity features of the ID field of the prediction entity
            if (isinstance(f, IdentityFeature) and
                    f.entity.id == self.prediction_entity_id and
                    f.variable.id == self.es[self.prediction_entity_id].id_variable):
                return False
        ...
        new_features.sort(key=lambda f: f._get_depth())
    ...
    def _run_dfs(self, entity, entity_path, all_features, max_depth):
        ...
        for b_entity_id in build_entities:
            ...
            if self.allowed_paths and tuple(entity_path + [b_entity_id]) not in self.allowed_paths:
                continue
            self._build_backward_features(parent_entity=self.es[entity.id],
    ...
    def _apply_traversal_filters(self, parent_entity, child_entity,
        ...
        for f in self.traversal_filters:
            if not f.is_valid(entity=parent_entity,
                              ...
                              prediction_entity_id=self.prediction_entity_id,
    ...
    def _handle_new_feature(self, new_feature, all_features):
        ...
        Returns:
            dict{:class:`.FeatureBase`: {featureid: feature}: Dict of
    ...
    def _add_identity_features(self, all_features, entity):
        ...
        for v in variables:
            if v.id in ignore_variables:
                continue
            new_f = IdentityFeature(variable=v, name=v.name)
            self._handle_new_feature(all_features=all_features,
                                     new_feature=new_f)
    ...
    def _build_transform_features(self, all_features, entity, max_depth=0):
        ...
        # if self.label_feature and entity.id == self.label_feature.entity.id:
        #     if entity.id != self.prediction_entity_id:
        #         deps = self.label_feature._get_deep_dependencies()
        #         dependent_vars = [f.variable for f in deps if isinstance(f, IdentityFeature)]
        #         if all(v.id not in self.ignore_variables[self.label_feature.entity.id]
        #                 for v in dependent_vars):
        #             # Include number of times label_feature was True in the past
        #             num_events = Sum(base_feature=self.label_feature,
        #                              parent_entity=self.es[self.prediction_entity_id])
        #             self._handle_new_feature(all_features=all_features,
        #                                      new_feature=num_events)

        #             # Include whether label_feature was ever True in the past
        #             events_exists = Compare(num_events, '>', 0)
        #             self._handle_new_feature(all_features=all_features,
        #                                  new_feature=events_exists)
        #     else:
        #         pass
        #         # TODO: possibly want to include label feature
        #         # self._handle_new_feature(all_features=all_features,
        #                                  # new_feature=self.label_feature)

        self._add_identity_features(all_features, entity)

        features = self._features_by_type(all_features, entity,
                                          variable_types.PandasTypes._all,
                                          max_depth)

        for trans_feat in self.trans_primitives:       # list of builder classes
            for f in features:
                try:
                    new_f = trans_feat(f)
                except:
                    ...
                    if new_f.can_apply(entity=entity):
                        self._handle_new_feature(all_features=all_features,
                                                 new_feature=new_f)
    ...
    def _build_forward_features(self, all_features, parent_entity,
        ...
        for f in features:
            ...
            new_f = DirectFeature(base_feature=f,
                                  child_entity=child_entity)
            self._handle_new_feature(all_features=all_features,
                                     new_feature=new_f)

    def _build_backward_features(self, all_features,
        ...
        for agg_feat in self.agg_primitives:
            for f in features:
                if self._feature_in_relationship_path(relationship_path, f):
                    continue

                wheres = [None] + list(self.where_clauses[child_entity.id])
                for where in wheres:
                    if not self._check_pre_instance_filters(agg_feat, f,
                                                            child_entity,
                                                            where=where):
                        continue

                    try:
                        new_f = agg_feat(base_feature=f,
                                         parent_entity=parent_entity,
                                         where=where)
                    except:
                        continue
                    else:
                        if new_f.can_apply(parent_entity,
                                           self.prediction_entity_id,
                                           filters=self.post_instance_filters):

                            self._handle_new_feature(new_f, all_features)

    def _check_pre_instance_filters(self, feature, child_feature,
                                    child_entity, where=None):

        for filt in self.pre_instance_filters:
            if not filt.is_valid(feature=feature, child_feature=child_feature,
                                 child_entity=child_entity, where=where):
                return False
        return True
    ...
    def _features_by_type(self, all_features, entity, variable_type, max_depth):
        ...
        for feat in all_features[entity.id]:
            ...
            if (variable_type == variable_types.PandasTypes._all or
                    ...
                    f.variable_type in variable_type):
                if ((max_depth is None or self._feature_depth(f) <= max_depth) and
    ...
    def _feature_depth(self, f):
        return f._get_depth()

    def _max_hlevel(self, f):
        # for each base_feat along each path in f,
        # if base_feat is a direct_feature of an agg_feature
        # determine aggfeat's hlevel
        # return max hlevel
        deps = [f] + f._get_deep_dependencies()
        hlevel = 0
        for d in deps:
            if isinstance(d, DirectFeature) and \
                    isinstance(d.base_feature, AggregationFeature):

                assert d.parent_entity.id == d.base_feature.entity.id
                path, new_hlevel = self.es.find_path(self.prediction_entity_id,
                                                     d.parent_entity.id,
