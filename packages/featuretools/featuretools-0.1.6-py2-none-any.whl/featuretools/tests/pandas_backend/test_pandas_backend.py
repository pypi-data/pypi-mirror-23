...
from featuretools.primitives import (IdentityFeature, DirectFeature, Sum,
                                     ...
                                     TopNMostCommon, AggregationFeature)
...
def test_cfm_approximate_correct_ordering():
    trips = {
        ...
        'flight_time': [datetime(1997, 4, 1) for i in xrange(350)] + [datetime(1997, 4, 2) for i in xrange(650)],
        'flight_id': [randint(0, 25) for i in xrange(1000)],
        'trip_duration': [randint(0, 999) for i in xrange(1000)]
    }
    ...
    flight_features = [feature for feature in features
                       ...
                       isinstance(feature.base_feature,
                                  AggregationFeature)]
    ...
    cutoff_time = pd.DataFrame.from_dict({'instance_id': df['trip_id'],
                                          'time': df['flight_time']})
    feature_matrix = calculate_feature_matrix(flight_features + [property_feature],
                                              cutoff_time=cutoff_time)
    assert((feature_matrix.index == feature_matrix['trip_id']).all())
    feature_matrix_2 = calculate_feature_matrix(flight_features + [property_feature],
                                                cutoff_time=cutoff_time,
                                                approximate=Timedelta(1, 'd'))
    assert((feature_matrix_2.index == feature_matrix_2['trip_id']).all())
...
def test_saveprogress(entityset):
    ...
    fm_no_save = calculate_feature_matrix([property_feature],
                                          instance_ids=range(17),
                                          cutoff_time=times)
    assert merged_df.sort_index().equals(fm_save.sort_index())
    assert fm_no_save.sort_index().equals(fm_save.sort_index())
    assert fm_no_save.sort_index().equals(merged_df.sort_index())
...
def test_cutoff_time_binning(entityset):
    ...
    labels = [datetime(2011, 4, 9, 11),
              datetime(2011, 4, 9, 11),
              datetime(2011, 4, 10, 12)]
    for i in binned_cutoff_times.index:
        assert binned_cutoff_times['time'][i] == labels[i]
