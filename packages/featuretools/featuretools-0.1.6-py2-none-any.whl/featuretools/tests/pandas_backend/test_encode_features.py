...
def test_to_encode_features(entityset):
    ...
    to_encode = ['purchased']
    features_encoded, feature_matrix_encoded = encode_features(features, feature_matrix, to_encode=to_encode)
    assert feature_matrix_encoded_shape != feature_matrix_encoded.shape

    to_encode = ['product_id', 'purchased']
    features_encoded, feature_matrix_encoded = encode_features(features, feature_matrix, to_encode=to_encode)
    assert feature_matrix_encoded_shape != feature_matrix_encoded.shape

    to_encode = ['product_id', 'purchased', 'value']
    features_encoded, feature_matrix_encoded = encode_features(features, feature_matrix, to_encode=to_encode)
    assert feature_matrix_encoded_shape != feature_matrix_encoded.shape
