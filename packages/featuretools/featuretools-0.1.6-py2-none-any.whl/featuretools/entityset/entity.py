...
class Entity(BaseEntity):
    ...
    def __init__(self, id, df, entityset, variable_types=None, name=None,
                 ...
                 encoding=None, relationships=None, already_sorted=False, verbose=False):
        """ Create Entity
        ...
        self.attempt_cast_id_to_int(id_variable)
    ...
    def attempt_cast_id_to_int(self, id_var):
        dtype_name = self.df[id_var].dtype.name
        if (dtype_name.find('int') == -1 and
                dtype_name.find('object') > -1 or dtype_name.find('categ') > -1):
            if isinstance(self.df[id_var].iloc[0], (int, np.int32, np.int64)):
                try:
                    self.df[id_var] = self.df[id_var].astype(int)
                except ValueError:
                    pass
    ...
    def sample_instances(self, n, random_seed=None):
        ...
        return instance_df[self.id_variable].sample(n, random_state=random_seed).values
    ...
    def set_time_index(self, variable_id, already_sorted=False):
        # use stable sort
        if variable_id is not None and not already_sorted:
            # sort by time variable, then by id variable
            self.df.sort_values([variable_id, self.id_variable],
                                kind="mergesort",
                                inplace=True)
