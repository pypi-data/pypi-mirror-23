...
class PandasBackend(ComputationalBackend):
    def __init__(self, entityset, features, precalculated_features=None, ignored=None):
        ...
        self.precalculated_features = precalculated_features or {}
        self.ignored = ignored or []
        self.feature_tree = FeatureTree(entityset, features, ignored=self.ignored)
    ...
    def calculate_all_features(self, instance_ids, time_last,
        ...
        if len(self.precalculated_features) > 0:
            for entity, values in self.precalculated_features[time_last].items():
                for entity_frames in eframes_by_filter.values():
                    if entity in entity_frames:
                        entity_frames[entity] = pd.merge(entity_frames[entity],
                                                         values,
                                                         left_index=True,
                                                         right_index=True)
        ...
        for filter_eid in ordered_entities:
            ...
            if filter_eid in self.feature_tree.ordered_feature_groups:
                for group in self.feature_tree.ordered_feature_groups[filter_eid]:
                    if verbose:
                        pbar.set_postfix({'running': len(group)})
                    ...
                    for f in group:
                        if f.requires_current_time and f.time is None:  # used for TimeSince
                            ...
                            set_time_feats.append(f)
                    handler(group, entity_frames)
                    for f in set_time_feats:
                        f.time = None
                    if verbose:
    ...
    def _calculate_agg_features(self, features, entity_frames):
        ...
        if frame[id_var].dtype != base_frame[groupby_var].dtype:
            try:
                frame_as_obj = frame[id_var].astype(object)
                base_frame_as_obj = base_frame[groupby_var].astype(object)
            except ValueError:
                msg = "Could not join {}.{} (dtype={}) with {}.{} (dtype={})"
                raise ValueError(msg.format(entity.id, id_var,
                                            frame[id_var].dtype,
                                            child_entity.id, groupby_var,
                                            base_frame[groupby_var].dtype))
            else:
                no_instances = frame_as_obj.isin(base_frame_as_obj).any()
        else:
            no_instances = frame[id_var].isin(base_frame[groupby_var]).any()

        if base_frame.empty or not no_instances:
            for f in features:
