...
from featuretools.primitives.api import (IdentityFeature, BinaryFeature,
...
class DeepFeatureSynthesis(object):
    """Automatically produce features for a target entity in an Entityset.
        ...
        Args:
            ...
            where_primitives (list[:class:`features.AggregationFeature`], optional):
    ...
    def __init__(self,
        ...
        if filters is None:
            filters = [TraverseUp(),
                       ...
                       LimitWhereClausesByFeatureType(where_primitives),
                       WhereFeatsDifferentThanBaseFeats(),
                       WhereStack(),
                       DFeatWhere(),
                       AggFeatStack()]
    ...
    def build_features(self, variable_types=None):
        ...
        def filt(f):
            ...
            if (isinstance(f, IdentityFeature) and
                    ...
                    f.variable.id == self.es[self.target_entity_id].id_variable):
                return False
    ...
    def _build_transform_features(self, all_features, entity, max_depth=0):
        ...
        relation_vars = []
        for r in entity.entityset.get_forward_relationships(entity.id):
            relation_vars.append(r.child_variable)

        # TODO:
        # should we include if not in path to target entity?

        for trans_prim in self.trans_primitives:       # list of builder classes
            if entity.has_time_index() and trans_prim.entity_groupby and trans_prim.require_time_index:
                for rv in relation_vars:
                    new_f = trans_prim(entity[entity.time_index], group_feature=rv)
                    if new_f.can_apply(entity=entity):
                        self._handle_new_feature(all_features=all_features,
                                                 new_feature=new_f)

            for f in features:
                if trans_prim.entity_groupby:
                    # this handles the TimBetwe
                    if trans_prim.require_time_index:
                        continue
                    for rv in relation_vars:
                        try:
                            new_f = trans_prim(f, group_feature=rv)
                        except:
                            pass
                        if f.expanding:
                            continue
                        elif new_f.can_apply(entity=entity):
                            self._handle_new_feature(all_features=all_features,
                                                     new_feature=new_f)
                else:
                    try:
            ...
            for f in features:
                ...
                for where in wheres:
                    ...
                    try:
                        ...
                        if f.expanding:
                            continue
                        elif new_f.can_apply(parent_entity,
