...
def encode_features(features, feature_matrix, top_n=10, include_unknown=True,
    ...
    encoded = []
    to_encode = to_encode or []
    for f in features:
        if not (f.variable_type == Categorical or f.get_name() in to_encode):
            encoded.append(f)
            continue
