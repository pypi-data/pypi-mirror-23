...
def create_remote_file(current_host, username, keyfile, file_location, file_contents, file_mode, file_owner='default', port=22, use_sudo=False, print_stdout=True):
    ...
    # create file
    ssh_cmd(current_host=current_host, command=command_dict['create_file_command'], username=username, keyfile=keyfile, port=port, print_stdout=print_stdout)
    # execute file mode
    ssh_cmd(current_host=current_host, command=command_dict['file_mode_command'], username=username, keyfile=keyfile, port=port, print_stdout=print_stdout)
    # change owners if requested
    if file_owner != 'default':
        ssh_cmd(current_host=current_host, command=command_dict['file_owner_command'], username=username, keyfile=keyfile, port=port, print_stdout=print_stdout)

def create_remote_file_multi_try(current_host, username, keyfile, file_location, file_contents, file_mode, file_owner='default', port=22, use_sudo=False, print_stdout=True, trys=10):
    """runs a commad via ssh on the host: current_host and returns a dictionary of the results
    :type current_host: string
    :param current_host: the host you want to run your command on
    :type username: string
    ...
    # create file
    ssh_cmd_multi_try(current_host=current_host, command=command_dict['create_file_command'], username=username, keyfile=keyfile, port=port, print_stdout=print_stdout, trys=trys)
    # execute file mode
    ssh_cmd_multi_try(current_host=current_host, command=command_dict['file_mode_command'], username=username, keyfile=keyfile, port=port, print_stdout=print_stdout, trys=trys)
    # change owners if requested
    if file_owner != 'default':
        ssh_cmd_multi_try(current_host=current_host, command=command_dict['file_owner_command'], username=username, keyfile=keyfile, port=port, print_stdout=print_stdout, trys=trys)
...
def ssh_cmd(current_host, command, username, keyfile, port=22, print_stdout=True, use_sudo=False):
    """runs a commad via ssh on the host: current_host and returns a dictionary of the results
    :type current_host: string
    :param current_host: the host you want to run your command on
    :type command: string
...
def ssh_cmd_multi_try(current_host, command, username, keyfile, port=22, print_stdout=True, use_sudo=False, trys=10):
    ...
    while actual_trys != trys:
        ...
        print(attempts_message)
        try:
            ssh_cmd(current_host=current_host, command=command, username=username, keyfile=keyfile, port=port, print_stdout=print_stdout, use_sudo=use_sudo)
        except (NoValidConnectionsError)  as e:
        # should we catch socket.errors too?
        #except (NoValidConnectionsError, socket.error)  as e:
            time.sleep(5)
...
def ssh_cmd_auto_check(current_host, command, username, keyfile, port=22, print_stdout=True, use_sudo=False):
    ...
    :param use_sudo: Wether to prefix all commands with 'sudo'
    """
    ssh_cmd_output = ssh_cmd(current_host, command, username=username, keyfile=keyfile, port=port, print_stdout=True, use_sudo=use_sudo)
