...
class Wcc:
    ...
    #每个book表示一个妹子的写真集
    #how有两种可能:new/all
    def __getArtbooks(self,www_from,how,thread_count):
        ...
        try:
            ...
            while True:
                ...
                if cur_art_inc != 0:
                    ...
                    if len(articles) > 0:
                        self.AddArticles(articles,str(cur_art_inc)+"/"+tid_from,thread_id)
                else:
    ...
    #如果是获取新文章,根本不用保存本地
    #所有操作全部在内存进行
    def __getNewArticles(self,www_from,max_pmt,max_wmt,max_run_seconds):
        Wcc.setParam(www_from,"artbook_inc",0)
        books = self.__getArtbooks(www_from,"new",max_pmt)
        if max_wmt < 1:
            self.__addArtbookSerial(www_from,books,max_run_seconds)
        else:
            self.__addArtbookParallel(www_from,books,max_wmt,max_run_seconds)
        return
    def __getAllArticles(self,www_from,max_pmt,max_wmt,max_run_seconds):
        Wcc.setParam(www_from,"artbook_inc",0)
        books = self.__getArtbooks(www_from,"all",max_pmt)
        if max_wmt < 1:
            self.__addArtbookSerial(www_from,books,max_run_seconds)
        else:
            self.__addArtbookParallel(www_from,books,max_wmt,max_run_seconds)
    ...
    --pmt: 页面爬虫的线程数, 爬虫第一步,页面爬取的线程数目,默认1
    ...
    """
        ...
        param_lot  = 0
        try:
            opts, args = getopt.getopt(argv[1:], 'hvm:', ['help', 'version', 'mode','pmt=','wmt=','lot='])
        except Exception as err:
            print(err)
            print(usage)
            return False
        ...
        if  crawl_type == "NewArticle":
            self.__getNewArticles(www_from,max_pmt,max_wmt,max_run_seconds)
        elif crawl_type == "AllArticle":
            self.__getAllArticles(www_from,max_pmt,max_wmt,max_run_seconds)
