...
class Wcc:
    ...
    """
        ...
        param_wmt  = 8
        param_lot  = 0
        param_books_file = www_from+".books.pdb"
        try:
            opts, args = getopt.getopt(argv[1:], 'hvm:', ['help', 'version', 'mode','pmt=','wmt=','lot=','books='])
        except Exception as err:
            print(err)
            print(usage)
            return False
    ...
    def make_fitem(farray):
        ...
        if "Images/Images/"  in file_localpath:
            print("出错的file_localpath:"+file_localpath)
            print(traceback.print_exc())
            return None
