...
class ConfigValue(object):
    ...
    def pre_collectstatic(self, merger, value):
        """Called before the "collectstatic" command"""
        pass
...
class File(Path):
    ...
    def pre_collectstatic(self, merger, value):
        ...
        if not os.path.isdir(dirname):
            os.makedirs(dirname)
        if self.mode is not None and (os.stat(value).st_mode & 0o777) != self.mode:
            os.chmod(value, self.mode)
...
class AutocreateFileContent(File):
    ...
    def __init__(self, value, create_function, mode=None, *args, **kwargs):
        ...
        :param create_function: called when the file does not exist.
            Must return a text string. First arg is a bool (True if django is ready, False otherwise)
        ...
        Django is ready when your create function is called during the 'pre_migrate' phase.
        Django is not ready when your create function is called during the configuration loading phase.
        """
