...
def resumable_upload(bucket, key, filename,
    ...
    实现中采用分片上传方式上传本地文件，缺省的并发数是 `prox_oss2.defaults.multipart_num_threads` ，并且在
    ...
    :param num_threads: 并发上传的线程数，如不指定则使用 `prox_oss2.defaults.multipart_num_threads` 。
    """
...
def resumable_download(bucket, key, filename,
    ...
    使用该函数应注意如下细节：
        ...
        #. 避免使用太小的范围（分片），即 `part_size` 不宜过小，建议大于或等于 `prox_oss2.defaults.multiget_part_size` 。
    ...
    :param num_threads: 并发下载的线程数，如不指定则使用 `prox_oss2.defaults.multiget_num_threads` 。

    :param store: 用来保存断点信息的持久存储，可以指定断点信息所在的目录。
    :type store: `ResumableDownloadStore`

    :raises: 如果OSS文件不存在，则抛出 :class:`NotFound <prox_oss2.exceptions.NotFound>` ；也有可能抛出其他因下载文件而产生的异常。
    """
