...
class AppleLiveGroupSettings(Model):
    ...
    segment_length = GenericField('segment_length', default=10)
    min_segment_length = GenericField('min_segment_length', default=0)
    emit_single_file = GenericField('emit_single_file', default=False)
    floating_point_manifest = GenericField('floating_point_manifest',
                                           default=True)
    include_resolution = GenericField('include_resolution', default=True)
    compress_manifests = GenericField('compress_manifests', default=False)
    use_subdirectories = GenericField('use_subdirectories', default=False)
    ...
    connection_retry_interval = GenericField('connection_retry_interval',
                                             default=2)
    generate_meta_file = GenericField('generate_meta_file', default=True)
    vod_mode = GenericField('vod_mode', default=False)
    num_retries = GenericField('num_retries', default=10)
    ...
    key_rotation_count = GenericField('key_rotation_count', default=3)
    show_iv = GenericField('show_iv', default=True)
    ...
    disable_cache = GenericField('disable_cache', default=False)
