...
class Output(Model):
    ...
    ebif_passthrough = GenericField('ebif_passthrough', default=False)
    extension = GenericField('extension')
    f4v_settings = GenericField('f4v_settings')
    full_uri = GenericField('full_uri')
    id = GenericField('id')
    insert_scte35_esam = GenericField('insert_scte35_esam', default=False)
    insert_timed_metadata = GenericField('insert_timed_metadata', default=False)
    klv_passthrough = GenericField('klv_passthrough', default=False)
    log_edit_points = GenericField('log_edit_points', default=False)
    ...
    scte35_passthrough = GenericField('scte35_passthrough', default=False)
    stream_assembly_name = GenericField('stream_assembly_name')
    uvu_settings = GenericField('uvu_settings')
    nielsen_id3_passthrough = GenericField(
        'nielsen_id3_passthrough', default=False)
