...
def update_config_file_key_value(file, section, key_name, key_value):
    ...
    if isinstance(key_value,int) or '%' not in key_value:
        ...
        if not re.search(r"\n\[%s\]\n" % section.replace(".","\."),content,re.I):
            ...
            #有这个域名section
            newcontent=re.sub(r"\[%s\]\n%s = .+" % (section,key_name),"[%s]\n%s = %s" %
                    (section,key_name,key_value),content)
            os.system("rm %s" % file)
            with open(file,"a+") as f:
                f.write(newcontent)
...
def get_key_value_from_config_file(file, section, key_name):
    ...
    with open(file,"r+") as f:
        content=f.read()
    if not re.search(r"%",content,re.I):
        import configparser
        config = configparser.ConfigParser()
        config.read(file)
        ...
        find=re.search(r"\[%s\]\n+(.+\n+)*%s[^\n\S]*\=[^\n\S]*(.*)" % (section.replace(".","\."),key_name),content)
        if find:
            return find.group(2)
        else:
            print("can not find the section:%s" % section)
            return None
...
def get_url_belong_main_target_domain(url):
    ...
    if tmp is None:
        ...
        sub_main = tmp['domain']
    if flag1==False and sub_main is not None:
        return sub_main
    else:
...
def sqlmap_crawl(origin_http_url_or_file, tor_or_not, post_or_not):
    ...
    if re.match("(http://)|(https://)", origin_http_url_or_file):
        ...
        sqlmap_string='''/usr/share/sqlmap/sqlmap.py -u "%s" --crawl=5 --crawl-exclude "(logout)|(logoff)|(exit)|(signout)|(signoff)" --delay %d --smart -v 4 --threads 4 --batch --random-agent --safe-url "%s" --safe-freq 1 --tamper=between,space2randomblank,randomcase,xforwardedfor,charencode --level 3''' % (
