...
{
	// config buffers
	d->last_sample = (unsigned int*)util_malloc(sizeof(unsigned int)*o->rawSenor_n, 8);  // (locked) last whole sample
...
void cGlove_update(cgData* d, cgOption* o)
{
	...
	while(d->updateGlove)
	{
		...
		// update
		d->cgGlove.lock();
		memcpy(d->last_sample, &inputSample.front(), persistentGlove->SampleSize() * sizeof(unsigned int));
...
// get the length of the raw samples.
int cGlove_getSampleSize(void)
{
	return persistentGlove->SampleSize();
}

// get a raw sample from the glove  (Note: not performant)
void cGlove_getSample(unsigned int *buff, const int n_buff)
{
	int n = cGlove_getSampleSize();
	if(n_buff < n)
	{
		printf("Warning:: Buffer too small to update. Minimum size should be %d", n);
		return;
	}
	cgdata.cgGlove.lock();
	memcpy(buff, cgdata.last_sample, n * sizeof(unsigned int));
	cgdata.cgGlove.unlock();
}
