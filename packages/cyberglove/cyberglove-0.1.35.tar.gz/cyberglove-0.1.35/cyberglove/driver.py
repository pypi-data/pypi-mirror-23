...
def find_port(timeout=None):
    ...
    while len(results) == 0:
        ...
        print([(p.device, p.hwid) for p in ports])
...
class CyberGlove():
    ...
    def read2shadow(self):
        ...
        if self._mapping is None:
            ...
            mapping_file = os.path.join(dirname, "..", "data", "mapping_%s.pkl" % OPENAI_USER)
            print("Loading cyberglove mapping from: %s" % mapping_file)
            if os.path.exists(mapping_file):
                with open(mapping_file, "rb") as f:
                    self._mapping = pickle.load(f)
            else:
                print("You have to record calibration for cyberglove.\n")
                print("Execute: ...")
                exit(-1)
        return dict(zip(joint_names, self._mapping.map(self.read())))
