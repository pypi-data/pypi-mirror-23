...
// TODO(aray): incorporate this somewhere useful
	//if(value == 'e')
	//{// Result: Error
	//    value = ReadByte();
	//    SynchInput(value);
...
#else  // Mac/Linux/Posix Serial Port
	port = open( portName.c_str(), O_RDWR );
...
void CyberGlove::Reset(bool hardwareReset)
{
	...
	// aray: we trust the python module to reset the hardware correctly
	// if(hardwareReset)
	// {
...
unsigned char CyberGlove::ReadByte()
{
	...
	if(!ReadFile(port, &result, 1, &bytesRead, NULL) || bytesRead != 1)
		throw std::runtime_error("Could not read byte from serial-port");
#else
	if(read(port, &result, 1) != 1)
		throw std::runtime_error("Could not read byte from serial-port");
#endif
	return result;
}
...
void CyberGlove::WriteByte(unsigned char value)
{
	...
	if(!WriteFile(port, &value, 1, &bytesWritten, NULL))
		throw std::runtime_error("Could not write byte to serial-port");
#else
	if(write(port, &value, 1) != 1)
		throw std::runtime_error("Could not write byte to serial-port");
#endif
}
...
{
	// TODO(aray): flush serial instead by switching to nonblocking and back
	int i=0;
	static unsigned char purged[1000];
	while(value)
	{
		printf("Spinning to sync input\n");
		value = ReadByte();
		purged[i] = value;
		i = i<999? i+1:i;
		//printf("CG:>\t\%c", value);
...
void CyberGlove::ClearInput(void)
{
	...
	do
	{
		...
		if(!ReadFile(port, nul, sizeof(nul), &bytesRead, NULL))
			throw std::runtime_error("Could not clear input from serial-port");
#else
		bytesRead = read(port, nul, sizeof(nul));
		if(bytesRead < 0)
			throw std::runtime_error("Could not clear input from serial-port");
#endif
	} while(bytesRead == sizeof(nul));
}
...
void CyberGlove::WriteCommand(const char* command)
{
	...
	if(!WriteFile(port, command, length, &bytes, NULL) || bytes != length)
		throw std::runtime_error("Could not write command to serial-port");
#else
	if(write(port, command, length) != length)
		throw std::runtime_error("Could not write command to serial-port");
#endif
