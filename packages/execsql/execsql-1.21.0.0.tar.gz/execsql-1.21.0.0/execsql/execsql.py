...
class Mailer(object):
	...
	def sendmail(self, send_from, send_to, subject, msg_content, content_filename=None, attach_filename=None):
		...
		if attach_filename is not None:
			...
			msg.attach(fdata)
		#print("Mailer.sendmail(); msg.as_string = %s" % msg.as_string())
		self.smtpconn.sendmail(send_from, send_to, msg.as_string())
...
class PostgresDatabase(Database):
	...
	def import_file(self, schema_name, table_name, csv_file_obj, skipheader):
		...
		curs.close()
		data_table_cols = [d[0].lower() for d in data_table_desc]
		...
		if len(unmatched_cols) > 0:
			raise ErrInfo(type="error", other_msg=u"The following columns are in the input file %s but not in the table %s: %s" % (csv_file_obj.csvfname, sq_name, ", ".join(unmatched_cols)))
		input_col_list = ",".join(csv_file_cols)
		...
		if enc_match or csv_file_obj.encoding.lower() in pg_encodings:
			...
			columns = [self.type.quoted(d) for d in csv_file_cols]
			colspec = ",".join(columns)
...
class MySQLDatabase(Database):
	...
	def import_file(self, schema_name, table_name, csv_file_obj, skipheader):
		...
		unmatched_cols = list(set([c.lower() for c in csv_file_cols]) - set(data_table_cols))
		if len(unmatched_cols) > 0:
			raise ErrInfo(type="error", other_msg=u"The following columns are in the input file %s but not in the table %s: %s" % (csv_file_obj.csvfname, sq_name, ", ".join(unmatched_cols)))
		input_col_list = ",".join(csv_file_cols)
...
class StrTemplateReport(object):
	...
	def write_report(self, data_dict_rows, output_dest, append=False):
	...
	def write_report(self, data_dict_rows, output_dest, append=False):
		if output_dest == 'stdout':
			...
			if append:
				...
				ofile = EncodedFile(output_dest, conf.output_encoding).open("w")
		ofile.write(self.template.render(datatable=data_dict_rows))
	...
	def write_report(self, data_dict_rows, output_dest, append=False):
		...
		if output_dest == 'stdout':
			...
			if append:
				...
				ofile = EncodedFile(output_dest, conf.output_encoding).open("w")
		try:
			ofile.write(self.template.merge({'datatable': data}))
...
metacommands.append(MetaCommand(r'\s*METACOMMAND_ERROR_HALT\s+(?P<onoff>ON|OFF)\s*$', x_metacommand_error_halt))
...
# email address rx: r"^[A-Za-z0-9\-\.!#$%&'*+/=?^`{|}~]+@[A-Za-z0-9]+(-[A-Za-z0-9]+)*(\.[A-Za-z0-9]+)*$"
metacommands.append(MetaCommand(ins_fn_rxs(ins_fn_rxs(r'^\s*EMAIL\s+'
							     r'FROM\s+(?P<from>[A-Za-z0-9\-\.!#$%&\'*+/=?^`{|}~]+@[A-Za-z0-9]+(-[A-Za-z0-9]+)*(\.[A-Za-z0-9]+)*)\s+'
								 r'TO\s+(?P<to>[A-Za-z0-9\-\.!#$%&\'*+/=?^`{|}~]+@[A-Za-z0-9]+(-[A-Za-z0-9]+)*(\.[A-Za-z0-9]+)*([;,]\s*[A-Za-z0-9\-\.!#$%&\'*+/=?^`{|}~]+@[A-Za-z0-9]+(-[A-Za-z0-9]+)*(\.[A-Za-z0-9]+)*)*)\s+'
...
def report_query(select_stmt, db, outfile, template_file, append=False):
	...
	ddict = db.select_rowdict(select_stmt)
	...
	t.write_report(ddict, outfile, append)
