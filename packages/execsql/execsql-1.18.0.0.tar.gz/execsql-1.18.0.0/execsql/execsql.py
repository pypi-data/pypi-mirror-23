...
def exception_info():
	...
	if isinstance(exc_param, basestring):
		...
		if hasattr(exc_param, 'message') and isinstance(exc_param.message, basestring):
			exc_message = exc_param.message
		elif hasattr(exc_param, 'value') and isinstance(exc_param.value, basestring):
			exc_message = exc_param.value
		elif hasattr(exc_param, 'exception') and isinstance(exc_param.exception, basestring):
			exc_message = exc_param.exception
		else:
...
class PostgresDatabase(Database):
	...
	def import_file(self, schema_name, table_name, csv_file_obj, skipheader):
		...
		if enc_match or csv_file_obj.encoding.lower() in pg_encodings:
			...
			copy_cmd = "copy %s from stdin with (format csv, null '', delimiter '%s'" % (sq_name, delim)
			if csv_file_obj.quotechar:
...
def old_export_html(outfile, hdrs, rows, append=False, querytext=None, desc=None):
	global conf
	script, lno = working_script.current_script_line()
	ef = EncodedFile(outfile, conf.output_encoding)
	if append:
