...
def main():
    ...
    for fname in all_files:
        ...
        with codecs.open(fname, 'rb') as filehandle:
            rsp = requests.post(urljoin(api_url, 'uploads'), files={'file': filehandle}, headers=auth)
            try:
    ...
    tag_details = requests.get(url, headers=auth).json()
    ...
    rsp = method(url + '/release', data={'description': description}, headers=auth)
    try:
        rsp.raise_for_status()
    except Exception as ex:
