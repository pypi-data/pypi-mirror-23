...
def main():
    ...
    print("Uploading %s" % list(all_files))

    error = []
    form = MultiPartForm()

    for fname in all_files:
        filehandle = codecs.open(fname, 'rb'):
    ...
    try:
        ...
        print("Failed in initial upload communication to %s" % request.full_url)
        error.append(ex)
        continue
    ...
    try:
        ...
        if '409' in str(ex):
            print("Failed posting upload due to file conflict")
            print(data)
        else:
    ...
    if error:
        raise Exception("Finished with errors")
