...
# Try and locate a working dcm4ch3e program, raising ImportError if we can't
...
try:
    with open(os.devnull, 'w') as null:
        status = subprocess.call([FINDSCU, '-V'], shell=USESHELL, stdout=null, stderr=null)
        if status != 0:
            raise NotImplementedError(FINDSCU)
    with open(os.devnull, 'w') as null:
        status = subprocess.call([GETSCU, '-V'], shell=USESHELL, stdout=null, stderr=null)
        if status != 0:
            raise NotImplementedError(GETSCU)
except OSError as e:
