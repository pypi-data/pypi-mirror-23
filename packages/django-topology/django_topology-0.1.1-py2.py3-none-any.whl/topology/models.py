...
class TopologyRule(models.Model):
    ...
    def validate(self, feature, **kwargs):
        ...
        errors = validation_method(self, feature)
...
class TopologyError(models.Model):
    ...
    def __str__(self):
        if self.object_id_a and self.object_id_b:
            return _('%(model_a)s %(rule)s %(model_b)s') % {'model_a': self.object_a.pk,
                                                            'rule': self.topology_rule.rule.message,
                                                            'model_b': self.object_b.pk}
        else:
            return _('%(model_a)s %(rule)s') % {'model_a': self.object_a.pk,
                                                'rule': self.topology_rule.rule.message}
    class Meta:
