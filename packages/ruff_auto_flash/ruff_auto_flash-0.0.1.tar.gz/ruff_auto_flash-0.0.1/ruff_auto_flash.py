...
class Flash(object):
    ...
    def _update_ruffimage(self):
        self._write_wait('tftp 0x80200000 {}'.format(self.conf['ruffimage']))
        self._write_wait('nand erase 0x780000 0xF880000', r'Erasing at.*100% complete')
        self._write_wait('nand write 0x80200000 0x780000 0x1be0000', r'\d+ bytes written: OK')
...
@click.option('--parity', default=DefaultConfig['parity'], help='serial port parity, default is "{}"'.format(DefaultConfig['parity']))
...
def main(**kwargs):
    ...
    cprint('\r\nUpdate completed, ruff version is {}'.format(ver_str), 'green')
