...
def cli(ctx, user, addon, project):
    ...
    if not user:
        for _sec in config_user_obj.sections:
            default = 'default' in config_user_obj[_sec].keys() and config_user_obj.get(_sec,
                                                                                                 'default') or False
            if default:
                user = _sec
    if not addon:
        for _sec in config_addon_obj.sections:
            default = 'default' in config_addon_obj[_sec].keys() and config_addon_obj.get(_sec,
                                                                                                   'default') or False
            if default:
                addon = _sec
    if not project:
        for _sec in config_project_obj.sections:
            default = 'default' in config_project_obj[_sec].keys() and config_project_obj.get(_sec,
                                                                                                       'default') or False
            if default:
                project = _sec
    ctx.obj['user'] = user
