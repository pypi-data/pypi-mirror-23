...
is_odoo = os.environ.get('ODOO_OPENERP', 'odoo').strip().lower() == 'odoo' and True or False
ODOO = is_odoo and 'odoo' or 'openerp'

MANIFEST_FILE = is_odoo and '__manifest__.py' or '__openerp__.py'
        ...
        if xml:
            ...
            if hasattr(xml, '__iter__'):
                for model_class in xml:
                    ...
                    create_file([xml_file], add_content='<?xml version="1.0" encoding="UTF-8"?>', condition='<?xml', test_on_plat=True)
                    create_file([xml_file], add_content='<templates xml:space="preserve">', condition='templates', test_on_plat=True)
        ...
        if js_backend:
            ...
            if hasattr(js_backend, '__iter__'):
                for model_class in js_backend:
                    ...
                    create_file([js_file], add_content='odoo.define(\'%s.%s\', function(require){' % (all_keys.get('addon_slug','addon'), model_filename))
    ...
    if kwargs.get('models', []):
        ...
        if hasattr(models, '__iter__'):
            for model in models:
                ...
                create_file([addon_path, MODELS, model_filename_ext], add_content='class %s(models.Model):' % model_class)
    ...
    if kwargs.get('wizard', []):
        ...
        if hasattr(wizard, '__iter__'):
            for model in wizard:
                ...
                create_file([addon_path, WIZARD, model_filename_ext],
                            add_content='from %s import models, fields, api, _' % ODOO)
                create_file([addon_path, WIZARD, model_filename_ext], add_content='class %s(models.TransientModel):' % model_class)
        ...
        if hasattr(parser, '__iter__'):
            for model in parser:
                ...
                create_file([addon_path, REPORT, model_filename_ext],
                            add_content='from %s import models, fields, api, _' % ODOO)
                create_file([addon_path, REPORT, model_filename_ext], add_content='class %s(models.AbstractModel):' % model_class)
                create_file([addon_path, REPORT, model_filename_ext], add_content='    _name = \'report.%s\'' % model_dot)
                create_file([addon_path, REPORT, model_filename_ext], add_content='    @api.model')
                create_file([addon_path, REPORT, model_filename_ext], add_content='    def render_html(self, docids, data=None):', )
                create_file([addon_path, REPORT, model_filename_ext], add_content='        self.model = self.env.context.get(\'active_model\')', )
                create_file([addon_path, REPORT, model_filename_ext], add_content='        docs = self.env[self.model].browse(self.env.context.get(\'active_ids\', []))', )
                ...
                create_file([addon_path, REPORT, model_filename_ext], add_content='            \'doc_model\': self.model,', )
                ...
                create_file([addon_path, REPORT, model_filename_ext], add_content='        return self.env[\'report\'].render(\'%s\', docargs)' % model_dot, )
    ...
    if click.confirm('Continue to patch the addon %s in the project %s' % (addon_slug, project_path)):
        ...
        for item in fuzzy:
            ...
            if item.strip().lower().replace('_','').replace('s','') in ['inheritwizard', 'wizardinherit']:
                item = 'inherit_wizard'
            if item in ['models', 'inherit_models', 'views', 'inherit_views', 'wizard', 'inherit_wizard', 'data', 'controllers', 'security', 'i18n', 'tests', 'icon',
                        'description', 'readme', 'js_frontend', 'css_frontend', 'js_backend', 'css_backend', 'xml', 'report', 'parser']:
...
def project_models(ctx):
    ...
    for f in x.field_names:
        x.align[f] = 'l'
    for model in astobj.get_models():
        x.add_row([model[2], model[1], model[3]])
    click.echo(x)
