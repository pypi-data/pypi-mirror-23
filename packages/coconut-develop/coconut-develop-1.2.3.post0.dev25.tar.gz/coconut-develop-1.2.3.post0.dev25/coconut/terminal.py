...
def format_error(err_type, err_value, err_trace=None):
    ...
    if err_trace is None:
        try:
            ...
            print("".join(traceback.format_exception_only(err_type, err_value)))
            print("".join(traceback.format_exception_only(err_type, err_value)).strip())
            print("".join(traceback.format_exception_only(err_type, err_value)).strip().split(": "), 1)
