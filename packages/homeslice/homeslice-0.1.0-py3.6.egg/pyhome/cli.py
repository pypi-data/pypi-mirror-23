...
from . import settings, git, symlink
...
def do_repo_list():
    ...
    pattern = os.path.join(settings.HOMESLICE_REPO, '*', '.git')
...
def do_repos_from_arguments(all, repos):
    ...
    if all:
        ...
        if len(repos) == 0:
            ...
            click.echo('    Repo dir: {}'.format(settings.HOMESLICE_REPOS))
            sys.exit(1)
    ...
    return [os.path.join(settings.HOMESLICE_REPO, r) for r in repos]
...
def clone(url, name, submodules):
    ...
    if not os.path.exists(settings.HOMESLICE_REPO):
        os.makedirs(settings.HOMESLICE_REPO)

    click.echo('Cloning repo from {} ...'.format(url))
    git.clone(settings.HOMESLICE_REPO, url, name, submodules)
...
def remove(repo, url, force):
    ...
    repopath = os.path.join(settings.HOMESLICE_REPO, repo)
