...
def rand_int(lo=0, hi=10, parity=-1):
    """
    ...
    Args:
        ...
        parity (int):
            1: odd
            0: even
            -1: default, either odd or even.

    Returns:
        int: a random integer.
    """
    # we do it this way, because we let user specify own hi and lo
    # so if they want odd but set lo=0, then 0 is a possibility if we just do range(hi, lo)
    l = [x for x in range(lo, hi)] if parity == -1 else [x for x in range(lo, hi) if x % 2 == parity]
...
def int_vector(lo=0, hi=10, size=4, sort=False, parity=-1):
    """
    ...
    res = [rand_int(lo, hi, parity=parity) for _ in range(n)] if parity != -1 else [rand_int(lo, hi) for _ in range(n)]
    if sort: res.sort()
    return res
