...
class ItemFactory(AbstractItemFactory):
    ...
    class Meta:
        model = 'orders_flavor.Item'


class ItemInOrderFactory(AbstractItemFactory):
    item = factory.SubFactory('orders_flavor.factories.ItemFactory')
    order = factory.SubFactory('orders_flavor.factories.OrderFactory')
    quantity = factory.fuzzy.FuzzyInteger(1, 10)

    class Meta:
        model = 'orders_flavor.ItemInOrder'
...
class OrderFactory(factory.django.DjangoModelFactory):
    ...
    currency = factory.SubFactory('countries_flavor.factories.CurrencyFactory')
    ...
    class Meta:
        model = 'orders_flavor.Order'

    @factory.post_generation
    def amount(self, create, extracted, **kwargs):
        if create:
            self.amount = (
                sum(item.amount for item in self.items_in_order.all())
            )
