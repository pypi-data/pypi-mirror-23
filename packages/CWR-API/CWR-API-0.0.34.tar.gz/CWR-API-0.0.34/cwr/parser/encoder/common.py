...
An encoder parser receives an instance of a model class and returns a data
structure.

For example, it may receive a CWRFile instance and return a JSON.

All encoders are expected to implement the Encoder interface. This offers a
single method which receives the class to encode, and returns an equivalent
data structure.
"""
...
class Encoder(object):
    ...
    def encode(self, entity):
        """
        Encodes the data, creating a structure from an instance from the
        domain model.
