...
class HackChat:
    ...
    Attributes:
        onlineUsers -- <list>; nicknames (<str>) of users online in <channel>
        nick -- <str>; the nickname being used
        channel -- <str>; the channel connected to
        pwd -- <str>; the password being used
        url -- <str>; the hack.chat instance connected to
        require_cert -- <bool>; set to <True> to prevent connecting if
                        the websites' certificate is invalid
    ...
    def __init__(self, callback, nick, channel, pwd="",
                 url="wss://hack.chat/chat-ws", require_cert=False):
        """Initializes values.
        ...
        opt = None if self._require_cert else {"cert_reqs": ssl.CERT_NONE}
        self._ws.run_forever(sslopt=opt)

    def _on_open(self, ws):
        """"Joins the https://hack.chat/ channel and starts pinging."""
