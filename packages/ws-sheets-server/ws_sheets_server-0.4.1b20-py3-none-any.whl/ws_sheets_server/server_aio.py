...
class Application(object):
    def __init__(self, loop, vargs):
        kwargs = {
                'dev': vargs.get('d', False,),
                'port': vargs.get('port', None),
                'console': vargs.get('console', False),
                'addr_storage': vargs.get('addr_storage', None),
                }

        conf = modconf.import_class(vargs['conf_mod'], 'Conf', tuple(),
                kwargs=kwargs,
                folder=vargs.get('conf_dir',None))
...
async def async_runserver(loop, vargs):

    async with Application(loop, vargs) as addr, app:
        try:
            loop.run_forever()
...
def runserver(args):
    ...
    loop.run_until_complete(async_runserver(loop, args))
