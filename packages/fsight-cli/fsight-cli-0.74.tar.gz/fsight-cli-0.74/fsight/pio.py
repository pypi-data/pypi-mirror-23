...
class PioCli(object):
    ...
    def _get_default_pio_git_home(self):
        return 'https://gitlab.qiotec-internal.com/nautilian-platform/'
...
#        print("")
    ...
    def _get_full_config(self):
        ...
        if not os.path.exists(config_file_filename):
            ...
            initial_config_dict = {'fsight_api_version': self._get_default_pio_api_version(),
                                   'fsight_git_home': self._get_default_pio_git_home(),
                                   'fsight_git_version': self._get_default_pio_git_version()}
            initial_config_yaml =  yaml.dump(initial_config_dict, default_flow_style=False, explicit_start=True)
            print("")
            print("Default config created at '%s'.  Override with 'fsight init-fsight'" % config_file_filename)
            print("")
    ...
    def deploy(self,
        ...
        if not model_namespace:
            try:
                ...
                print("")
                print("Model needs to be configured with 'fsight init-model'.")
                print("")
                return
        ...
        if (os.path.isdir(model_path)):
            compressed_model_bundle_filename = 'foresight-model.tar.gz' 
