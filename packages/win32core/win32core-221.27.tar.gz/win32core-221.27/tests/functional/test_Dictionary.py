...
def Register(quiet):
    ...
    RegisterPythonServer(win32com.servers.dictionary.__file__,
                         'Python.Dictionary')
...
def TestDict(quiet=None):
    ...
    try:
        ...
        if hr != winerror.DISP_E_BADPARAMCOUNT:
            raise error("Expected DISP_E_BADPARAMCOUNT - got %d (%s)" % (hr,
                                                                         desc))
    ...
    try:
        ...
        if hr != winerror.DISP_E_TYPEMISMATCH:
            raise error("Expected DISP_E_TYPEMISMATCH - got %d (%s)" % (hr,
                                                                        desc))
