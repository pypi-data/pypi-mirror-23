...
BOOL PyWinObject_AsMSG(PyObject *ob, MSG *pMsg)
{
	...
	if (!PyArg_ParseTuple(ob, "OiOOi(ii):MSG param",
						  ...
						  &pMsg->pt.y))
		return FALSE;
	return PyWinObject_AsHANDLE(obhwnd, (HANDLE *)&pMsg->hwnd) && PyWinObject_AsPARAM(obwParam, &pMsg->wParam) && PyWinObject_AsPARAM(oblParam, &pMsg->lParam);
}
...
int PyWinGlobals_Ensure()
{
	...
	if (PyWinExc_ApiError == NULL)
	{
		...
		if (!d)
			return -1;
		PyObject *name = PyWinCoreString_FromString("win32.types");
		if (!name)
		{
			Py_DECREF(d);
			return -1;
	...
	*/
	if (PyType_Ready(&PyHANDLEType) == -1 || PyType_Ready(&PyOVERLAPPEDType) == -1 || PyType_Ready(&PyDEVMODEAType) == -1 || PyType_Ready(&PyDEVMODEWType) == -1 || PyType_Ready(&PyWAVEFORMATEXType) == -1
...
#ifndef NO_PYWINTYPES_SECURITY
		|| PyType_Ready(&PySECURITY_DESCRIPTORType) == -1 || PyType_Ready(&PySECURITY_ATTRIBUTESType) == -1 || PyType_Ready(&PySIDType) == -1 || PyType_Ready(&PyACLType) == -1
#endif
		)
		return -1;
...
#define ADD_TYPE(type_name)                                                                                             \
	if (PyType_Ready(&Py##type_name) == -1 || PyDict_SetItemString(dict, #type_name, (PyObject *)&Py##type_name) == -1) \
		PYWIN_MODULE_INIT_RETURN_ERROR;

PYWIN_MODULE_INIT_FUNC(types)
{
	PYWIN_MODULE_INIT_PREPARE(types, win32types_functions,
	...
	if (PyDict_SetItemString(dict, "error", PyWinExc_ApiError) == -1 || PyDict_SetItemString(dict, "com_error", PyWinExc_COMError) == -1 || PyDict_SetItemString(dict, "TRUE", Py_True) == -1 || PyDict_SetItemString(dict, "FALSE", Py_False) == -1)
		PYWIN_MODULE_INIT_RETURN_ERROR;
