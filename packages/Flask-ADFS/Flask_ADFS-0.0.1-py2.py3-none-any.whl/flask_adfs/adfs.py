...
adfs_bp = Blueprint('adfs_bp', __name__)


@adfs_bp.route('/guard/', methods=['GET'])
@role_required("view")
...
def login():  # pragma: no cover
    ...
    if o.path == url_for("adfs_bp.login", _external=False):
        app.logger.debug('attempt to directly access login page')
        abort(403, 'Abort: direct access to login pagenot allowed' )

    if not requested_page:
        app.logger.debug('Abort: No page requested')
        abort(403, 'No page requested')

    if app.config.get('LOGIN_DISABLED') or current_user.is_authenticated:
        app.logger.debug('/login login disabled ({0}) or already authenticated{(1)}'.format
        ...
        if user_ticket is None:
            ...
            auth_url = (app.config.get('ADFS_URL') + "/adfs/oauth2/authorize?" +
                ...
                "&state=" + script_root)
            app.logger.debug('/login ADFS redirect: {0}'.format(auth_url))
            return redirect(auth_url)
        else:
            app.logger.debug('/login user ticket ={0}={1}'.format(user_ticket[:4],user_ticket[-4:]))
...
def OAUTH_validate(user_ticket):  # pragma: no cover
    app.logger.debug('OAUTH_validate')
    ...
    try:
        ...
        try:
            app.logger.debug('session requested page = {0}'.format(session['requested_page']))
        ...
        app.logger.debug('exception login_user')
        return render_template(ERROR_TEMPLATE, requested_page=session['requested_page']), 403
...
def validate_user_ticket(user_ticket):  # pragma: no cover
    ...
    if resp.status_code == 200:
        try:
            resp = resp.json()
            app.logger.debug('ADFS Ticket Validated')
            return resp['access_token']
        except:
            app.logger.debug('ADFS Ticket Not Validated')
            pass
    return ''
...
def load_user(user_id):  # pragma: no cover
    try:
        ...
        app.logger.debug('load user suceeded')
        app.logger.debug('load user suceeded {0} {1}'.format(g.user.get_racf_id(), g.user.is_authenticated))
        return g.user
    except:
        app.logger.debug('load user failed')
        return None
...
def handle_needs_login():
    ...
    app.logger.debug('unauthorized_handler {0}'.format(request.url))
