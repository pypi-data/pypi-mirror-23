...
# pick a single instance out of the set.
@task
def set_one_host(type,primary=None,appname=None,region=None):
    if appname is None:
        local('echo "ERROR: appname option is not set"')
    if region is None:
        local('echo "ERROR: region option is not set"')
    environment = os.environ["AWS_ENVIRONMENT"]
    awsaddresses    = _get_awsaddress(type, primary, environment, appname, action, region)
    env.hosts[0] = awsaddresses[0].publicdns
    print env.hosts
...
@task
def set_action(actionneeded):
    action = actionneeded
    print actionneeded, action
...
def _get_awsaddress(type,primary, environment,appname,actionneeded,region):
    awsaddresses = []
    ...
    if actionneeded:
        aws_tags["tag:ActionNeeded"] = actionneeded
    if primary:
...
def deploycode(branch,doCollectStatic=True):
    ...
    if doCollectStatic in TRUTH_VALUES:
        ...
        sudo('cp -r ' + distutils.sysconfig.get_python_lib() + '/django/contrib/admin/static/admin /data/deploy/current/static/')
