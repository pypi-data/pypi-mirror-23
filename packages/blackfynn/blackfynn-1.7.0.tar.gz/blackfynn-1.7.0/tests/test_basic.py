...
def test_get_all_datasets(client, superuser_client):
    ...
    client.set_context('Blackfynn')
    n_ds_bf = len(client.datasets())

    ds3 = client.create_dataset("DS 3")

    # should only return the datasets of the active organization (current context).
    assert len(client.datasets()) == n_ds_bf + 1

    client.set_context('Test Organization')
    assert len(client.datasets()) == n_ds_to + 2

    # cleanup
    superuser_client.delete(ds1)
    superuser_client.delete(ds2)
    superuser_client.delete(ds3)

    assert len(client.datasets()) == n_ds_to
    client.set_context('Blackfynn')
    assert len(client.datasets()) == n_ds_bf
