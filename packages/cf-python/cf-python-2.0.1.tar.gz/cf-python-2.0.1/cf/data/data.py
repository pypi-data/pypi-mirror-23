...
:Returns:
    ...
    def _return_auxiliary_mask(self):
        '''
'''
    ...
    def _add_auxiliary_mask(self, mask):
        '''
        '''
        if mask is None:
            return
...
"Auxiliary mask shape {} is not broadcastable to data array shape {}".format(
    ...
    def _index_auxiliary_mask(self, indices):
        '''
        '''
    ...
    mask_shape: `tuple`
        ...
        contain `None` for axes not spanned by `ind`.
...
:Returns:
    ...
    def _add_auxiliary_masks(self, masks):
        '''
'''
        for mask in masks:
            self._add_auxiliary_mask(mask)
    #--- End: def
    
    def _tidy_auxiliary_mask(self):
        '''
        '''
...
'''
        ...
        auxiliary_mask = []
#        print d.Units,'__________________________', repr(indices)
#        print '________END'
...
#        print d.Units, type(auxiliary_mask), len(auxiliary_mask)
...
#        print  'FINAL',new.Units, new._auxiliary_mask
#        print
#        print
        return new
    #--- End: def
...
#                    f_partitions.expand_dims(Paxis, i=True)
            ...
            if data0._auxiliary_mask:
                ...
                for mask in data0._auxiliary_mask:
                    ...
                    if ((size > 1 and mask.shape[axis] > 1) or
                        ...
                        mask = cls.empty(new_shape, dtype=bool)

                        indices = [slice(None)] * mask.ndim

                        indices[axis] = slice(0, original_shape0[axis])
                        mask[tuple(indices)] = mask

                        indices[axis] = slice(original_shape0[axis], None)
                        mask[tuple(indices)] = False
                    else:
                        mask = mask0
                    #--- End: if
            ...
            if new_auxiliary_mask:
                ...
                data0._set_auxiliary_mask(new_auxiliary_mask)                
        #--- End: for
...
True
        ...
        # ------------------------------------------------------------
        if self._auxiliary_mask:
#            print 'DCH Data.array: Applying auxilary mask =', self._auxiliary_mask
            if not masked:
...
#                partition.subarray = SubArray(numpy_array(~array.mask, dtype='int32'))
            else:
                # data is not masked
                partition.subarray = numpy_ones(array.shape, dtype='int32')
#                partition.subarray = SubArray(numpy_ones(array.shape, dtype='int32'))
