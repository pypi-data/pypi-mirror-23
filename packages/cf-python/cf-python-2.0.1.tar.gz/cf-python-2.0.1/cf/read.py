...
#    #--- End: for
    ...
    # ----------------------------------------------------------------
    if squeeze:
        [f.squeeze(i=True) for f in field_list]
    elif unsqueeze:
        [f.unsqueeze(i=True) for f in field_list]

    if nfields is not None and len(field_list) != nfields:
        raise ValueError(
            "Error whilst reading file{}: {} field{} requested but {} fields found".format(
    _plural(file_counter), nfields, _plural(nfields), len(field_list)))
...
def read1(*args, **kwargs):
    '''``f=cf.read1(*args, **kwargs)`` is equivalent to ``f=cf.read(*args,
nfields=1, **kwargs)[0]``
