...
<numpy:arrays.indexing>`. There are three extensions to the numpy
...
"Can't where: FieldList condition must contain exactly 1 element")
    ...
    @property
    def subspace(self):
        '''Return a subspace of the field.
...
Alternatively, a subapce my be defined be in "index-space" using an
extended Python slicing syntax.

The benefits of subspacing by values of domain items:
...
#            return self._list_method('regrids', kwargs2)
        ...
        method = type(self)._regrid_check_bounds(src_coords, dst_coords, method)
...
{+Fef,}Returns the centred finite difference along the specified
axis. If the axis is cyclic then the boundary is wrapped around,
...
:Parameters:
    ...
    {+i}
        ...
        f = self.filter([-1, 0, 1], axis=axis_key, mode=mode, cval=numpy_nan,
                        i=i)
...
'''
    def __call__(self, *args, **kwargs):
        ...
        #### WARNING: THIS METHOD MIGHT BE DELETED IN THE FUTURE IN FAVOUR OF THE METHOD cf.Field.subspace
...
'''
        #### WARNING: THIS METHOD MIGHT BE DELETED IN THE FUTURE IN FAVOUR OF cf.Field.__getitem__
