...
"WARNING: Missing bounds variable {0!r} in {1}".format(bounds, filename))
    ...
    # ----------------------------------------------------------------
    if _debug:
        print 'Data variables which will become fields:'
        for ncvar in variables:
            print '   ', ncvar
...
#    ncvar_to_field = {}
#    if coordref_parameters:
#        i = 0        
#        while i < len(fields_in_file):
#            f = fields_in_file[i]
#            ncvar = f.ncvar            
#            if ncvar in coordref_parameters:
...
#
#    # ----------------------------------------------------------------
#    # Inside coordinate references, replace parameter nc variables
#    # with Data objects
#    # ----------------------------------------------------------------
#    if ncvar_to_field:
#        for f in fields_in_file:
#            for ref in f.refs().values():
#                for term, value in ref.parameter_terms.items():
#                    try:
#                        ref[term] = ncvar_to_field[value].Data.copy()
#                    except (KeyError, AttributeError):
#                        ref[term] = None
#    #--- End: if
        
#    # ----------------------------------------------------------------
#    # Get rid of any rogue fields
#    # ----------------------------------------------------------------
#    if rogue_fields:
#        i = 0
#        while rogue_fields:
...
#            
#            ncvar = f.ncvar            
#            if ncvar in rogue_fields:
#                # Get rid of this field
#                rogue_fields.discard(ncvar)
#                del fields_in_file[i]
#                i -= 1 
#            #--- End: if
#
...
'''
    ...
    count_array = _create_Variable(nc, ncvar, attributes, uncompress=False, g=g).array
...
def _parse_DSG_indexed_compression(nc, ncvar, attributes,
    ...
    index_array = _create_Variable(nc, ncvar, attributes, uncompress=False, g=g).array
...
#    f.ncdimensions = {}
    ...
    for ncdim in field_ncdimensions:
        if ncdim in nc.variables:
            ...
            if _debug:
                print '    Inserting dimension coordinate:', repr(coord)
            ...
            if _debug:
                print '    Inserting axis:', repr(axis)
    ...
    if coordinates is not None:
        ...
        for ncvar in re.split('\s+|\s*,\s*', coordinates):
            ...
            if is_dimension_coordinate:
                ...
                if _debug:
                    print '    Inserting auxiliary coordinate:', repr(coord)
