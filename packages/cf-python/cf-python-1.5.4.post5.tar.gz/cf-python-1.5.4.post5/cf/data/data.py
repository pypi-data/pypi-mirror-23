...
_units_calendar_year = Units('calendar_year')
...
#        self._isdt     = False
        ...
        u = getattr(data, 'Units', None)
        if u is not None and isinstance(u, Units):
            # data has its own Units attribute, so use it.
            if units is not None and not u.equals(Units(units)):
                raise ValueError("aklsjdaskldjs")
            units = u
