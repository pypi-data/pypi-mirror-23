...
from .functions           import (flat, RTOL, ATOL, equals, hash_array,
                                  allclose, _numpy_allclose)
...
'''
        ...
        signature = [self.identity,
                     self.units.formatted(definition=True),
                     self.cell_methods,
                     self._hasData]
        ...
        axis = self.axis
        x = [(axis[identity]['ids'],
              tuple([u.formatted(definition=True) for u in axis[identity]['units']]),
        ...
        # Cell measures
        msr = self.msr
        x = [(units.formatted(definition=True),
...
##                    coord.Units = coord_units
        ...
        if donotchecknonaggregatingaxes:
            ...
            for aux in m.nd_aux.itervalues():
                ...
                h = _get_hfl(coord, canonical_units, sort_indices, False, False,
                             False, hfl_cache, rtol, atol)
                
                if coord._hasbounds:
                    # Get the hash of the bounds data array
                    hb  = _get_hfl(coord.bounds, canonical_units, sort_indices,
...
##                    if qwerty: 
                    # Get the hash of the data array
                    h = _get_hfl(coord, canonical_units, tuple(sort_indices),
                                 False, False, False, hfl_cache, rtol, atol)
...
def _get_hfl(v, canonical_units, sort_indices, null_sort, first_and_last_values,
             first_and_last_bounds, hfl_cache, rtol, atol):
    '''
