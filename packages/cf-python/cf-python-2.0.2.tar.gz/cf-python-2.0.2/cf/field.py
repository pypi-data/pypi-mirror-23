...
#        self._private['special_attributes']['axes']  = _Axes()
...
:Parameters:
    ...
    dst_coord_order: `list`, optional
        ...
        if not dst_dict:
            for src_axis_key, dst_axis_key in zip(src_axis_keys, dst_axis_keys):
                ncdim = getattr(dst._Axes[dst_axis_key], 'ncdim', None)
...
Weights components for may be derived for the specified axes from the
field's metadata (such as coordinate values) or provided explicitly in
the form of other `cf.Field` objects. In any case, the outer product
of these weights components is returned in a field which is
...
#               exact=False, match_and=True, inverse=False):
#        '''`cf.Field.select` has been deprecated.
#
#Use `cf.Field.match` to see if an individual field meets given
#criteria.
#        '''
#        raise DeprecationError(
#"cf.Field.select has been deprecated. Use cf.Field.match to see if an individual field meets given criteria.")
#    #--- End: def
...
#            return self._list_method('unsqueeze', kwargs2)
    ...
    def aux(self, items=None, axes=None, axes_all=None,
            ...
            copy=False):
        '''{+Fef,}Return an auxiliary coordinate object, or its identifier.
...
#        return self.item(role=role, _restrict_inverse=role, **kwargs2)#
    ...
    def measure(self, items=None, axes=None, axes_all=None,
        ...
        '''{+Fef,}Return a cell measure object, or its identifier.
...
[+1]    out:
[+N]    out: `list`
        {+Fef,}The unique cell measure object or its identifier or, if
...
{+Fef,}Return a dimension or auxiliary coordinate object, or its identifier.
...
[+N]    out: `list`
        ...
        its identifier or, if there is not one, `None`.
...
#        return self.item(role=role, _restrict_inverse=role, **kwargs2)
    ...
    def ref(self, items=None, axes=None, axes_all=None,
            ...
            copy=False):
        '''{+Fef,}Return a coordinate reference object, or its identifier.
...
#            out = a
    ...
    def axis(self, axes=None, size=None, default=None, key=False,
             **kwargs):
        '''Return a domain axis, or its identifier, from the field.
...
#"Incompatible key and axes parameters: {}, {}".format(key, axes))
        ...
        if not replace and key in self and self._Axes[key].size != axis.size:
            raise ValueError(
    ...
    method: `str`
        ...
        one of:
          ...
          *method*            Description
          ...
          ``'conservative'``  First-order conservative regridding will
                              be used (requirer both of the fields to
                              have contiguous bounds).
