...
'''
        ...
        self._subarray = subarray  # DO NOT UPDATE INPLACE
...
#        '''
#
#True if and only if the partition's sub-array is a shared memory array.
#
...
'''
        return not isinstance(self._subarray, FileArray)
    #--- End: if
...
>>> p.close()
        ...
        if _debug:  print 'Partition.close: original =', original
        ...
        config = self.config
        if _debug:  print 'Partition.close: config =',config

        if config['serial']:
            # --------------------------------------------------------
            # SERIAL
            # --------------------------------------------------------
            if _debug: print 'Partition.close: serial'
            if config['readonly']:
                if originally_on_disk:
                    if config.get('to_disk', False):
                        ...
                        if (config['unique_subarray'] and 
                            ...
                            # other partitions
                            _remove_temporary_file(original_subarray._partition_file)

                        if _debug: print 'Partition.close: 1.1.1.3 pass'
                        del self.masked       
                        pass

                else:
                    if _debug: print 'Partition.close: originally in memory'
                    if config.get('to_disk', False):
                        # 1.1.2.1 Original subarray was in memory and
                        #         we don't want to keep the current
                        #         subarray in memory
                        if _debug: print 'Partition.close: 1.1.2.1 to_disk'
                        ...
                        if _debug: print 'Partition.close: 1.1.2.2 to_disk'
                        self.to_disk(reopen=False)
                    else:
                        # 1.1.2.3 Original subarray was in memory and
                        #         unique and there is enough memory to
                        #         keep the current subarray in memory
                        if _debug: print 'Partition.close: 1.1.2.3 pass'
                        pass
            else:
...
#            print '    id(subarray) =', id(subarray)
        ...
        if not self.in_memory:
            # --------------------------------------------------------
            # The sub-array is not in memory.
            #
...
"Can't recast partition array from {} to {}".format(p_dtype.name, dtype.name))
        ...
        # ------------------------------------------------------------
        if update:
            self._subarray = p_data ##?? DCH CHECK
            self.Units     = p_units
            self.part      = []
            self.axes      = axes
            self.flip      = flip
...
''' 
    ...
    def extra_memory(self):
        ...
        if not self.in_memory:
            # --------------------------------------------------------
            # The sub-array is on disk so getting the partition's data
            # array will require extra memory
            # --------------------------------------------------------
            extra_memory = True
        else:
            # --------------------------------------------------------
            # The sub-array is already in memory
            # --------------------------------------------------------
            config = self.config
...
#    #--- End: def
    ...
    def to_disk(self, reopen=True):
        '''Move the partition's sub-array to a temporary file on disk.

.. note:: It is assumed that the partition's sub-array is currently in
...
:Returns:
    ...
    out: `bool`
        True if the sub-array was moved to disk, False otherwise.
...
#
#    out : bool
#        Whether or not the sub-array is in shared memory.
#
#'''
...
#
#        try:
#            self._subarray = SharedMemoryArray(self.array)
#        except:
#            return False
#
#        # Re-open the partition
