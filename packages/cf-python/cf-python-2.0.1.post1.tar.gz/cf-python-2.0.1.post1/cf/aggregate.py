...
##                                        None)
#        f_ancillary_variables = self.field.field_ancs()
#
#        if not f_ancillary_variables:
...
#                if self.info:
#                    self.message = \
#"multiple {!r} ancillary variables".format(av.name(''))
#                return None
#            #--- End: if
...
#        x = [(units,
        ...
        x = [(identity,
              domain_anc[identity]['units'],
              domain_anc[identity]['axes']) for identity in sorted(domain_anc)]
        signature_append(tuple(x))
                
        # Field ancillaries
        field_anc = self.field_anc
        x = [(identity,
              field_anc[identity]['units'],
              field_anc[identity]['axes']) for identity in sorted(field_anc)]
        signature_append(tuple(x))
