...
:Parameters:
    ...
    copy: `bool`, optional
        ...
        self._private['special_attributes']['axes']  = Axes()
...
"Can't take a cyclic slice from non-cyclic {!r} axis".format(self.axis_name(iaxis)))
        ...
        items = self.items(role=('d', 'a', 'm', 'f', 'c'), axes=data_axes)
        for key, item in items.iteritems():
            ...
            if auxiliary_mask and new.DSG: # and set(item_axes).intersection(data_axes):
                item_mask = []
...
#AND HEREIN LIE TE PROBLEM            
        ...
        for axis0 in s['size1_broadcast_axes'] + s['new_size1_axes']:
            ...
            field0.Axes[axis0] = field1.Axes[axis1]
            if _debug:
...
"Can't assign: Can't broadcast undefined axis with size {}".format(
    ...
    # ----------------------------------------------------------------
    def _regrid_get_latlong(self, name, axes=None, return_2D=True):
...
these are returned, otherwise, if requested, 2D lat/long coordinates
are searched for and if found returned.
...
:Parameters:
    ...
    return_2D: `bool`, optional
        Whether to search for, and return if found, 2D lat/long
...
:Returns:
    ...
    coords_2D : `bool`
        ...
        if axes is None:
            ...
            len_x = len(x)
            if not len_x:
                msg = 'No X dimension coordinate found for the ' + \
                      name + ' field.'
                if return_2D:
                    msg += ' If none is present you may need to specify ' + \
                           'the axes keyword, otherwise you may need to ' + \
                           'set the X attribute of the X dimension ' + \
                           'coordinate to True.'
            ...
            if not len_y:
                msg = 'No Y dimension coordinate found for the ' + \
                      name + ' field.'
                if return_2D:
                    msg += ' If none is present you may need to specify ' + \
                           'the axes keyword, otherwise you may need to ' + \
                           'set the Y attribute of the Y dimension ' + \
                           'coordinate to True.'
                raise ValueError(msg)
        ...
        if axes is not None or not x.Units.islongitude \
            ...
            # If 2D auxiliary coordinates are not requested throw an error
            if not return_2D:
                raise ValueError('Both 1D longitude and 1D latitude ' +
        ...
        if return_2D:
            ...
            return axis_keys, axis_sizes, coords
        #--- End: if
    #--- End: def
...
:Returns:
    ...
    coords : list
        ...
        for axis in axes:
            ...
            if len(tmp) != 1:
                raise ValueError('Each item in axes keyword must specify ' +
                                 'exactly one axis.')
...
:Returns:
    ...
    @staticmethod
    def _regrid_check_bounds(src_coords, dst_coords, method,
                             src_coords_2D=False, dst_coords_2D=False,
                             ext_coords=None):
        '''
    ...
    dst_coord_order: `list`, optional
        ...
        if cartesian:
            ...
            for axis_key, axis_size in zip(src_axis_keys, dst_axis_sizes):
                self.Axes[axis_key].size = axis_size
            #--- End: for
        ...
        if not dst_dict:
            for src_axis_key, dst_axis_key in zip(src_axis_keys, dst_axis_keys):
                self.ncdimensions[src_axis_key] = \
                    dst.ncdimensions[dst_axis_key]
            #--- End: for
        #--- End: if
    #--- End: def
    # ----------------------------------------------------------------
...
'''
    ...
    def Axes(self):
        '''
        '''
#        return self._private['special_attributes']['axes']
...
#        # List functionality
#        if self._list:
#            raise DeprecationError("fl.cell_area has been deprecated. Use cf.FieldList(f.cell_area(...) for f in fl) instead")
...
#            return self._list_method('weights', kwargs2)
        ...
        asd = set(field.Axes).difference(weights_axes)
        field.remove_items(axes=asd)
        field.remove_axes(asd)
 
        field = field.copy()
        
        field.insert_data(data, axes=waxes)
...
:Parameters:
    ...
    weights: *optional*
        ...
        components=True)``. By default *weights* is ``'auto'``,
        meaning that weights for all axes are created from the field's
...
'''
        axis = self.axis(axes, size=size, **kwargs)
        if axis is None:
            return True
...
>>>
        ...
        for cm0, cm1 in zip(cms0, cms1):
            ...
            for axis0 in axes0:
                for axis1 in axes1:
                    if axis0 in self.Axes and axis1 in field1.Axes:
                        if axis1 == m.get(axis0, None):
                            axes1.remove(axis1)
                            argsort.append(cm1.axes.index(axis1))
                            break
                    elif axis0 in self.Axes or axis1 in field1.Axes:
                        continue
                    elif axis0 == axis1:
                        axes1.remove(axis1)
                        argsort.append(cm1.axes.index(axis1))
...
"Can't insert an axis of size {0}: {0!r}".format(self.axis_size(axis), axis))
        ...
        f.insert_axis(DomainAxis(1), key=axis, replace=False)
        ...
        for sorted_axes, axes_key_item_value in parsed.iteritems():
            ...
            if n_items > n_axes:
                if n_axes == 1:
                    ...
                    a = 'axes'
                raise IndexError(
"Error: Can't specify {0} conditions for {1} {2}: {3}".format(
            ...
            if n_axes == 1:
                ...
                if isinstance(value, (list, slice, tuple, numpy_ndarray)):
                    ...
                    if not item_match.any():
                        raise IndexError(
"No {} axis indices found from: {}".format(identity, value))
...
#        if envelope:
#            if envelope_mask:
#             indices = ('envelope', auxiliary_mask) + indices
#            indices = ('mask', auxiliary_mask) + indices
#            else:
#                indices = ('envelope_no_mask', None) + indices
#        else:
#            indices = ('compress', auxiliary_mask) + indices
...
(axis, size))
            ...
            if not self.axes():
                ...
                if len(data_shape) != len(set(data_shape)):
                    raise ValueError(
"Can't set data: Ambiguous data shape: {}. Consider setting the axes parameter.".format(
    data_shape))
...
"Can't set data: Ambiguous data shape: {}. Consider setting the axes parameter.".format(
    data_shape))
            ...
            if len(axes) != data.ndim:
                raise ValueError(
"Can't set data: Wrong number of axes for data array: {!r}".format(
    axes))
            ...
            for axis, size in izip(axes, data.shape):
                try:
                    ...
                    raise ValueError(
"Can't set data: Incompatible size for axis {!r}: {}".format(
    axis, size))
            #--- End: for
        #--- End: if
...
#        mask.Data = Data.zeros(self.shape, dtype=bool)
        mask.insert_data(Data.zeros(self.shape, dtype=bool),
                         axes=self.data_axes(), copy=False)
        mask.Data[mask.indices(*args, **kwargs)] = True

        mask.long_name = 'binary mask'
...
##    #--- End: def
    ...
    def filter(self, weights, axis=-1, mode='reflect', cval=0.0, origin=0,
               i=False):
        '''

{+Fef,}Returns the field convolved along the given axis with the
specified filter.

:Parameters:

    weights: ndarray
        One-dimensional sequence of numbers defining the filter.

    {+axes}

    mode: {'reflect', 'constant', 'nearest', 'mirror', 'wrap'}
        optional Determines how the edges of the array are handled. A
        table of the different behaviours is given here:
        https://stackoverflow.com/questions/22669252/how-exactly-does-the-reflect-mode-for-scipys-ndimage-filters-work
        "Suppose the data in one axis is 1 2 3 4 5 6 7 8. The following
        table shows how the data is extended for each mode (assuming
        cval=0):
    ...
    origin: `int`, optional
        Controls the placement of the filter.
...
"select method has been removed from a field. Use match on a field or select on a field list.")
...
"Anchor value has incompatible units: {0!r}".format(value.Units))
    ...
    def field_anc(self, items=None, axes=None, axes_all=None,
...
.. seealso:: `ancillaries`, `aux`, `coord`, `dim`, `item`, `measure`,
             `ref`, `remove_item`
...
#        kwargs2['_restrict_inverse'] = role
    ...
    def field_ancs(self, items=None, axes=None, axes_all=None,
                   axes_subset=None, axes_superset=None, ndim=None,
                   match_and=True, exact=False, inverse=False, copy=False):
...
.. seealso:: `ancillary`, `auxs`, `axes`, `measures`, `refs`,
             `coords`, `dims`, `items`, `remove_items`
...
#
#    REINSTATE WHEN class SubspaceField IS REMOVED
#
...
"Can't transpose {}: Bad axis specification: {!r}".format(
    ...
    def transpose_item(self, item=None, iaxes=None, **kwargs):
        '''{+Fef,}Permute the axes of the data array.
...
>>> f.transpose((1, 'time', 'dim2'))
        ...
        '''     
        key, item = self.key_item(item, **kwargs)
...
#        return self.item(role=role, _restrict_inverse=role, **kwargs2)
    ...
    def domain_anc(self, items=None, axes=None, axes_all=None,
                   axes_subset=None, axes_superset=None, ndim=None,
                   match_and=True, exact=False, inverse=False,
                   key=False, copy=False):
        '''Return a domain ancillary object, or its domain identifier.
...
#        return self.item(role=role, _restrict_inverse=role, **kwargs2)
    ...
    def ref(self, items=None, axes=None, axes_all=None,
            axes_subset=None, axes_superset=None, ndim=None,
...
#        return self.items(role=role, _restrict_inverse=role, **kwargs2)
    ...
    def domain_ancs(self, items=None, axes=None, axes_all=None,
                    axes_subset=None, axes_superset=None, ndim=None,
                    match_and=True, exact=False, inverse=False,
                    copy=False):
...
#        return self.items(role=role, _restrict_inverse=role, **kwargs2)
    ...
    def refs(self, items=None, axes=None, axes_all=None,
             axes_subset=None, axes_superset=None, ndim=None,
             match_and=True, exact=False, inverse=False, copy=False):
...
>>> 
    ...
    def key(self, items=None, role=None, axes=None, axes_all=None,
            axes_subset=None, axes_superset=None, exact=False,
...
>>>
    ...
    def key_item(self, items=None, role=None, axes=None,
...
>>> f.axes().difference(f.data_axes())
    ...
    :Parameters:
        ...
        data_axes: `list` of domain axis identifiers
...
#            out = a
    ...
    def axis(self, axes=None, size=None, default=None, **kwargs):
...
:Examples:
    ...
    def insert_axis(self, axis, key=None, axes=None, replace=True):
...
:Returns:
    ...
    out: `str`
        The domain identifier of the new domain axis.
...
"Incompatible key and axes parameters: {}, {}".format(key, axes))
        ...
        if not replace and key in self and self.Axes[key].size != axis.size:
            raise ValueError(
"Can't insert axis: Existing axis {0!r} has different size (got {1}, expected {2})".format(
...
'''
        if key in self._item_axes and not replace:
            raise ValueError(
"Can't insert {0}: Domain identifier {1!r} already exists".format(role, key))
...
#            # Key is not set, axes is not set
#            item_size = item.size
#            c = [axis for axis, size in self.Axes.iteritems() 
#                 if size == item_size]
#            if len(c) == 1:                
#                key = c[0]
#                axes = [key]
...
'''    
    ...
    def items(self, items=None, role=None, axes=None, axes_all=None,
              ...
              match_and=True, exact=False, inverse=False, copy=False):
#              _restrict_inverse=False):
        '''Return items of the field's domain.

Domain items consist of:

  * Dimension coordinate objects
  * Auxiliary coordinate objects
  * Cell measure objects
  * Domain ancillary objects
  * Field ancillary objects

Coordinate reference objects may be accessed with the `refs` method.
...
None
        ...
        '''
        axis = self.axis(axes=axes, **kwargs)
        if axis is None:
...
:Examples:
        ...
        raise ValueError(
            "Can't remove non-unique item defined by {0}".format(kwargs2))
    #--- End: def
...
#
        ...
        if dim is not None and dim.period() is None:
            raise ValueError(
"Can't roll: {0!r} axis has non-periodic dimension coordinates".format(
    dim.name()))
        ...
        for key, item in self.items(role=('d', 'a', 'm', 'c', 'f')).iteritems():
            axes = self.item_axes(key)
            if axis in axes:
...
[+1]>>> f[northern_hemisphere] = cf.masked
[+N]
[+N]>>> for g in f:
[+N]...     northern_hemisphere = g.indices(latitude=cf.ge(0))
[+N]...     g[northern_hemisphere] = cf.masked
...
"Can't where: FieldList condition must contain exactly 1 element")
            ...
            if item is not None:
                # Apply the Query to the data of one of the field's
                # items, making sure that the item's data is
                # broadcastable to the field's data.
                g = f.transpose(f.data_axes(), items=True)
                key, item = g.key_item(item)
                if key is None:
                    ValueError("Humdrum")
    ...
    # ----------------------------------------------------------------
    # DELETE WHEN class SubspaceField IS REMOVED
    # ----------------------------------------------------------------
    @property
    def subspace(self):
        '''Return a subspace of the field defined by metadata values

A subspace may be defined via data array values of its metadata items
(dimension coordinate, auxiliary coordinate, cell measure, field
ancillary objects, etc.).
...
#            return self._list_method('regrids', kwargs2)
        ...
        if dst_dict:
            ...
            dst_axis_keys, dst_axis_sizes, dst_coord_keys, dst_coords, \
                dst_coords_2D = dst._regrid_get_latlong('destination', axes=dst_axes)
        #--- End: if
        ...
        method = type(self)._regrid_check_bounds(src_coords,
                                                 dst_coords, method,
                                                 src_coords_2D,
                                                 dst_coords_2D)
...
#            return self._list_method('regridc', kwargs2)
        ...
        src_axis_keys, src_coords = self._regrid_get_cartesian_coords('source', axes)
        ...
        if dst_dict:
            ...
            dst_axis_keys, dst_coords = dst._regrid_get_cartesian_coords('destination', axes)
        #--- End: if
        
        # Get the axis indices and their order for the source field
        src_axis_indices, src_order, self = self._regrid_get_axis_indices(src_axis_keys, i=i)
        
        # Get the axis indices and their order for the destination field.
        if not dst_dict:
            dst_axis_indices, dst_order, dst = dst._regrid_get_axis_indices(dst_axis_keys, i=i)
        #--- End: if
        ...
        if n_axes == 1 and self.squeeze().ndim > 1:
            ...
            if src_shape[src_axis_indices].prod()*max_length*8 < CHUNKSIZE():
                ...
                src_axis_indices_ext, src_order_ext, self = \
                    self._regrid_get_axis_indices(axis_keys_ext + src_axis_keys, i=i)
        
        # Calculate shape of each regridded section
        shape = self._regrid_get_section_shape([coord.size for coord in coords_ext +
        ...
        for k in section_keys:
            ...
            for k2 in subsections.keys():
                ...
                # Get the regridded data or frac field as a numpy array
                regridded_data = self._regrid_get_regridded_data(method, fracfield,
...
'''
    def __call__(self, *args, **kwargs):
        ...
        #### WARNING: THIS METHOD WILL BE DELETED IN THE FUTURE IN FAVOUR OF THE METHOD cf.Field.subspace
...
#        # List functionality
#        if field._list:
#            print  "DEPRECATION WARNING: fl.subspace has been deprecated. Use cf.FieldList(f.subspace(...)# for f in fl) instead."
##            return type()([f[f.indices(*args, **kwargs)] for f in field])
#            return type(field)([f.subspace(*args, **kwargs) for f in field])
...
'''
        #### WARNING: THIS METHOD WILL BE DELETED IN THE FUTURE IN FAVOUR OF cf.Field.__getitem__
        
        print "DEPRECATION WARNING: f.subspace[indices] has been deprecated. Use f[indices] instead."
...
'''
        #### WARNING: THIS METHOD WILL BE DELETED IN THE FUTURE IN FAVOUR OF cf.Field.__setitem__
#        if self.variable._list:
#            print  "DEPRECATION WARNING: fl.subspace[indices]=x has been deprecated. Use [f[indices]=x for f in fl] instead."
#            for f in self.variable:
#                f[indices] = value
#            return 

        print "DEPRECATION WARNING: f.subspace[indices]=x has been deprecated. Use f[indices]=x instead."
...
#    #--- End: def
    ...
    def close(self, *args, **kwargs):        raise DeprecationError(self._deprecated_method('close'))
...
:Parameters:
    ...
    display: `bool`, optional
        ...
        string. By default the descriptions are printed.
          ...
          *Example:*
            ``f.dump()`` is equivalent to ``for g in f: print
            g.dump(display=False)``.

:Returns:

    out: `None` or `list`
        If *display* is True then{+,fef,} the description is printed
        and `None` is returned. Otherwise a list of strings containing
        the description for each field is returned.
...
If no fields satisfy the conditions then an empty `cf.FieldList`
object is returned.
...
#            return type(self)(fl)
    ...
    def select1(self, select=None, items=None, rank=None, ndim=None,
                exact=False, match_and=True, inverse=False):
...
...                  'time': cf.cellsize(cf.wi(28, 31, 'days'))})
        ...
        raise ValueError(
            "Error whilst selecting a single field: {} fields found".format(len(out)))
    #--- End: def
...
class Items(dict):
    ...
    def __init__(self):
        '''
'''
        self.f = set()  # Field ancillaries
        self.a = set()  # Auxiliary coordinates
        self.c = set()  # Domain ancillaries
        self.d = set()  # Dimension coordinates
        self.m = set()  # Cell measures
        self.r = set()  # Coordinate references
...
#        self.ncdimensions = {}
    ...
    def __call__(self, items=None, role=None, axes=None,
                 axes_all=None, axes_subset=None, axes_superset=None,
...
#        #--- End: if
        ...
        if (items is None and axes is None and axes_all is None and
...
{'msr0': <CF CellMeasures: >}
    ...
    def refs(self):
        '''Return coordiante reference objects and their identifiers
        
:Returns:
        
    out: `dict`
          A dictionary of coordiante reference objects keyed by their
          identifiers.
...
:Examples:
        ...
        try:
            # Get the name from netCDF dimension name
#            return 'ncdim%{0}'.format(self.ncdimensions[axis])
            return 'ncdim%{0}'.format(self.Axes[axis].ncdim)
        except (KeyError, AttributeError):
            # Get the name from axis domain identifier
            return 'axis%{0}'.format(axis)
    #--- End: def
...
#        new.ncdimensions = self.ncdimensions.copy()
...
'''
    ...
    def item(self, items=None, key=False, default=None, **kwargs):
        '''
'''    
        if key:
            return self.key(items=items, default=default, **kwargs)
    ...
    def key(self, items=None, default=None, **kwargs):
        '''
'''    
        d = self(items, **kwargs)
        if not d:
    ...
    def key_item(self, items=None, default=(None, None), **kwargs):
        '''
'''    
        d = self(items, **kwargs)
        if not d:
...
'''
        return self.equals(other, traceback=False)
    #--- End: def
