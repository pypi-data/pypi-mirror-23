  ...
  [-99, -99, -99, -99]]])
    ...
    def test_CONTIGUOUS(self):  
        ...
        for chunksize in self.chunk_sizes:
            ...
            f = cf.read(self.contiguous)

            q = f.select1('specific')
            
            self.assertTrue(cf.functions._numpy_allclose(q.array, self.a))
        #--- End: for
    ...
    def test_INDEXED_CONTIGUOUS(self):  
        if self.test_only and inspect.stack()[0][3] not in self.test_only:
            return

        for chunksize in self.chunk_sizes:
            cf.CHUNKSIZE(chunksize)
            f = cf.read(self.indexed_contiguous)

            q = f.select1('specific')

            self.assertTrue(cf.functions._numpy_allclose(q.array, self.b))
        #--- End: for
        cf.CHUNKSIZE(self.original_chunksize)            
    #--- End: def        
