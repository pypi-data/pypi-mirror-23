...
"""
Usage:
    main.py (-g | --grade) <course_category> [-u <username> -p <password>]
    main.py (-c | --course) [-u <username> -p <password>]
...
Examples:
    python main.py -c
    python main.py -g BCD
    python main.py -g BCD -u your_username -p your_password
"""
...
def get_grade_info(i, table):
    ...
    for info in table:
        if len(info('td')) == 8:
            result.append([info('td')[2].text, info('td')[3].text, info('td')[5].text])
