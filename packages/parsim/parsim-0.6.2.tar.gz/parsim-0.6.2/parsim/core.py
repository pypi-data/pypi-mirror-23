...
def create_caselist_file(output_path, case_names, parameter_names=None, column_dict=None, case_dicts=None, csv=None):
    """
    Create and write a caselist to a specified file (``output_path``).
    ...
    case_names  --- List of case names (output in first column, "CASENAME")

    Mutually exclusive arguments:
    column_dict --- Dict from parameter name to list of values (one value per case).
    case_dicts  --- List of dicts from parameter name to value (one dict per case).

    Optional argument:
    parameter_names --- List of parameter names to output.
    csv             --- Flag to request output in CSV format. Default (False) is to output in fixed-width
                        whitespace-separated columns. If ``csv`` is one of the characters ",;%|#", this
                        will be used as separator. If ``csv`` is any other boolean "True" value, ";" is
                        used as separator.

    :param output_path: str
    :param case_names: list(str)
    :param parameter_names: list(str)
    :param column_dict: dict(name, list(value))
    :param case_dicts: list(dict(name, value))
    :param csv: str
    """
...
def create_parameter_file(output_path, parameters, parameter_names=None):
    ...
    Paremeter definitions are specified as dictionary (``parameters``).
    ...
    Mandatory arguments:
    output_path --- Path to output file.
    parameters  --- Dictionary with parameter/value pairs.

    Optional argument:
    parameter_names --- List of parameter names, to control order of parameters in output.
    csv --- Flag to request output in CSV format. Default (False) is to output in fixed-width
            whitespace-separated columns. If ``csv`` is one of the characters ",;%|#", this will be
            used as separator. If ``csv`` is any other boolean "True" value, ";" is used as separator.

    :param output_path: str
    :param parameters: dict(name, value)
    :param parameter_names: list(str)
    """
...
class ParsimObject:
    ...
    def delete(self, force=False, logging=True):
        """Delete ParsimObject directory on disk."""
    ...
    def info(self):
        ...
        """Show some basic information about the ParsimObject."""
...
class Project(ParsimObject):
    ...
    def find_target(self, target_arg):
        """
        Process a command-line target argument and generate ParsimErrors if invalid.
        """
        if self.exists:
            if target_arg:
...
class Study(ParsimObject):
    ...
    def _create_cases(self, template_path, user_parameters, cases):
        """
        Create cases inside study, given list of (case_id, case_dict) tuples.
        :param template_path: Path to valid model template
        :param user_parameters: Dict with parameters specified by user on the commandline or in parameter file
        :param cases: List of (case_id, case_dict) tuples, as returned by parse_caselist_file()
        :return:
        """
        ...
        if cases:
            create_caselist_file(caselist_path, case_id_list, parameter_names=self.get('caselist_param_list'),
                                 case_dicts=[d for n, d in cases])
    ...
    def run_dakota(self, dakota_file=None, executable=None, pre_run=False,
        ...
        if pre_run:
            ...
            caselist_path = os.path.join(self.path, 'study.caselist')
            create_caselist_file(caselist_path, case_id_list, parameter_names=caselist_param_list,
                                 case_dicts=list(case_dicts))
    ...
    def collect(self, **kwargs):
        ...
        try:
            ...
            create_caselist_file(output_path, cases, parameter_names=column_keys, case_dicts=results, csv=csv)
            # Always output standard csv file "collect.csv" (for internal use, e.g. statistics)
            create_caselist_file(os.path.join(self.path, 'collect.csv'), cases, parameter_names=column_keys,
                                 case_dicts=results, csv=';')
