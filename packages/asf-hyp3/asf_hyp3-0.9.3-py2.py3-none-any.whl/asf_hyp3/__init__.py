...
class API:
    ...
    # attempt to login
    def __init__(self, username=None, password=None, api_key=None, url="https://api.hyp3.asf.alaska.edu/"):
        self.url = url
    ...
    def _did_all_succeeded(self, statuses):
        # make status into a list
        if not isinstance(statuses, list):
            # confusing, but in this case statuses is just one status.
            statuses = [statuses]

        # Get total and number passed
        num_successes = len(
            [status for status in statuses if status.get('status') == "SUCCESS"])
        total = len(statuses)
    ...
    def login(self, password=None, doesPrint=True):
        hadUsername = True
        ...
        if self.username is None:
            ...
            try:
                ...
                print("{} -- error with username...".format(str(e)))
                return
    ...
    def one_time_process(self,
        ...
        if granules is not None:
            ...
            if return_object is False:
                ...
                if self._did_all_succeeded(statuses):
                    jobs = self._ids_to_items(statuses, "job")
        ...
        # Otherwise just a single granule
            ...
            if return_object is True and self._did_all_succeeded(status):
...
def __test_job_processes(test, doesPrint=True):
    ...
    status = test.one_time_process(
        granule, process_id, return_object=True, priority=5, message="Hello thar!")
    print(status)
...
def __test_one_time_processes(interface):
    ...
    granule = "S1A_IW_SLC__1SSV_20150127T095630_20150127T095657_004355_005505_7518"
    status = interface.one_time_process(granule, 2, priority=5, return_object=False)
    print(status)
    ...
    status = interface.one_time_process(granules, 1, return_object=False)
    print(status)
...
if __name__ == "__main__":
    ...
    devel_api = API(
        "wbhorn", api_key="cYr6hzfg49efLvjXgNZbu6BgxmkQ7qYvy00iLBeksngZFoheVe2kpoijFgjLdlL2", url=url_local)

    processes = devel_api.get_processes()

    __test_one_time_processes(devel_api)
