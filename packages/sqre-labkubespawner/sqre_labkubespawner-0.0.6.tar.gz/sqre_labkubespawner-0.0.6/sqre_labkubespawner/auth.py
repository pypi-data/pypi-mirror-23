...
class _ExpiringDict(dict):
    ...
    A monotonic timer is used (time.monotonic).  Or would be if we had it,
    which under Python 2 we don't, so we use time.time there.
    ...
    def __setitem__(self, key, value):
        ...
        self.timestamps[key] = self.time_monotonic()
...
class HubAuth(Configurable):
    ...
    def _check_hub_authorization(self, url, cache_key=None, use_cache=True):
        ...
        if use_cache:
            ...
            # check for a cached reply, so we don't check with the Hub if we
            # don't have to
    ...
    def _api_request(self, method, url, **kwargs):
        ...
        if r.status_code == 404 and allow_404:
            ...
            app_log.error(
                "I don't have permission to check authorization with JupyterHub, my auth token may have expired: [%i] %s", r.status_code, r.reason)
            app_log.error(r.text)
            raise HTTPError(
                500, "Permission failure checking authorization, I may need a new token")
        elif r.status_code >= 500:
            app_log.error(
                "Upstream failure verifying auth token: [%i] %s", r.status_code, r.reason)
            app_log.error(r.text)
            raise HTTPError(
